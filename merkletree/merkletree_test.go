...
// TestRefHasher tests that the RefHasher computes the expected MerkleTree hash for
...
func TestRefHasher(t *testing.T) {
	...
	// the test struct is used to specify the expected MerkleTree hash for data
...
func testHasherReuse(i int, t *testing.T) {
	...
	for i := 0; i < 500; i++ {
		...
		err := testHasherCorrectness(merkletree, hasher, data, n, 128)
		if err != nil {
			t.Fatal(err)
		}
	}
...
func testBaseHasher(hasher BaseHasher, d []byte, n, count int) error {
	...
	defer pool.Drain(0)
	merkletree := New(pool)
	return testHasherCorrectness(merkletree, hasher, d, n, count)
}

func testHasherCorrectness(merkletree hash.Hash, hasher BaseHasher, d []byte, n, count int) (err error) {
	data := d[:n]
	rmerkletree := NewRefHasher(hasher, count)
	exp := rmerkletree.Hash(data)
	timeout := time.NewTimer(time.Second)
	c := make(chan error)

	go func() {
		merkletree.Reset()
		merkletree.Write(data)
		got := merkletree.Sum(nil)
	...
	select {
	case <-timeout.C:
		err = fmt.Errorf("MerkleTree hash calculation timed out")
	case err = <-c:
	}
	return err
}
...
func BenchmarkMerkleTreeBaseline_4k(t *testing.B)   { benchmarkMerkleTreeBaseline(4096, t) }
func BenchmarkMerkleTreeBaseline_2k(t *testing.B)   { benchmarkMerkleTreeBaseline(4096/2, t) }
func BenchmarkMerkleTreeBaseline_1k(t *testing.B)   { benchmarkMerkleTreeBaseline(4096/4, t) }
func BenchmarkMerkleTreeBaseline_512b(t *testing.B) { benchmarkMerkleTreeBaseline(4096/8, t) }
func BenchmarkMerkleTreeBaseline_256b(t *testing.B) { benchmarkMerkleTreeBaseline(4096/16, t) }
func BenchmarkMerkleTreeBaseline_128b(t *testing.B) { benchmarkMerkleTreeBaseline(4096/32, t) }
...
// benchmarks the minimum hashing time for a balanced (for simplicity) MerkleTree
// by doing count/segmentsize parallel hashings of 2*segmentsize bytes
// doing it on n maxproccnt each reusing the base hasher
// the premise is that this is the minimum computation needed for a MerkleTree
// therefore this serves as a theoretical optimum for concurrent implementations
func benchmarkMerkleTreeBaseline(n int, t *testing.B) {
...
func benchmarkRefHasher(n int, t *testing.B) {
	...
	tdata.Read(data)
	hasher := sha3.NewKeccak256
	rmerkletree := NewRefHasher(hasher, 128)

	t.ReportAllocs()
	t.ResetTimer()
	for i := 0; i < t.N; i++ {
		rmerkletree.Hash(data)
	}
}
