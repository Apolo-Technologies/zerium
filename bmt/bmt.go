...
// Package merkletree provides a binary merkle tree implementation
package merkletree
...
/*
Binary MerkleTree Hash is a hash function over arbitrary datachunks of limited size
...
The MerkleTree is optimal for providing compact inclusion proofs, i.e. prove that a
segment is a substring of a chunk starting at a particular offset
The size of the underlying segments is fixed at 32 bytes (called the resolution
of the MerkleTree hash), the Zvm word size to optimize for on-chain MerkleTree verification
...
const (
	...
	// DefaultPoolSize is the maximum number of merkletree trees used by the hashers, i.e,
	// the maximum number of concurrent MerkleTree hashing operations performed by the same hasher
	DefaultPoolSize = 8
)

// BaseHasher is a hash.Hash constructor function used for the base hash of the  MerkleTree.
type BaseHasher func() hash.Hash

// Hasher a reusable hasher for fixed maximum size chunks representing a MerkleTree
...
type Hasher struct {
	pool        *TreePool   // MerkleTree resource pool
	merkletree  *Tree       // prebuilt MerkleTree resource for flowcontrol and proofs
...
// Node is a reuseable segment hasher representing a node in a MerkleTree
...
type Node struct {
	...
	root         bool  // whether the node is root to a smaller MerkleTree
	isLeft       bool  // whether it is left side of the parent double segment
	unbalanced   bool  // indicates if a node has only the left segment
	parent       *Node // MerkleTree connections
	state        int32 // atomic increment impl concurrent boolean toggle
	left, right  []byte
}

// NewNode constructor for segment hasher nodes in the MerkleTree
...
// Tree is a reusable control structure representing a MerkleTree
...
// Draw draws the MerkleTree (badly)
...
// NewTree initialises the Tree by building up the nodes of a MerkleTree
...
func (self *Hasher) Sum(b []byte) (r []byte) {
	...
	// sha3(length + MerkleTree(pure_chunk))
...
// caller must call this on a MerkleTree Hasher being written to
...
func (self *Hasher) releaseTree() {
	if self.merkletree != nil {
		n := self.merkletree.leaves[self.cur]
		...
		self.pool.Release(self.merkletree)
		self.merkletree = nil
...
// getTree obtains a MerkleTree resource by reserving one from the pool
