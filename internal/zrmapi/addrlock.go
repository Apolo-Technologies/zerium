...
type AddrLocker struct {
	mu    sync.Mutex
	locks map[zrmcom.Address]*sync.Mutex
}

// lock returns the lock of the given address.
func (l *AddrLocker) lock(address zrmcom.Address) *sync.Mutex {
	l.mu.Lock()
	defer l.mu.Unlock()
	if l.locks == nil {
		l.locks = make(map[zrmcom.Address]*sync.Mutex)
	}
...
func (l *AddrLocker) LockAddr(address zrmcom.Address) {
	l.lock(address).Lock()
}

// UnlockAddr unlocks the mutex of the given account.
func (l *AddrLocker) UnlockAddr(address zrmcom.Address) {
