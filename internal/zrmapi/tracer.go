...
func (dw *dbWrapper) getBalance(addr []byte) *big.Int {
	return dw.db.GetBalance(zrmcom.BytesToAddress(addr))
}

// getNonce retrieves an account's nonce
func (dw *dbWrapper) getNonce(addr []byte) uint64 {
	return dw.db.GetNonce(zrmcom.BytesToAddress(addr))
}

// getCode retrieves an account's code
func (dw *dbWrapper) getCode(addr []byte) []byte {
	return dw.db.GetCode(zrmcom.BytesToAddress(addr))
}

// getState retrieves an account's state data for the given hash
func (dw *dbWrapper) getState(addr []byte, hash zrmcom.Hash) zrmcom.Hash {
	return dw.db.GetState(zrmcom.BytesToAddress(addr), hash)
}

// exists returns true iff the account exists
func (dw *dbWrapper) exists(addr []byte) bool {
	return dw.db.Exist(zrmcom.BytesToAddress(addr))
}
...
func (c *contractWrapper) caller() zrmcom.Address {
	return c.contract.Caller()
}

func (c *contractWrapper) address() zrmcom.Address {
...
func (jst *JavascriptTracer) CaptureState(env *vm.Zvm, pc uint64, op vm.OpCode, gas, cost uint64, memory *vm.Memory, stack *vm.Stack, contract *vm.Contract, depth int, err error) error {
	if jst.err == nil {
		jst.op.op = op
