...
const (
	...
	subscribeTimeout     = 5 * time.Second  // overall timeout zrm_subscribe, rpc_modules calls
)
...
type requestOp struct {
	...
	sub  *ClientSubscription  // only set for ZrmSubscribe requests
}
...
// ZrmSubscribe registers a subscripion under the "zrm" namespace.
func (c *Client) ZrmSubscribe(ctx context.Context, channel interface{}, args ...interface{}) (*ClientSubscription, error) {
	return c.Subscribe(ctx, "zrm", channel, args...)
}
...
func (c *Client) handleResponse(msg *jsonrpcMessage) {
	...
	// indicates success. ZrmSubscribe gets unblocked in either case through
	// the op.resp channel.
	defer close(op.resp)
	if msg.Error != nil {
		op.err = msg.Error
...
// A ClientSubscription represents a subscription established through ZrmSubscribe.
