...
// Package zrmstats implements the network stats reporting service.
package zrmstats
...
// Service implements an Zerium netstats reporting daemon that pushes local
	...
	for {
		...
		if len(msg["emit"]) == 2 && command == "history" {
			...
			if !ok {
				...
				s.histCh <- nil
				continue // Zrmstats sometime sends invalid history requests, ignore those
			}
...
func (s *Service) reportLatency(conn *websocket.Conn) error {
	// Send the current time to the zrmstats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to zrmstats", "latency", latency)
...
func (s *Service) reportBlock(conn *websocket.Conn, block *types.Block) error {
	...
	log.Trace("Sending new block to zrmstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) assembleBlockStats(block *types.Block) *blockStats {
	...
	if s.zrm != nil {
		...
		if block == nil {
			block = s.zrm.BlockChain().CurrentBlock()
		}
		header = block.Header()
		td = s.zrm.BlockChain().GetTd(header.Hash(), header.Number.Uint64())
...
func (s *Service) reportHistory(conn *websocket.Conn, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to zrmstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportStats(conn *websocket.Conn) error {
	...
	if s.zrm != nil {
		mining = s.zrm.Miner().Mining()
		hashrate = int(s.zrm.Miner().HashRate())

		sync := s.zrm.Downloader().Progress()
		syncing = s.zrm.BlockChain().CurrentHeader().Number.Uint64() >= sync.HighestBlock
