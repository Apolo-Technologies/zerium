...
import (
	...
	"github.com/apolo-technologies/zerium/swarm/fuse"
...
type Swarm struct {
	...
	storage     storage.ChunkStore     // internal access to storage, zrmcom interface to cloud storage backends
...
func NewSwarm(ctx *node.ServiceContext, backend chequebook.Backend, ensClient *ethclient.Client, config *api.Config, swapEnabled, syncEnabled bool, cors string) (self *Swarm, err error) {
	if bytes.Equal(zrmcom.FromHex(config.PublicKey), storage.ZeroKey) {
		return nil, fmt.Errorf("empty public key")
	}
	if bytes.Equal(zrmcom.FromHex(config.BzzKey), storage.ZeroKey) {
	...
	self.hive = network.NewHive(
		zrmcom.HexToHash(self.config.BzzKey), // key to hive (kademlia base address)
	...
	log.Debug(fmt.Sprintf("-> ABT17 virtual server API"))
...
func (self *Swarm) Start(srv *peertopeer.Server) error {
...
// implements the node.Service interface
func (self *Swarm) Protocols() []peertopeer.Protocol {
	...
	return []peertopeer.Protocol{proto}
}
