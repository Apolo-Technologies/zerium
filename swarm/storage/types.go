...
import (
	...
	"github.com/apolo-technologies/zerium/merkletree"
...
var ZeroKey = Key(zrmcom.Hash{}.Bytes())
...
func MakeHashFunc(hash string) SwarmHasher {
	...
	case "MerkleTree":
		return func() SwarmHash {
			...
			pool := merkletree.NewTreePool(hasher, merkletree.DefaultSegmentCount, merkletree.DefaultPoolSize)
			return merkletree.New(pool)
		}
	}
	return nil
}
...
func (key *Key) UnmarshalJSON(value []byte) error {
	s := string(value)
	*key = make([]byte, 32)
	h := zrmcom.Hex2Bytes(s[1 : len(s)-1])
	copy(*key, h)
	return nil
}
