...
func (s *Server) HandleGetRaw(w http.ResponseWriter, r *Request) {
	...
	if r.uri.Path != "" {
		...
		key = storage.Key(zrmcom.Hex2Bytes(entry.Hash))
	}
...
func (s *Server) HandleGetFiles(w http.ResponseWriter, r *Request) {
	...
	err = walker.Walk(func(entry *api.ManifestEntry) error {
		...
		reader := s.api.Retrieve(storage.Key(zrmcom.Hex2Bytes(entry.Hash)))
...
// contained in <manifest> under <path> grouped into zrmcom prefixes using
// "/" as a delimiter
...
func (s *Server) getManifestList(key storage.Key, prefix string) (list api.ManifestList, err error) {
	...
	err = walker.Walk(func(entry *api.ManifestEntry) error {
		...
		if entry.ContentType != api.ManifestType {
			...
			// zrmcom prefix to the list, otherwise add the entry
		...
		// path after the prefix contains a slash, add a zrmcom prefix
