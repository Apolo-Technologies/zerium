...
func testGet(t *testing.T, api *Api, bzzhash, path string) *testResponse {
	key := storage.Key(zrmcom.Hex2Bytes(bzzhash))
...
func newTestResolver(addr string) *testResolver {
	r := &testResolver{}
	if addr != "" {
		hash := zrmcom.HexToHash(addr)
		r.hash = &hash
	}
	return r
}

func (t *testResolver) Resolve(addr string) (zrmcom.Hash, error) {
	if t.hash == nil {
		return zrmcom.Hash{}, fmt.Errorf("DNS name not found: %q", addr)
	}
	return *t.hash, nil
}
