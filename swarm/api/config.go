...
type Config struct {
	...
	*network.SyncParams
	Contract    zrmcom.Address
	EnsRoot     zrmcom.Address
	EnsApi      string
	Path        string
	ListenAddr  string
	Port        string
...
func NewDefaultConfig() (self *Config) {
	...
	self = &Config{
		...
		EnsApi:        node.DefaultIPCEndpoint("zrmd"),
...
func (self *Config) Init(prvKey *ecdsa.PrivateKey) {
	...
	self.Path = filepath.Join(self.Path, "bzz-"+zrmcom.Bytes2Hex(address.Bytes()))
