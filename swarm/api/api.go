...
type Resolver interface {
	Resolve(string) (zrmcom.Hash, error)
}

/*
Api implements webserver/file system related content storage and retrieval
on top of the dpa
it is the public interface of the dpa which is included in the zerium stack
*/
type Api struct {
	dpa *storage.DPA
	dns Resolver
...
func (self *Api) Resolve(uri *URI) (storage.Key, error) {
	...
	if uri.Immutable() {
		...
		return zrmcom.Hex2Bytes(uri.Addr), nil
	}
...
func (self *Api) Get(key storage.Key, path string) (reader storage.LazySectionReader, mimeType string, status int, err error) {
	...
	if entry != nil {
		key = zrmcom.Hex2Bytes(entry.Hash)
		status = entry.Status
