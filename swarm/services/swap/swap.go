...
type PayProfile struct {
	...
	Contract    zrmcom.Address // address of chequebook contract
	Beneficiary zrmcom.Address // recipient address for swarm sales revenue
	privateKey  *ecdsa.PrivateKey
	publicKey   *ecdsa.PublicKey
	owner       zrmcom.Address
...
//have been evaluated
func (self *SwapParams) Init(contract zrmcom.Address, prvkey *ecdsa.PrivateKey) {
	pubkey := &prvkey.PublicKey

	self.PayProfile = &PayProfile{
		PublicKey:   zrmcom.ToHex(crypto.FromECDSAPub(pubkey)),
		Contract:    contract,
...
func NewSwap(local *SwapParams, remote *SwapProfile, backend chequebook.Backend, proto swap.Protocol) (self *swap.Swap, err error) {
	...
	if !ok {
		...
		in, err = chequebook.NewInbox(local.privateKey, remote.Contract, local.Beneficiary, crypto.ToECDSAPub(zrmcom.FromHex(remote.PublicKey)), backend)
		if err != nil {
...
func deployChequebookLoop(opts *bind.TransactOpts, backend chequebook.Backend) (addr zrmcom.Address, err error) {
...
func (self *SwapParams) newChequebookFromContract(path string, backend chequebook.Backend) error {
	hexkey := zrmcom.Bytes2Hex(self.Contract.Bytes())
