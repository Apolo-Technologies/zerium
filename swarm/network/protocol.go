...
bzz protocol handler is registered on the peertopeer server.
...
type bzz struct {
	...
	peer       *peertopeer.Peer            // the peertopeer peer object
	rw         peertopeer.MsgReadWriter    // messageReadWriter to send messages to
...
the Dev peertopeer layer then calls Run(p *peertopeer.Peer, rw peertopeer.MsgReadWriter) error
...
*/
func Bzz(cloud StorageHandler, backend chequebook.Backend, hive *Hive, dbaccess *DbAccess, sp *bzzswap.SwapParams, sy *SyncParams, networkId uint64) (peertopeer.Protocol, error) {
	...
	if err != nil {
		return peertopeer.Protocol{}, fmt.Errorf("error setting up request db: %v", err)
	}
	if networkId == 0 {
		networkId = NetworkId
	}
	return peertopeer.Protocol{
		Name:    "bzz",
		Version: Version,
...
*/
func run(requestDb *storage.LDBDatabase, depo StorageHandler, backend chequebook.Backend, hive *Hive, dbaccess *DbAccess, sp *bzzswap.SwapParams, sy *SyncParams, networkId uint64, p *peertopeer.Peer, rw peertopeer.MsgReadWriter) (err error) {
...
func (self *bzz) Drop() {
	self.peer.Disconnect(peertopeer.DiscSubprotocolError)
}
...
func (self *bzz) handleStatus() (err error) {
	...
	err = peertopeer.Send(self.rw, statusMsg, handshake)
	if err != nil {
		return err
	}

	// read and handle remote status
	var msg peertopeer.Msg
	msg, err = self.rw.ReadMsg()
	if err != nil {
		return err
	}
...
func (self *bzz) send(msg uint64, data interface{}) error {
	...
	err := peertopeer.Send(self.rw, msg, data)
	if err != nil {
		self.Drop()
	}
	return err
