...
#
# The cluster contains a bootnode, a zrmd node and multiple swarm nodes, with
...
# start_bootnode starts a bootnode which is used to bootstrap the zrmd and
# swarm nodes
start_bootnode() {
...
# start_zrmd_node starts a zrmd node with --datadir pointing at <base-dir>/zrmd
# and a single, unlocked account with password "zrmd"
start_zrmd_node() {
  local dir="${base_dir}/zrmd"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get zrmd account address"
  fi
  ...
  start_node "zrmd" "${GETH_IP}" "$(which zrmd)" ${args[@]}
}
...
create_account() {
  ...
  zrmd --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
