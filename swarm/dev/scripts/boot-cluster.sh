...
#
# The cluster contains a bootnode, a zaed node and multiple swarm nodes, with
...
# static zaed configuration
zaed_IP="192.168.33.3"
zaed_RPC_PORT="8545"
zaed_RPC_URL="http://${zaed_IP}:${zaed_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the zaed and
# swarm nodes
start_bootnode() {
...
# start_zaed_node starts a zaed node with --datadir pointing at <base-dir>/zaed
# and a single, unlocked account with password "zaed"
start_zaed_node() {
  local dir="${base_dir}/zaed"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get zaed account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${zaed_IP}"
    --rpcport   "${zaed_RPC_PORT}"
    --verbosity "6"
  )

  start_node "zaed" "${zaed_IP}" "$(which zaed)" ${args[@]}
}
...
create_account() {
  ...
  zaed --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
