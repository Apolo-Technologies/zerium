...
#
# The cluster contains a bootnode, a gabt node and multiple swarm nodes, with
...
# static gabt configuration
GABT_IP="192.168.33.3"
GABT_RPC_PORT="8545"
GABT_RPC_URL="http://${GABT_IP}:${GABT_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the gabt and
# swarm nodes
start_bootnode() {
...
# start_gabt_node starts a gabt node with --datadir pointing at <base-dir>/gabt
# and a single, unlocked account with password "gabt"
start_gabt_node() {
  local dir="${base_dir}/gabt"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gabt account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${GABT_IP}"
    --rpcport   "${GABT_RPC_PORT}"
    --verbosity "6"
  )

  start_node "gabt" "${GABT_IP}" "$(which gabt)" ${args[@]}
}
...
create_account() {
  ...
  gabt --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
