...
#
# The cluster contains a bootnode, a gzrm node and multiple swarm nodes, with
...
# start_bootnode starts a bootnode which is used to bootstrap the gzrm and
# swarm nodes
start_bootnode() {
...
start_gzrm_node() {
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gzrm account address"
  fi
...
create_account() {
  ...
  gzrm --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
