...
// mapped from textual names used in the tests below to actual Zerium private
...
func (ap *testerAccountPool) address(account string) zrmcom.Address {
	// Ensure we have a persistent key for the account
	if ap.accounts[account] == nil {
		ap.accounts[account], _ = crypto.GenerateKey()
	}
	// Resolve and return the Zerium address
...
func (r *testerChainReader) GetHeader(zrmcom.Hash, uint64) *types.Header { panic("not supported") }
func (r *testerChainReader) GetBlock(zrmcom.Hash, uint64) *types.Block   { panic("not supported") }
func (r *testerChainReader) GetHeaderByHash(zrmcom.Hash) *types.Header   { panic("not supported") }
...
func TestVoting(t *testing.T) {
	...
	for i, tt := range tests {
		...
		signers := make([]zrmcom.Address, len(tt.signers))
		...
		genesis := &core.Genesis{
			ExtraData: make([]byte, extraVanity+zrmcom.AddressLength*len(signers)+extraSeal),
		}
		for j, signer := range signers {
			copy(genesis.ExtraData[extraVanity+j*zrmcom.AddressLength:], signer[:])
		}
		...
		signers = make([]zrmcom.Address, len(tt.results))
