...
func (zrmash *Zrmash) VerifyHeader(chain consensus.ChainReader, header *types.Header, seal bool) error {
	...
	return zrmash.verifyHeader(chain, header, parent, false, seal)
}
	...
	for i := 0; i < workers; i++ {
		go func() {
			for index := range inputs {
				errors[index] = zrmash.verifyHeaderWorker(chain, headers, seals, index)
				done <- index
			}
		}()
	}
...
func (zrmash *Zrmash) verifyHeaderWorker(chain consensus.ChainReader, headers []*types.Header, seals []bool, index int) error {
	...
	return zrmash.verifyHeader(chain, headers[index], parent, false, seals[index])
}
...
func (zrmash *Zrmash) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
	...
	for _, uncle := range block.Uncles() {
		...
		if err := zrmash.verifyHeader(chain, uncle, ancestors[uncle.ParentHash], true, true); err != nil {
			return err
		}
	}
	return nil
...
func (zrmash *Zrmash) verifyHeader(chain consensus.ChainReader, header, parent *types.Header, uncle bool, seal bool) error {
	...
	if seal {
		if err := zrmash.VerifySeal(chain, header); err != nil {
			return err
		}
	}
...
func (zrmash *Zrmash) VerifySeal(chain consensus.ChainReader, header *types.Header) error {
	...
	if zrmash.fakeMode {
		...
		if zrmash.fakeFail == header.Number.Uint64() {
	...
	if zrmash.shared != nil {
		return zrmash.shared.VerifySeal(chain, header)
	}
