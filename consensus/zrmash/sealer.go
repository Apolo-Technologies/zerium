...
func (zrmash *Ethash) Seal(chain consensus.ChainReader, block *types.Block, stop <-chan struct{}) (*types.Block, error) {
	...
	if zrmash.shared != nil {
		return zrmash.shared.Seal(chain, block, stop)
	}
	...
	if zrmash.rand == nil {
		...
		zrmash.rand = rand.New(rand.NewSource(seed.Int64()))
	}
	zrmash.lock.Unlock()
	if threads == 0 {
	...
	for i := 0; i < threads; i++ {
		...
		go func(id int, nonce uint64) {
			defer pend.Done()
			zrmash.mine(block, id, nonce, abort, found)
		}(i, uint64(zrmash.rand.Int63()))
	}
	...
	case <-zrmash.update:
		...
		close(abort)
		pend.Wait()
		return zrmash.Seal(chain, block, stop)
	}
...
func (zrmash *Ethash) mine(block *types.Block, id int, seed uint64, abort chan struct{}, found chan *types.Block) {
	...
	logger.Trace("Started zrmash search for new nonces", "seed", seed)
	for {
		select {
		case <-abort:
