...
func (s *PublicWhisperAPI) HasIdentity(identity string) (bool, error) {
	...
	return s.w.HasIdentity(crypto.ToECDSAPub(zrmcom.FromHex(identity))), nil
}
...
func (s *PublicWhisperAPI) NewIdentity() (string, error) {
	...
	return zrmcom.ToHex(crypto.FromECDSAPub(&identity.PublicKey)), nil
}
...
func (s *PublicWhisperAPI) NewFilter(args NewFilterArgs) (hexutil.Uint, error) {
	...
	var id hexutil.Uint
	filter := Filter{
		To:     crypto.ToECDSAPub(zrmcom.FromHex(args.To)),
		From:   crypto.ToECDSAPub(zrmcom.FromHex(args.From)),
...
func (s *PublicWhisperAPI) Post(args PostArgs) (bool, error) {
	...
	message := NewMessage(zrmcom.FromHex(args.Payload))
	options := Options{
		To:     crypto.ToECDSAPub(zrmcom.FromHex(args.To)),
	...
	// set sender identity
	if len(args.From) > 0 {
		if key := s.w.GetIdentity(crypto.ToECDSAPub(zrmcom.FromHex(args.From))); key != nil {
			options.From = key
		} else {
...
func (args *PostArgs) UnmarshalJSON(data []byte) (err error) {
	...
	for i, topic := range obj.Topics {
		args.Topics[i] = zrmcom.FromHex(topic)
	}
	...
	if obj.Topics != nil {
		...
		for i, condition := range topics {
			...
			for j, topic := range condition {
				topicsDecoded[i][j] = zrmcom.FromHex(topic)
			}
		}
...
type whisperFilter struct {
	...
	skip   map[zrmcom.Hash]struct{} // List of retrieved messages to avoid duplication
...
func (w *whisperFilter) messages() []*Message {
	...
	w.skip = make(map[zrmcom.Hash]struct{})
...
func newWhisperFilter(id hexutil.Uint, ref *Whisper) *whisperFilter {
	return &whisperFilter{
		...
		ref:    ref,
		update: time.Now(),
		skip:   make(map[zrmcom.Hash]struct{}),
	}
}
...
func NewWhisperMessage(message *Message) WhisperMessage {
	return WhisperMessage{
		...
		Payload: zrmcom.ToHex(message.Payload),
		From:    zrmcom.ToHex(crypto.FromECDSAPub(message.Recover())),
		To:      zrmcom.ToHex(crypto.FromECDSAPub(message.To)),
		Sent:    message.Sent.Unix(),
		TTL:     int64(message.TTL / time.Second),
		Hash:    zrmcom.ToHex(message.Hash.Bytes()),
	}
}
