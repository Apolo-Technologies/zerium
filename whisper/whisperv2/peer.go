...
type peer struct {
	host *Whisper
	peer *peertopeer.Peer
	ws   peertopeer.MsgReadWriter
...
func newPeer(host *Whisper, remote *peertopeer.Peer, rw peertopeer.MsgReadWriter) *peer {
	return &peer{
		host:  host,
		peer:  remote,
		ws:    rw,
...
func (self *peer) handshake() error {
	...
	go func() {
		errc <- peertopeer.SendItems(self.ws, statusCode, protocolVersion)
	}()
...
func (self *peer) expire() {
	...
	unmark := make(map[zrmcom.Hash]struct{})
	self.known.Each(func(v interface{}) bool {
		if !available.Has(v.(zrmcom.Hash)) {
			unmark[v.(zrmcom.Hash)] = struct{}{}
		}
		return true
	})
...
func (self *peer) broadcast() error {
	...
	if err := peertopeer.Send(self.ws, messagesCode, transmit); err != nil {
		return err
	}
