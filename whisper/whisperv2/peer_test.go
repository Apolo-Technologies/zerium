...
type testPeer struct {
	client *Whisper
	stream *peertopeer.MsgPipeRW
	termed chan struct{}
}

func startTestPeer() *testPeer {
	// Create a simulated P2P remote peer and data streams to it
	remote := peertopeer.NewPeer(discover.NodeID{}, "", nil)
	tester, tested := peertopeer.MsgPipe()
...
func startTestPeerInited() (*testPeer, error) {
	...
	if err := peertopeer.ExpectMsg(peer.stream, statusCode, []uint64{protocolVersion}); err != nil {
		peer.stream.Close()
		return nil, err
	}
	if err := peertopeer.SendItems(peer.stream, statusCode, protocolVersion); err != nil {
		peer.stream.Close()
		return nil, err
	}
	return peer, nil
...
func TestPeerStatusMessage(t *testing.T) {
	...
	if err := peertopeer.ExpectMsg(tester.stream, statusCode, []uint64{protocolVersion}); err != nil {
...
func TestPeerHandshakeFail(t *testing.T) {
	...
	if err := peertopeer.SendItems(tester.stream, messagesCode); err != nil {
...
func TestPeerHandshakeSuccess(t *testing.T) {
	...
	if err := peertopeer.SendItems(tester.stream, statusCode, protocolVersion); err != nil {
...
func TestPeerSend(t *testing.T) {
	...
	if err := peertopeer.ExpectMsg(tester.stream, messagesCode, payload); err != nil {
	...
	if err := peertopeer.ExpectMsg(tester.stream, messagesCode, []interface{}{}); err != nil {
		t.Fatalf("message mismatch: %v", err)
	}
}
...
func TestPeerDeliver(t *testing.T) {
	...
	if err := peertopeer.Send(tester.stream, messagesCode, []*Envelope{envelope}); err != nil {
