...
type Whisper struct {
	protocol peertopeer.Protocol
	filters  *filter.Filters

	keys map[string]*ecdsa.PrivateKey

	messages    map[zrmcom.Hash]*Envelope // Pool of messages currently tracked by this node
...
// New creates a Whisper client ready to communicate through the Zerium P2P
// network.
func New() *Whisper {
	whisper := &Whisper{
		filters:     filter.New(),
		keys:        make(map[string]*ecdsa.PrivateKey),
		messages:    make(map[zrmcom.Hash]*Envelope),
...
func (self *Whisper) Protocols() []peertopeer.Protocol {
	return []peertopeer.Protocol{self.protocol}
}
...
func (self *Whisper) Start(*peertopeer.Server) error {
...
func (self *Whisper) handlePeer(peer *peertopeer.Peer, rw peertopeer.MsgReadWriter) error {
...
func (self *Whisper) expire() {
	...
	for then, hashSet := range self.expirations {
		...
		hashSet.Each(func(v interface{}) bool {
			delete(self.messages, v.(zrmcom.Hash))
			return true
		})
