...
func main() {
	...
	name := zrmcom.MakeName("whisper-go", "1.0")
	shh := whisper.New()

	// Create an Zerium peer to communicate through
	server := peertopeer.Server{
		PrivateKey: key,
		MaxPeers:   10,
		Name:       name,
		Protocols:  []peertopeer.Protocol{shh.Protocol()},
		ListenAddr: ":30300",
		NAT:        nat.Any(),
	}
	fmt.Println("Starting Zerium peer...")
	if err := server.Start(); err != nil {
		fmt.Printf("Failed to start Zerium peer: %v.\n", err)
		os.Exit(1)
	}
