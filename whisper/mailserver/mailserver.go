...
func NewDbKey(t uint32, h zrmcom.Hash) *DBKey {
	const sz = zrmcom.HashLength + 4
	var k DBKey
	k.timestamp = t
	k.hash = h
	k.raw = make([]byte, sz)
...
func (s *WMailServer) validateRequest(peerID []byte, request *whisper.Envelope) (bool, uint32, uint32, whisper.TopicType) {
	...
	if decrypted == nil {
		log.Warn(fmt.Sprintf("Failed to decrypt peertopeer request"))
		return false, 0, 0, topic
	}

	if len(decrypted.Payload) < 8 {
		log.Warn(fmt.Sprintf("Undersized peertopeer request"))
		return false, 0, 0, topic
	}
	...
	if !bytes.Equal(peerID, src) {
		log.Warn(fmt.Sprintf("Wrong signature of peertopeer request"))
		return false, 0, 0, topic
	}
