...
// Whisper represents a dark communication interface through the Zerium
// network, using its very own P2P communication layer.
type Whisper struct {
	protocol peertopeer.Protocol // Protocol description and parameters
	...
	envelopes   map[zrmcom.Hash]*Envelope // Pool of envelopes currently tracked by this node
	...
	peertopeerMsgQueue  chan *Envelope // Message queue for peer-to-peer messages (not to be forwarded any further)
...
// New creates a Whisper client ready to communicate through the Zerium P2P network.
func New(cfg *Config) *Whisper {
	...
	whisper := &Whisper{
		...
		envelopes:    make(map[zrmcom.Hash]*Envelope),
		expirations:  make(map[uint32]*set.SetNonTS),
		peers:        make(map[*Peer]struct{}),
		messageQueue: make(chan *Envelope, messageQueueLimit),
		peertopeerMsgQueue:  make(chan *Envelope, messageQueueLimit),
	...
	// peertopeer whisper sub protocol handler
	whisper.protocol = peertopeer.Protocol{
		Name:    ProtocolName,
...
// MailServer will process all the incoming messages with peertopeerRequestCode.
func (w *Whisper) RegisterServer(server MailServer) {
	w.mailServer = server
}

// Protocols returns the whisper sub-protocols ran by this particular client.
func (w *Whisper) Protocols() []peertopeer.Protocol {
	return []peertopeer.Protocol{w.protocol}
}
...
// RequestHistoricMessages sends a message with peertopeerRequestCode to a specific peer,
...
func (w *Whisper) RequestHistoricMessages(peerID []byte, envelope *Envelope) error {
	...
	p.trusted = true
	return peertopeer.Send(p.ws, peertopeerRequestCode, envelope)
}
...
func (w *Whisper) SendP2PDirect(peer *Peer, envelope *Envelope) error {
	return peertopeer.Send(peer.ws, peertopeerCode, envelope)
}
...
func (w *Whisper) Start(*peertopeer.Server) error {
...
func (wh *Whisper) HandlePeer(peer *peertopeer.Peer, rw peertopeer.MsgReadWriter) error {
...
func (wh *Whisper) runMessageLoop(p *Peer, rw peertopeer.MsgReadWriter) error {
	for {
		...
		case peertopeerRequestCode:
			// Must be processed if mail server is implemented. Otherwise ignore.
			if wh.mailServer != nil {
				var request Envelope
				if err := packet.Decode(&request); err != nil {
					log.Warn("failed to decode peertopeer request message, peer will be disconnected", "peer", p.peer.ID(), "err", err)
					return errors.New("invalid peertopeer request")
				}
	...
	for expiry, hashSet := range w.expirations {
		if expiry < now {
			...
			hashSet.Each(func(v interface{}) bool {
				sz := w.envelopes[v.(zrmcom.Hash)].size()
				delete(w.envelopes, v.(zrmcom.Hash))
...
func (w *Whisper) isEnvelopeCached(hash zrmcom.Hash) bool {
	w.poolMu.Lock()
	defer w.poolMu.Unlock()
