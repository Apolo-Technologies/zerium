...
func (db *MemDatabase) Put(key []byte, value []byte) error {
	...
	db.db[string(key)] = zrmcom.CopyBytes(value)
	return nil
}
...
func (db *MemDatabase) Get(key []byte) ([]byte, error) {
	...
	if entry, ok := db.db[string(key)]; ok {
		return zrmcom.CopyBytes(entry), nil
	}
...
/*
func (db *MemDatabase) GetKeys() []*zrmcom.Key {
	data, _ := db.Get([]byte("KeyRing"))

	return []*zrmcom.Key{zrmcom.NewKeyFromBytes(data)}
}
*/
...
func (b *memBatch) Put(key, value []byte) error {
	b.writes = append(b.writes, kv{zrmcom.CopyBytes(key), zrmcom.CopyBytes(value)})
	b.size += len(value)
	return nil
}
