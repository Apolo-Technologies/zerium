...
func TestAccountListEmpty(t *testing.T) {
	gzrm := runGzrm(t, "account", "list")
	gzrm.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gzrm := runGzrm(t, "account", "list", "--datadir", datadir)
	defer gzrm.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gzrm := runGzrm(t, "account", "new", "--lightkdf")
	defer gzrm.ExpectExit()
	gzrm.Expect(`
...
`)
	gzrm.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestWalletImport(t *testing.T) {
	gzrm := runGzrm(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gzrm.ExpectExit()
	gzrm.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gzrm.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gzrm.StderrText(), m) {
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gzrm.SetTemplateFunc("keypath", func(file string) string {
