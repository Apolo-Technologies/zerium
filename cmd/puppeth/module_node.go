...
{{end}}
RUN \
  echo 'zrmd --cache 512 init /genesis.json' > zrmd.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.zerium/keystore/ && cp /signer.json /root/.zerium/keystore/' >> zrmd.sh && \{{end}}
	echo $'zrmd --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .BootV4}}--bootnodesv4 {{.BootV4}}{{end}} {{if .BootV5}}--bootnodesv5 {{.BootV5}}{{end}} {{if .Etherbase}}--etherbase {{.Etherbase}} --mine --minerthreads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --targetgaslimit {{.GasTarget}} --gasprice {{.GasPrice}}' >> zrmd.sh

ENTRYPOINT ["/bin/sh", "zrmd.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an Zerium node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    volumes:
      - {{.Datadir}}:/root/.zerium{{if .Ethashdir}}
...
// deployNode deploys a new Zerium node container to a remote machine via SSH,
	...
	if info.gasTarget > 0 {
		...
		if info.keyJSON != "" {
			...
			if err := json.Unmarshal([]byte(info.keyJSON), &key); err == nil {
				report["Signer account"] = zrmcom.HexToAddress(key.Address).Hex()
			} else {
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 zrmd --exec admin.nodeInfo.id attach", network, kind)); err != nil {
	...
	// Run a sanity check to see if the devpeertopeer is reachable
	port := infos.portmap[infos.envvars["FULL_PORT"]]
	if err = checkPort(client.server, port); err != nil {
		log.Warn(fmt.Sprintf("%s devpeertopeer port seems unreachable", strings.Title(kind)), "server", client.server, "port", port, "err", err)
	}
	// Assemble and return the useful infos
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.zerium"],
