...
RUN \
  apk add --update git go make gcc musl-dev ca-certificates linux-headers                             && \
	mkdir -p $GOPATH/src/github.com/zerium                                                            && \
	(cd $GOPATH/src/github.com/zerium && git clone --depth=1 https://github.com/apolo-technologies/zerium) && \
...
CMD [ \
	"/faucet", "--genesis", "/genesis.json", "--network", "{{.NetworkID}}", "--bootnodes", "{{.Bootnodes}}", "--zrmstats", "{{.Zrmstats}}", "--ethport", "{{.EthPort}}", \
