...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.zrmstats == "" {
		log.Error("No zrmstats server configured")
		return
	}
	...
	if err != nil {
		if boot {
			infos = &nodeInfos{portFull: 32310, peersTotal: 512, peersLight: 256}
		} else {
			infos = &nodeInfos{portFull: 32310, peersTotal: 50, peersLight: 0, gasTarget: 4.7, gasPrice: 18}
		}
	}
	...
	if infos.zrmstats == "" {
		...
		infos.zrmstats = w.readString() + ":" + w.conf.zrmstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.zrmstats)
		infos.zrmstats = w.readDefaultString(infos.zrmstats) + ":" + w.conf.zrmstats
	}
			...
			if infos.zeriumbase == "" {
				...
				for {
					if address := w.readAddress(); address != nil {
						infos.zeriumbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner user? (default = %s)\n", infos.zeriumbase)
				infos.zeriumbase = w.readDefaultAddress(common.HexToAddress(infos.zeriumbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootFull, w.conf.bootLight, infos); err != nil {
		log.Error("Failed to deploy Zerium node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
