...
func (w *wizard) makeGenesis() {
	...
	case choice == "" || choice == "2":
		...
		genesis.ExtraData = make([]byte, 32+len(signers)*zrmcom.AddressLength+65)
		for i, signer := range signers {
			copy(genesis.ExtraData[32+i*zrmcom.AddressLength:], signer[:])
		}
	...
	for i := int64(0); i < 256; i++ {
		genesis.Alloc[zrmcom.BigToAddress(big.NewInt(i))] = core.GenesisAccount{Balance: big.NewInt(1)}
	}
