...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'zrmd --cache 512 init /genesis.json' >> wallet.sh && \
...
func checkWallet(client *sshClient, network string) (*walletInfos, error) {
	...
	// Run a sanity check to see if the devpeertopeer and RPC ports are reachable
	nodePort := infos.portmap[infos.envvars["NODE_PORT"]]
	if err = checkPort(client.server, nodePort); err != nil {
		log.Warn(fmt.Sprintf("Wallet devpeertopeer port seems unreachable"), "server", client.server, "port", nodePort, "err", err)
	}
