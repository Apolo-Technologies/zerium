...
// config contains all the configurations needed by puppzrm that should be saved
// between sessions.
type config struct {
	...
	zrmstats  string        // Zrmstats settings to cache for node deploys
...
func (c config) flush() {
	...
	if err := ioutil.WriteFile(c.path, out, 0644); err != nil {
		log.Warn("Failed to save puppzrm configs", "file", c.path, "err", err)
	}
}
...
type wizard struct {
	...
	services map[string][]string   // Zerium services known to be running on servers
...
// converts it to an Zerium address. If an empty line is entered, the default
// value is returned.
