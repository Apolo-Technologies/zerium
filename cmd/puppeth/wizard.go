...
func (w *wizard) readAddress() *zrmcom.Address {
	for {
		...
		address := zrmcom.BigToAddress(bigaddr)
		return &address
	}
}
...
// value is returned.
func (w *wizard) readDefaultAddress(def zrmcom.Address) zrmcom.Address {
	for {
		// Read the address from the user
		fmt.Printf("> 0x")
		text, err := w.in.ReadString('\n')
		...
		bigaddr, _ := new(big.Int).SetString(text, 16)
		return zrmcom.BigToAddress(bigaddr)
	}
}
