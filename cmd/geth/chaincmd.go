...
Use "zerium dump 0" to dump the genesis block.`,
	}
)
...
func copyDb(ctx *cli.Context) error {
	...
	db, err := zrmdb.NewLDBDatabase(ctx.Args().First(), ctx.GlobalInt(utils.CacheFlag.Name), 256)
	if err != nil {
		return err
	}
	...
	start = time.Now()
	fmt.Println("Compacting entire database...")
	if err = chainDb.(*zrmdb.LDBDatabase).LDB().CompactRange(util.Range{}); err != nil {
		utils.Fatalf("Compaction failed: %v", err)
	}
	fmt.Printf("Compaction done in %v.\n\n", time.Since(start))
...
func removeDB(ctx *cli.Context) error {
	...
	for _, name := range []string{"chaindata", "lightchaindata"} {
		...
		default:
			start := time.Now()
			os.RemoveAll(dbdir)
			logger.Info("Database successfully deleted", "elapsed", zrmcom.PrettyDuration(time.Since(start)))
		}
	}
	return nil
}
...
func dump(ctx *cli.Context) error {
	...
	for _, arg := range ctx.Args() {
		...
		if hashish(arg) {
			block = chain.GetBlockByHash(zrmcom.HexToHash(arg))
		} else {
