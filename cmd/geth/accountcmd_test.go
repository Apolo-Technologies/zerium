...
func TestAccountListEmpty(t *testing.T) {
	zrmd := runGeth(t, "account", "list")
	zrmd.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	zrmd := runGeth(t, "account", "list", "--datadir", datadir)
	defer zrmd.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	zrmd := runGeth(t, "account", "new", "--lightkdf")
	defer zrmd.ExpectExit()
	zrmd.Expect(`
...
`)
	zrmd.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestWalletImport(t *testing.T) {
	zrmd := runGeth(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer zrmd.ExpectExit()
	zrmd.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(zrmd.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(zrmd.StderrText(), m) {
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	zrmd.SetTemplateFunc("keypath", func(file string) string {
