...
// zrmd is the official command-line client for Zerium.
package main
...
const (
	clientIdentifier = "zrmd" // Client identifier to advertise over the network
)
...
var (
	...
	relOracle = zrmcom.HexToAddress("0xfa7b9770ca4cb04296cac84f37736d4041251cdf")
...
func init() {
	...
	app.Copyright = "Copyright 2017-2017 The zerium Authors"
...
// zrmd is the main entry point into the system if no special subcommand is ran.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func zrmd(ctx *cli.Context) error {
...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) {
		...
		var zerium *eth.Zerium
		if err := stack.Service(&zerium); err != nil {
			utils.Fatalf("zerium service not running: %v", err)
		}
		...
		if threads := ctx.GlobalInt(utils.MinerThreadsFlag.Name); threads > 0 {
			...
			if th, ok := zerium.Engine().(threaded); ok {
				th.SetThreads(threads)
			}
		}
		// Set the gas price to the limits from the CLI and start mining
		zerium.TxPool().SetGasPrice(utils.GlobalBig(ctx, utils.GasPriceFlag.Name))
		if err := zerium.StartMining(true); err != nil {
