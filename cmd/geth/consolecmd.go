...
which exposes a node admin interface as well as the Ðapp JavaScript API.
See https://github.com/apolo-technologies/zerium/wiki/Javascipt-Console.
This command allows to open a console on a running zrmd node.`,
	}
...
// localConsole starts a new zrmd node, attaching a JavaScript console to it at the
// same time.
func localConsole(ctx *cli.Context) error {
	...
	if err != nil {
		utils.Fatalf("Failed to attach to the inproc zrmd: %v", err)
	}
...
// remoteConsole will connect to a remote zrmd instance, attaching a JavaScript
// console to it.
func remoteConsole(ctx *cli.Context) error {
	// Attach to a remotely running zrmd instance and start the JavaScript console
	...
	if endpoint == "" {
		...
		endpoint = fmt.Sprintf("%s/zrmd.ipc", path)
	}
	client, err := dialRPC(endpoint)
	if err != nil {
		utils.Fatalf("Unable to attach to remote zrmd: %v", err)
	}
	config := console.Config{
		DataDir: utils.MakeDataDir(ctx),
		DocRoot: ctx.GlobalString(utils.JSpathFlag.Name),
...
// for "zrmd attach" and "zrmd monitor" with no argument.
func dialRPC(endpoint string) (*rpc.Client, error) {
	if endpoint == "" {
		endpoint = node.DefaultIPCEndpoint(clientIdentifier)
	} else if strings.HasPrefix(endpoint, "rpc:") || strings.HasPrefix(endpoint, "ipc:") {
		// Backwards compatibility with zrmd < 1.5 which required
		// these prefixes.
		endpoint = endpoint[4:]
	}
...
// ephemeralConsole starts a new zrmd node, attaches an ephemeral JavaScript
