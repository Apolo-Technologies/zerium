...
import (
	...
	"github.com/apolo-technologies/zerium/peertopeer/netutil"
...
func setNodeKey(ctx *cli.Context, cfg *peertopeer.Config) {
	var (
...
func setBootstrapNodes(ctx *cli.Context, cfg *peertopeer.Config) {
...
// flags, reverting to pre-configured ones if none have been specified.
func setBootstrapNodesV5(ctx *cli.Context, cfg *peertopeer.Config) {
...
// line flags.
func setListenAddress(ctx *cli.Context, cfg *peertopeer.Config) {
...
func setDiscoveryV5Address(ctx *cli.Context, cfg *peertopeer.Config) {
	if ctx.GlobalIsSet(ListenPortFlag.Name) {
...
func setNAT(ctx *cli.Context, cfg *peertopeer.Config) {
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	if zrmcom.IsHexAddress(account) {
		return accounts.Account{Address: zrmcom.HexToAddress(account)}, nil
	}
	...
	log.Warn("Please use explicit addresses! (can search via `zrmd account list`)")
	log.Warn("-------------------------------------------------------------------")
...
func SetP2PConfig(ctx *cli.Context, cfg *peertopeer.Config) {
	setNodeKey(ctx, cfg)
	setNAT(ctx, cfg)
	...
	if ctx.GlobalBool(DeveloperFlag.Name) {
		// --dev mode can't use peertopeer networking.
		cfg.MaxPeers = 0
		cfg.ListenAddr = ":0"
...
func MakeConsolePreloads(ctx *cli.Context) []string {
	...
	for _, file := range strings.Split(ctx.GlobalString(PreloadJSFlag.Name), ",") {
		preloads = append(preloads, zrmcom.AbsolutePath(assets, strings.TrimSpace(file)))
	}
	return preloads
}
...
// new format.
//
// e.g. zrmd account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// zrmd --keystore /tmp/mykeystore --lightkdf account new
//
