...
func runCmd(ctx *cli.Context) error {
	...
	var (
		...
		sender      = zrmcom.StringToAddress("sender")
		receiver    = zrmcom.StringToAddress("receiver")
	)
	...
	if ctx.GlobalString(SenderFlag.Name) != "" {
		sender = zrmcom.HexToAddress(ctx.GlobalString(SenderFlag.Name))
	}
	statedb.CreateAccount(sender)

	if ctx.GlobalString(ReceiverFlag.Name) != "" {
		receiver = zrmcom.HexToAddress(ctx.GlobalString(ReceiverFlag.Name))
	}
	...
	if ctx.GlobalString(CodeFileFlag.Name) != "" {
		...
		code = zrmcom.Hex2Bytes(string(bytes.TrimRight(hexcode, "\n")))

	} else if ctx.GlobalString(CodeFlag.Name) != "" {
		code = zrmcom.Hex2Bytes(ctx.GlobalString(CodeFlag.Name))
	...
	if ctx.GlobalBool(CreateFlag.Name) {
		input := append(code, zrmcom.Hex2Bytes(ctx.GlobalString(InputFlag.Name))...)
		ret, _, leftOverGas, err = runtime.Create(input, &runtimeConfig)
	} else {
		if len(code) > 0 {
			statedb.SetCode(receiver, code)
		}
		ret, leftOverGas, err = runtime.Call(receiver, zrmcom.Hex2Bytes(ctx.GlobalString(InputFlag.Name)), &runtimeConfig)
	}
