...
func (l *JSONLogger) CaptureState(env *vm.Zvm, pc uint64, op vm.OpCode, gas, cost uint64, memory *vm.Memory, stack *vm.Stack, contract *vm.Contract, depth int, err error) error {
	log := vm.StructLog{
		Pc:         pc,
		Op:         op,
		Gas:        gas,
...
func (l *JSONLogger) CaptureEnd(output []byte, gasUsed uint64, t time.Duration, err error) error {
	...
	if err != nil {
		return l.encoder.Encode(endLog{zrmcom.Bytes2Hex(output), math.HexOrDecimal64(gasUsed), t, err.Error()})
	}
	return l.encoder.Encode(endLog{zrmcom.Bytes2Hex(output), math.HexOrDecimal64(gasUsed), t, ""})
}
