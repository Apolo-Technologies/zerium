...
import (
	...
	"github.com/apolo-technologies/zerium/peertopeer/discover"
	"github.com/apolo-technologies/zerium/peertopeer/discv5"
	"github.com/apolo-technologies/zerium/peertopeer/nat"
...
var (
	...
	ethPortFlag = flag.Int("ethport", 30303, "Listener port for the devpeertopeer connection")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Zerium and the faucet")
)
...
type request struct {
	...
	Account zrmcom.Address     `json:"account"` // Zerium address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Zerium light client.
type faucet struct {
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	// Assemble the raw devpeertopeer protocol stack
	stack, err := node.New(&node.Config{
		Name:    "zrmd",
		Version: params.Version,
		DataDir: filepath.Join(os.Getenv("HOME"), ".faucet"),
		P2P: peertopeer.Config{
			NAT:              nat.Any(),
			NoDiscovery:      true,
			DiscoveryV5:      true,
	...
	// Assemble the Zerium light client protocol
...
// close terminates the Zerium connection and tears down the faucet.
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Retrieve the Zerium address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		if !fund {
			if err = sendError(conn, fmt.Errorf("%s left until next allowance", zrmcom.PrettyDuration(timeout.Sub(time.Now())))); err != nil { // nolint: gosimple
...
// the username, avatar URL and Zerium address to fund on success.
func authGitHub(url string) (string, string, zrmcom.Address, error) {
	...
	if gist.Owner.Login == "" {
		return "", "", zrmcom.Address{}, errors.New("Anonymous Gists not allowed")
	}
	// Iterate over all the files and look for Zerium addresses
	var address zrmcom.Address
	for _, file := range gist.Files {
		content := strings.TrimSpace(file.Content)
		if len(content) == 2+zrmcom.AddressLength*2 {
			address = zrmcom.HexToAddress(content)
		}
	}
	if address == (zrmcom.Address{}) {
		return "", "", zrmcom.Address{}, errors.New("No Zerium address found to fund")
	}
	...
	if res.StatusCode != 200 {
		return "", "", zrmcom.Address{}, errors.New("Invalid user... boom!")
	}
...
// the username, avatar URL and Zerium address to fund on success.
func authTwitter(url string) (string, string, zrmcom.Address, error) {
	// Ensure the user specified a meaningful URL, no fancy nonsense
	parts := strings.Split(url, "/")
	if len(parts) < 4 || parts[len(parts)-2] != "status" {
		return "", "", zrmcom.Address{}, errors.New("Invalid Twitter status URL")
	}
	username := parts[len(parts)-3]

	// Twitter's API isn't really friendly with direct links. Still, we don't
	// want to do ask read permissions from users, so just load the public posts and
	// scrape it for the Zerium address and profile URL.
	...
	address := zrmcom.HexToAddress(string(regexp.MustCompile("0x[0-9a-fA-F]{40}").Find(body)))
	if address == (zrmcom.Address{}) {
		return "", "", zrmcom.Address{}, errors.New("No Zerium address found to fund")
	}
	var avatar string
...
// returning the username, avatar URL and Zerium address to fund on success.
func authGooglePlus(url string) (string, string, zrmcom.Address, error) {
	// Ensure the user specified a meaningful URL, no fancy nonsense
	parts := strings.Split(url, "/")
	if len(parts) < 4 || parts[len(parts)-2] != "posts" {
		return "", "", zrmcom.Address{}, errors.New("Invalid Google+ post URL")
	}
	username := parts[len(parts)-3]
...
// returning the username, avatar URL and Zerium address to fund on success.
func authFacebook(url string) (string, string, zrmcom.Address, error) {
	// Ensure the user specified a meaningful URL, no fancy nonsense
	parts := strings.Split(url, "/")
	if len(parts) < 4 || parts[len(parts)-2] != "posts" {
		return "", "", zrmcom.Address{}, errors.New("Invalid Facebook post URL")
	}
	username := parts[len(parts)-3]
...
// authNoAuth tries to interpret a faucet request as a plain Zerium address,
// without actually performing any remote authentication. This mode is prone to
// Byzantine attack, so only ever use for truly private networks.
func authNoAuth(url string) (string, string, zrmcom.Address, error) {
	address := zrmcom.HexToAddress(regexp.MustCompile("0x[0-9a-fA-F]{40}").FindString(url))
	if address == (zrmcom.Address{}) {
		return "", "", zrmcom.Address{}, errors.New("No Zerium address found to fund")
	}
