...
func (w *wizard) deployFaucet() {
	...
	if err != nil {
		infos = &faucetInfos{
			node:    &nodeInfos{portFull: 32310, peersTotal: 25},
			port:    80,
			host:    client.server,
			amount:  1,
			minutes: 1440,
	...
	fmt.Println()
	if infos.node.zrmstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.zrmstats = w.readString() + ":" + w.conf.zrmstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.zrmstats)
		infos.node.zrmstats = w.readDefaultString(infos.node.zrmstats) + ":" + w.conf.zrmstats
	}
