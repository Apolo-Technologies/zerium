...
var (
	...
	SwarmSwapAPIFlag = cli.StringFlag{
		Name:   "swap-api",
		Usage:  "URL of the Zerium API provider to use to settle SWAP payments",
...
// This init function sets defaults so cmd/swarm can run alongside zrmd.
func init() {
...
var app = utils.NewApp(gitCommit, "Zerium Swarm")

// This init function creates the cli.App.
func init() {
	app.Action = bzzd
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2017-2016 The zerium Authors"
...
Export a local chunk database as a tar archive (use - to send to stdout).
    ...
    swarm db export ~/.zerium/swarm/bzz-KEY/chunks chunks.tar

The export may be quite large, consider piping the output through the Unix
pv(1) tool to get a progress bar:

    swarm db export ~/.zerium/swarm/bzz-KEY/chunks - | pv > chunks.tar
`,
				},
				{
					Action:    dbImport,
...
Import chunks from a tar archive into a local chunk database (use - to read from stdin).
    ...
    swarm db import ~/.zerium/swarm/bzz-KEY/chunks chunks.tar

The import may be quite large, consider piping the input through the Unix
pv(1) tool to get a progress bar:

    pv chunks.tar | swarm db import ~/.zerium/swarm/bzz-KEY/chunks -
`,
				},
				{
					Action:    dbClean,
...
func bzzd(ctx *cli.Context) error {
	...
	//zrmd only supports --datadir via command line
	//in order to be consistent within swarm, if we pass --datadir via environment variable
	//or via config file, we get the same directory for zrmd and swarm
	if _, err := os.Stat(bzzconfig.Path); err == nil {
		cfg.DataDir = bzzconfig.Path
	}
	//setup the zerium node
	...
	//register BZZ as node.Service in the zerium node
...
func detectEnsAddr(client *rpc.Client) (zrmcom.Address, error) {
	...
	default:
		return zrmcom.Address{}, fmt.Errorf("unknown version and genesis hash: %s %s", version, block.Hash())
	}
}
...
func registerBzzService(bzzconfig *bzzapi.Config, ctx *cli.Context, stack *node.Node) {
	...
	boot := func(ctx *node.ServiceContext) (node.Service, error) {
		...
		if bzzconfig.EnsApi != "" {
			...
			if bzzconfig.EnsRoot == (zrmcom.Address{}) {
	...
	//register within the zerium node
...
func decryptStoreAccount(ks *keystore.KeyStore, account string, passwords []string) *ecdsa.PrivateKey {
	...
	if zrmcom.IsHexAddress(account) {
		a, err = ks.Find(accounts.Account{Address: zrmcom.HexToAddress(account)})
...
func injectBootnodes(srv *peertopeer.Server, nodes []string) {
