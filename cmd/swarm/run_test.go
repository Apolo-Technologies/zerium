...
// it.
//
// Each node listens on 127.0.0.1 with random ports for both the HTTP and peertopeer
// ports (assigned by first listening on 127.0.0.1:0 and then passing the ports
// as flags).
//
// When starting more than one node, they are connected tozrmder using the
...
func newTestNode(t *testing.T, dir string) *testNode {
	...
	peertopeerPort, err := assignTCPPort()
	if err != nil {
		t.Fatal(err)
	}
	...
	var nodeInfo peertopeer.NodeInfo
	if err := node.Client.Call(&nodeInfo, "admin_nodeInfo"); err != nil {
		t.Fatal(err)
	}
	node.Enode = fmt.Sprintf("enode://%s@127.0.0.1:%s", nodeInfo.ID, peertopeerPort)
