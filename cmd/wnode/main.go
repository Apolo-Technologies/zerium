...
import (
	...
	"github.com/apolo-technologies/zerium/whisper/mailserver"
...
func processArgs() {
	...
	if *asymmetricMode && len(*argPub) > 0 {
		pub = crypto.ToECDSAPub(zrmcom.FromHex(*argPub))
		if !isKeyValid(pub) {
...
func echo() {
	...
	fmt.Printf("mspow = %f \n", *argServerPoW)
	fmt.Printf("ip = %s \n", *argIP)
	fmt.Printf("pub = %s \n", zrmcom.ToHex(crypto.FromECDSAPub(pub)))
	fmt.Printf("idfile = %s \n", *argIDFile)
	fmt.Printf("dbpath = %s \n", *argDBPath)
	fmt.Printf("boot = %s \n", *argEnode)
}
...
func initialize() {
	...
	server = &peertopeer.Server{
		Config: peertopeer.Config{
			PrivateKey:     nodeid,
			MaxPeers:       maxPeers,
			Name:           zrmcom.MakeName("wnode", "5.0"),
...
func startServer() {
	...
	fmt.Printf("my public key: %s \n", zrmcom.ToHex(crypto.FromECDSAPub(&asymKey.PublicKey)))
...
func sendMsg(payload []byte) zrmcom.Hash {
