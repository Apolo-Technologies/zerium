...
func testDAOForkBlockNewChain(t *testing.T, test int, genesis string, expectBlock *big.Int, expectVote bool) {
	...
	// Start a zaed instance with the requested flags set and immediately terminate
	if genesis != "" {
		...
		zaed := runzaed(t, append(args, []string{"--exec", "2+2", "zaeconsole"}...)...)
		zaed.WaitExit()
	}
	// Retrieve the DAO config flag from the database
	path := filepath.Join(datadir, "zaed", "chaindata")
