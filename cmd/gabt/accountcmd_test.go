...
func TestAccountListEmpty(t *testing.T) {
	zaed := runzaed(t, "account", "list")
	zaed.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	zaed := runzaed(t, "account", "list", "--datadir", datadir)
	defer zaed.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	zaed := runzaed(t, "account", "new", "--lightkdf")
	defer zaed.ExpectExit()
	zaed.Expect(`
...
`)
	zaed.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestWalletImport(t *testing.T) {
	zaed := runzaed(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer zaed.ExpectExit()
	zaed.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(zaed.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(zaed.StderrText(), m) {
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	zaed.SetTemplateFunc("keypath", func(file string) string {
