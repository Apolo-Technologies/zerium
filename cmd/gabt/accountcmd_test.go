...
func TestAccountListEmpty(t *testing.T) {
	gabt := runGabt(t, "account", "list")
	gabt.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gabt := runGabt(t, "account", "list", "--datadir", datadir)
	defer gabt.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gabt := runGabt(t, "account", "new", "--lightkdf")
	defer gabt.ExpectExit()
	gabt.Expect(`
...
`)
	gabt.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestWalletImport(t *testing.T) {
	gabt := runGabt(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gabt.ExpectExit()
	gabt.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gabt.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gabt.StderrText(), m) {
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gabt.SetTemplateFunc("keypath", func(file string) string {
