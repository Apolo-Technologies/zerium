...
func (db *Dashboard) Protocols() []peertopeer.Protocol { return nil }

// APIs is a meaningless implementation of node.Service.
func (db *Dashboard) APIs() []rpc.API { return nil }

// Start implements node.Service, starting the data collection thread and the listening server of the dashboard.
func (db *Dashboard) Start(server *peertopeer.Server) error {
	db.wg.Add(2)
	go db.collectData()
...
func (db *Dashboard) collectData() {
	...
	for {
		...
		case <-time.After(db.config.Refresh):
			inboundTraffic := metrics.DefaultRegistry.Get("peertopeer/InboundTraffic").(metrics.Meter).Rate1()
