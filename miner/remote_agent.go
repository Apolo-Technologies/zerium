...
type RemoteAgent struct {
	...
	currentWork *Work
	work        map[zrmcom.Hash]*Work

	hashrateMu sync.RWMutex
	hashrate   map[zrmcom.Hash]hashrate
...
func NewRemoteAgent(chain consensus.ChainReader, engine consensus.Engine) *RemoteAgent {
	return &RemoteAgent{
		chain:    chain,
		engine:   engine,
		work:     make(map[zrmcom.Hash]*Work),
		hashrate: make(map[zrmcom.Hash]hashrate),
	}
}

func (a *RemoteAgent) SubmitHashrate(id zrmcom.Hash, rate uint64) {
	a.hashrateMu.Lock()
...
func (a *RemoteAgent) GetWork() ([3]string, error) {
	...
	if a.currentWork != nil {
		...
		res[1] = zrmcom.BytesToHash(seedHash).Hex()
		// Calculate the "target" to be returned to the external miner
		n := big.NewInt(1)
		n.Lsh(n, 255)
		n.Div(n, block.Difficulty())
		n.Lsh(n, 1)
		res[2] = zrmcom.BytesToHash(n.Bytes()).Hex()
...
func (a *RemoteAgent) SubmitWork(nonce types.BlockNonce, mixDigest, hash zrmcom.Hash) bool {
	a.mu.Lock()
	defer a.mu.Unlock()
