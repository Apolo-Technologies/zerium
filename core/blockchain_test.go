...
func TestFastVsFullChains(t *testing.T) {
	...
	blocks, receipts := GenerateChain(gspec.Config, genesis, gendb, 1024, func(i int, block *BlockGen) {
		block.SetCoinbase(zrmcom.Address{0x00})

		// If the block number is multiple of 3, send a few bonus transactions to the miner
		if i%3 == 2 {
			for j := 0; j < i%4+1; j++ {
				tx, err := types.SignTx(types.NewTransaction(block.TxNonce(address), zrmcom.Address{0x00}, big.NewInt(1000), bigTxGas, nil, nil), signer, key)
				if err != nil {
					panic(err)
				}
				block.AddTx(tx)
	...
	archiveDb, _ := zrmdb.NewMemDatabase()
...
func TestLogReorgs(t *testing.T) {
	...
	var (
		...
		code    = zrmcom.Hex2Bytes("60606040525b7f24ec1d3ff24c2f6ff210738839dbc339cd45a5294d85c79361016243157aae7b60405180905060405180910390a15b600a8060416000396000f360606040526008565b00")
...
func TestReorgSideEvent(t *testing.T) {
	...
	// heavier chain.
	expectedSideHashes := map[zrmcom.Hash]bool{
	...
	blocks, _ := GenerateChain(gspec.Config, genesis, db, 4, func(i int, block *BlockGen) {
		var (
			...
			basicTx = func(signer types.Signer) (*types.Transaction, error) {
				return types.SignTx(types.NewTransaction(block.TxNonce(address), zrmcom.Address{}, new(big.Int), big.NewInt(21000), new(big.Int), nil), signer, key)
			}
		)
		switch i {
		case 0:
