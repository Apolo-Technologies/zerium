...
func testChainIndexer(t *testing.T, count int) {
	...
	for i := 0; i < count; i++ {
		...
		backends[i].indexer = NewChainIndexer(db, zrmdb.NewTable(db, string([]byte{byte(i)})), backends[i], sectionSize, confirmsReq, 0, fmt.Sprintf("indexer-%d", i))
...
func (b *testChainIndexBackend) Reset(section uint64, prevHead zrmcom.Hash) error {
	b.section = section
	b.headerCnt = 0
	return nil
}
