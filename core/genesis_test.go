...
func TestSetupGenesis(t *testing.T) {
	var (
		customghash = zrmcom.HexToHash("0x89c99d90b79719238d2645c7642f2c9295246e80775b38cfd162b696817fbd50")
		customg     = Genesis{
			Config: &params.ChainConfig{HomesteadBlock: big.NewInt(3)},
			Alloc: GenesisAlloc{
				{1}: {Balance: big.NewInt(1), Storage: map[zrmcom.Hash]zrmcom.Hash{{1}: {1}}},
			},
		}
		oldcustomg = customg
	)
	...
	tests := []struct {
		name       string
		fn         func(zrmdb.Database) (*params.ChainConfig, zrmcom.Hash, error)
		wantConfig *params.ChainConfig
		wantHash   zrmcom.Hash
		wantErr    error
	}{
		{
			name: "genesis without ChainConfig",
			fn: func(db zrmdb.Database) (*params.ChainConfig, zrmcom.Hash, error) {
