...
func (dummyContractRef) Address() zrmcom.Address     { return zrmcom.Address{} }
func (dummyContractRef) Value() *big.Int             { return new(big.Int) }
func (dummyContractRef) SetCode(zrmcom.Hash, []byte) {}
func (d *dummyContractRef) ForEachStorage(callback func(key, value zrmcom.Hash) bool) {
...
func TestStoreCapture(t *testing.T) {
	...
	var index zrmcom.Hash

	logger.CaptureState(env, 0, SSTORE, 0, 0, mem, stack, contract, 0, nil)
	if len(logger.changedValues[contract.Address()]) == 0 {
		t.Fatalf("expected exactly 1 changed value on address %x, got %d", contract.Address(), len(logger.changedValues[contract.Address()]))
	}
	exp := zrmcom.BigToHash(big.NewInt(1))
