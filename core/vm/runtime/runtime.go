...
type Config struct {
	...
	State     *state.StateDB
	GetHashFn func(n uint64) zrmcom.Hash
}
...
func setDefaults(cfg *Config) {
	...
	if cfg.GetHashFn == nil {
		cfg.GetHashFn = func(n uint64) zrmcom.Hash {
			return zrmcom.BytesToHash(crypto.Keccak256([]byte(new(big.Int).SetUint64(n).String())))
		}
	}
}
...
func Execute(code, input []byte, cfg *Config) ([]byte, *state.StateDB, error) {
	...
	if cfg.State == nil {
		...
		cfg.State, _ = state.New(zrmcom.Hash{}, state.NewDatabase(db))
	}
	var (
		address = zrmcom.StringToAddress("contract")
		vmenv   = NewEnv(cfg)
	...
	ret, _, err := vmenv.Call(
		sender,
		zrmcom.StringToAddress("contract"),
		input,
		cfg.GasLimit,
		cfg.Value,
	)
...
func Create(input []byte, cfg *Config) ([]byte, zrmcom.Address, uint64, error) {
	if cfg == nil {
		cfg = new(Config)
	}
	setDefaults(cfg)
...
// be set.
func Call(address zrmcom.Address, input []byte, cfg *Config) ([]byte, uint64, error) {
	setDefaults(cfg)
