...
// PrecompiledContractsHomestead contains the default set of pre-compiled Zerium
// contracts used in the Frontier and Homestead releases.
var PrecompiledContractsHomestead = map[zrmcom.Address]PrecompiledContract{
	zrmcom.BytesToAddress([]byte{1}): &ecrecover{},
	zrmcom.BytesToAddress([]byte{2}): &sha256hash{},
	zrmcom.BytesToAddress([]byte{3}): &ripemd160hash{},
	zrmcom.BytesToAddress([]byte{4}): &dataCopy{},
}

// PrecompiledContractsByzantium contains the default set of pre-compiled Zerium
// contracts used in the Byzantium release.
var PrecompiledContractsByzantium = map[zrmcom.Address]PrecompiledContract{
	zrmcom.BytesToAddress([]byte{1}): &ecrecover{},
	zrmcom.BytesToAddress([]byte{2}): &sha256hash{},
	zrmcom.BytesToAddress([]byte{3}): &ripemd160hash{},
	zrmcom.BytesToAddress([]byte{4}): &dataCopy{},
	zrmcom.BytesToAddress([]byte{5}): &bigModExp{},
	zrmcom.BytesToAddress([]byte{6}): &bn256Add{},
	zrmcom.BytesToAddress([]byte{7}): &bn256ScalarMul{},
	zrmcom.BytesToAddress([]byte{8}): &bn256Pairing{},
}
...
func (c *ecrecover) Run(input []byte) ([]byte, error) {
	...
	input = zrmcom.RightPadBytes(input, ecRecoverInputLength)
	...
	return zrmcom.LeftPadBytes(crypto.Keccak256(pubKey[1:])[12:], 32), nil
}
...
func (c *ripemd160hash) Run(input []byte) ([]byte, error) {
	...
	ripemd.Write(input)
	return zrmcom.LeftPadBytes(ripemd.Sum(nil), 32), nil
}
...
func (c *bigModExp) Run(input []byte) ([]byte, error) {
	...
	if mod.BitLen() == 0 {
		...
		return zrmcom.LeftPadBytes([]byte{}, int(modLen)), nil
	}
	return zrmcom.LeftPadBytes(base.Exp(base, exp, mod).Bytes(), int(modLen)), nil
}
