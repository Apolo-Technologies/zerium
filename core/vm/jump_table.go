...
type (
	executionFunc       func(pc *uint64, env *ZVM, contract *Contract, memory *Memory, stack *Stack) ([]byte, error)
	gasFunc             func(params.GasTable, *ZVM, *Contract, *Stack, *Memory, uint64) (uint64, error) // last parameter is the requested memory size as a uint64
...
type operation struct {
	...
	halts   bool // indicates whzerium the operation should halt further execution
	jumps   bool // indicates whzerium the program counter should not increment
	writes  bool // determines whzerium this a state modifying operation
	valid   bool // indication whzerium the retrieved operation is valid and known
	reverts bool // determines whzerium the operation reverts state (implicitly halts)
	returns bool // determines whzerium the operations sets the return data content
}
