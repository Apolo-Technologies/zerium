...
func makeTestState() (Database, *zrmdb.MemDatabase, zrmcom.Hash, []*testAccount) {
	// Create an empty state
	mem, _ := zrmdb.NewMemDatabase()
	db := NewDatabase(mem)
	state, _ := New(zrmcom.Hash{}, db)

	// Fill it with some arbitrary data
	accounts := []*testAccount{}
	for i := byte(0); i < 96; i++ {
		obj := state.GetOrNewStateObject(zrmcom.BytesToAddress([]byte{i}))
		acc := &testAccount{address: zrmcom.BytesToAddress([]byte{i})}
...
// account array.
func checkStateAccounts(t *testing.T, db zrmdb.Database, root zrmcom.Hash, accounts []*testAccount) {
...
func checkTrieConsistency(db zrmdb.Database, root zrmcom.Hash) error {
...
func checkStateConsistency(db zrmdb.Database, root zrmcom.Hash) error {
...
func TestEmptyStateSync(t *testing.T) {
	empty := zrmcom.HexToHash("56e81f171bcc55a6ff8345e692c0f86e5b48e01b996cadc001622fb5e363b421")
