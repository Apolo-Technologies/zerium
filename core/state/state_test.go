...
type StateSuite struct {
	db    *zrmdb.MemDatabase
	state *StateDB
}

var _ = checker.Suite(&StateSuite{})

var toAddr = zrmcom.BytesToAddress
...
func (s *StateSuite) SetUpTest(c *checker.C) {
	s.db, _ = zrmdb.NewMemDatabase()
	s.state, _ = New(zrmcom.Hash{}, NewDatabase(s.db))
}

func (s *StateSuite) TestNull(c *checker.C) {
	address := zrmcom.HexToAddress("0x823140710bf13990e4500136726d8b55")
	s.state.CreateAccount(address)
	//value := zrmcom.FromHex("0x823140710bf13990e4500136726d8b55")
	var value zrmcom.Hash
	s.state.SetState(address, zrmcom.Hash{}, value)
	s.state.CommitTo(s.db, false)
	value = s.state.GetState(address, zrmcom.Hash{})
	if !zrmcom.EmptyHash(value) {
...
func (s *StateSuite) TestSnapshot(c *checker.C) {
	...
	var storageaddr zrmcom.Hash
	data1 := zrmcom.BytesToHash([]byte{42})
	data2 := zrmcom.BytesToHash([]byte{43})
...
func TestSnapshot2(t *testing.T) {
	...
	data0 := zrmcom.BytesToHash([]byte{17})
	data1 := zrmcom.BytesToHash([]byte{18})
