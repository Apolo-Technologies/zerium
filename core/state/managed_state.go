...
type ManagedState struct {
	*StateDB
	...
	accounts map[zrmcom.Address]*account
}
...
func ManageState(statedb *StateDB) *ManagedState {
	return &ManagedState{
		...
		accounts: make(map[zrmcom.Address]*account),
	}
}
...
func (ms *ManagedState) RemoveNonce(addr zrmcom.Address, n uint64) {
...
func (ms *ManagedState) NewNonce(addr zrmcom.Address) uint64 {
	ms.mu.Lock()
	defer ms.mu.Unlock()
...
func (ms *ManagedState) GetNonce(addr zrmcom.Address) uint64 {
	ms.mu.Lock()
	defer ms.mu.Unlock()
...
func (ms *ManagedState) SetNonce(addr zrmcom.Address, nonce uint64) {
	ms.mu.Lock()
	defer ms.mu.Unlock()
...
func (ms *ManagedState) HasAccount(addr zrmcom.Address) bool {
	ms.mu.RLock()
	defer ms.mu.RUnlock()
...
func (ms *ManagedState) getAccount(addr zrmcom.Address) *account {
