...
// Package core implements the Zerium consensus protocol.
package core
...
import (
	...
	"github.com/apolo-technologies/zerium/zrmcom/mclock"
...
// available in the database. It initialises the default Zerium Validator and
// Processor.
func NewBlockChain(chainDb zrmdb.Database, config *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config) (*BlockChain, error) {
...
func (bc *BlockChain) loadLastState() error {
	...
	if head := GetHeadHeaderHash(bc.chainDb); head != (zrmcom.Hash{}) {
	...
	if head := GetHeadFastBlockHash(bc.chainDb); head != (zrmcom.Hash{}) {
...
func (bc *BlockChain) SetHead(head uint64) error {
	...
	delFn := func(hash zrmcom.Hash, num uint64) {
...
func (bc *BlockChain) FastSyncCommitHead(hash zrmcom.Hash) error {
...
func (bc *BlockChain) LastBlockHash() zrmcom.Hash {
	bc.mu.RLock()
	defer bc.mu.RUnlock()
...
func (bc *BlockChain) Status() (td *big.Int, currentBlock zrmcom.Hash, genesisBlock zrmcom.Hash) {
	bc.mu.RLock()
	defer bc.mu.RUnlock()
...
func (bc *BlockChain) StateAt(root zrmcom.Hash) (*state.StateDB, error) {
...
func (bc *BlockChain) GetBody(hash zrmcom.Hash) *types.Body {
...
// caching it if found.
func (bc *BlockChain) GetBodyRLP(hash zrmcom.Hash) rlp.RawValue {
	// Short circuit if the body's already in the cache, retrieve otherwise
...
func (bc *BlockChain) HasBlock(hash zrmcom.Hash, number uint64) bool {
...
func (bc *BlockChain) HasBlockAndState(hash zrmcom.Hash) bool {
...
// caching it if found.
func (bc *BlockChain) GetBlock(hash zrmcom.Hash, number uint64) *types.Block {
...
func (bc *BlockChain) GetBlockByHash(hash zrmcom.Hash) *types.Block {
...
func (bc *BlockChain) GetBlocksFromHash(hash zrmcom.Hash, n int) (blocks []*types.Block) {
...
func (bc *BlockChain) Rollback(chain []zrmcom.Hash) {
	bc.mu.Lock()
	defer bc.mu.Unlock()
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
	...
	for i, block := range blockChain {
		...
		if batch.ValueSize() >= zrmdb.IdealBatchSize {
	...
	log.Info("Imported new block receipts",
		...
		"elapsed", zrmcom.PrettyDuration(time.Since(start)),
		"bytes", bytes,
		"number", head.Number(),
		"hash", head.Hash(),
	...
	for i, block := range chain {
		...
		case CanonStatTy:
			log.Debug("Inserted new block", "number", block.Number(), "hash", block.Hash(), "uncles", len(block.Uncles()),
				"txs", len(block.Transactions()), "gas", block.GasUsed(), "elapsed", zrmcom.PrettyDuration(time.Since(bstart)))
		...
		case SideStatTy:
			log.Debug("Inserted forked block", "number", block.Number(), "hash", block.Hash(), "diff", block.Difficulty(), "elapsed",
				zrmcom.PrettyDuration(time.Since(bstart)), "txs", len(block.Transactions()), "gas", block.GasUsed(), "uncles", len(block.Uncles()))
...
func (st *insertStats) report(chain []*types.Block, index int) {
	...
	if index == len(chain)-1 || elapsed >= statsReportLimit {
		...
		context := []interface{}{
			...
			"elapsed", zrmcom.PrettyDuration(elapsed), "mgasps", float64(st.usedGas) * 1000 / float64(elapsed),
...
func (bc *BlockChain) reorg(oldBlock, newBlock *types.Block) error {
	var (
		...
		zrmcomBlock *types.Block
		deletedTxs  types.Transactions
		deletedLogs []*types.Log
		// collectLogs collects the logs that were generated during the
		// processing of the block that corresponds with the given hash.
		// These logs are later announced as deleted.
		collectLogs = func(h zrmcom.Hash) {
	...
	if len(oldChain) > 0 && len(newChain) > 0 {
		...
		logFn("Chain split detected", "number", zrmcomBlock.Number(), "hash", zrmcomBlock.Hash(),
...
func (bc *BlockChain) GetTd(hash zrmcom.Hash, number uint64) *big.Int {
	return bc.hc.GetTd(hash, number)
}

// GetTdByHash retrieves a block's total difficulty in the canonical chain from the
// database by hash, caching it if found.
func (bc *BlockChain) GetTdByHash(hash zrmcom.Hash) *big.Int {
	return bc.hc.GetTdByHash(hash)
}

// GetHeader retrieves a block header from the database by hash and number,
// caching it if found.
func (bc *BlockChain) GetHeader(hash zrmcom.Hash, number uint64) *types.Header {
	return bc.hc.GetHeader(hash, number)
}

// GetHeaderByHash retrieves a block header from the database by hash, caching it if
// found.
func (bc *BlockChain) GetHeaderByHash(hash zrmcom.Hash) *types.Header {
	return bc.hc.GetHeaderByHash(hash)
}

// HasHeader checks if a block header is present in the database or not, caching
// it if present.
func (bc *BlockChain) HasHeader(hash zrmcom.Hash, number uint64) bool {
	return bc.hc.HasHeader(hash, number)
}

// GetBlockHashesFromHash retrieves a number of block hashes starting at a given
// hash, fetching towards the genesis block.
func (bc *BlockChain) GetBlockHashesFromHash(hash zrmcom.Hash, max uint64) []zrmcom.Hash {
