...
type Receipt struct {
	...
	TxHash          zrmcom.Hash    `json:"transactionHash" gencodec:"required"`
	ContractAddress zrmcom.Address `json:"contractAddress"`
...
type receiptStorageRLP struct {
	...
	Bloom             Bloom
	TxHash            zrmcom.Hash
	ContractAddress   zrmcom.Address
	Logs              []*LogForStorage
	GasUsed           *big.Int
}
...
func NewReceipt(root []byte, failed bool, cumulativeGasUsed *big.Int) *Receipt {
	r := &Receipt{PostState: zrmcom.CopyBytes(root), CumulativeGasUsed: new(big.Int).Set(cumulativeGasUsed)}
	if failed {
...
func (r *Receipt) setStatus(postStateOrStatus []byte) error {
	...
	case bytes.Equal(postStateOrStatus, receiptStatusFailedRLP):
		r.Status = ReceiptStatusFailed
	case len(postStateOrStatus) == len(zrmcom.Hash{}):
