...
func TestEIP155SigningVitalik(t *testing.T) {
	...
	for i, test := range []struct {
		...
		var tx *Transaction
		err := rlp.DecodeBytes(zrmcom.Hex2Bytes(test.txRlp), &tx)
		if err != nil {
			t.Errorf("%d: %v", i, err)
			continue
		}
		...
		addr := zrmcom.HexToAddress(test.addr)
		if from != addr {
...
func TestChainId(t *testing.T) {
	...
	tx := NewTransaction(0, zrmcom.Address{}, new(big.Int), new(big.Int), new(big.Int), nil)
