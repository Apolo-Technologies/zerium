...
func TestHeaderStorage(t *testing.T) {
	...
	if entry := GetHeaderRLP(db, header.Hash(), header.Number.Uint64()); entry == nil {
		...
		if hash := zrmcom.BytesToHash(hasher.Sum(nil)); hash != header.Hash() {
...
func TestBodyStorage(t *testing.T) {
	...
	hash := zrmcom.BytesToHash(hasher.Sum(nil))
	...
	if entry := GetBodyRLP(db, hash, 0); entry == nil {
		...
		if calc := zrmcom.BytesToHash(hasher.Sum(nil)); calc != hash {
...
func TestTdStorage(t *testing.T) {
	...
	hash, td := zrmcom.Hash{}, big.NewInt(314)
...
func TestCanonicalMappingStorage(t *testing.T) {
	...
	hash, number := zrmcom.Hash{0: 0xff}, uint64(314)
	if entry := GetCanonicalHash(db, number); entry != (zrmcom.Hash{}) {
...
func TestHeadStorage(t *testing.T) {
	...
	if entry := GetHeadHeaderHash(db); entry != (zrmcom.Hash{}) {
		t.Fatalf("Non head header entry returned: %v", entry)
	}
	if entry := GetHeadBlockHash(db); entry != (zrmcom.Hash{}) {
		t.Fatalf("Non head block entry returned: %v", entry)
	}
	if entry := GetHeadFastBlockHash(db); entry != (zrmcom.Hash{}) {
...
func TestLookupStorage(t *testing.T) {
	...
	tx1 := types.NewTransaction(1, zrmcom.BytesToAddress([]byte{0x11}), big.NewInt(111), big.NewInt(1111), big.NewInt(11111), []byte{0x11, 0x11, 0x11})
	tx2 := types.NewTransaction(2, zrmcom.BytesToAddress([]byte{0x22}), big.NewInt(222), big.NewInt(2222), big.NewInt(22222), []byte{0x22, 0x22, 0x22})
	tx3 := types.NewTransaction(3, zrmcom.BytesToAddress([]byte{0x33}), big.NewInt(333), big.NewInt(3333), big.NewInt(33333), []byte{0x33, 0x33, 0x33})
...
func TestBlockReceiptStorage(t *testing.T) {
	...
	receipt1 := &types.Receipt{
		...
		Logs: []*types.Log{
			{Address: zrmcom.BytesToAddress([]byte{0x11})},
			{Address: zrmcom.BytesToAddress([]byte{0x01, 0x11})},
		},
		TxHash:          zrmcom.BytesToHash([]byte{0x11, 0x11}),
		ContractAddress: zrmcom.BytesToAddress([]byte{0x01, 0x11, 0x11}),
		GasUsed:         big.NewInt(111111),
	}
	receipt2 := &types.Receipt{
		PostState:         zrmcom.Hash{2}.Bytes(),
		CumulativeGasUsed: big.NewInt(2),
		Logs: []*types.Log{
			{Address: zrmcom.BytesToAddress([]byte{0x22})},
			{Address: zrmcom.BytesToAddress([]byte{0x02, 0x22})},
		},
		TxHash:          zrmcom.BytesToHash([]byte{0x22, 0x22}),
		ContractAddress: zrmcom.BytesToAddress([]byte{0x02, 0x22, 0x22}),
		GasUsed:         big.NewInt(222222),
	}
	receipts := []*types.Receipt{receipt1, receipt2}

	// Check that no receipt entries are in a pristine database
	hash := zrmcom.BytesToHash([]byte{0x03, 0x14})
