...
func (bc *testBlockChain) GetBlock(hash zrmcom.Hash, number uint64) *types.Block {
	return bc.CurrentBlock()
}

func (bc *testBlockChain) StateAt(zrmcom.Hash) (*state.StateDB, error) {
...
func pricedTransaction(nonce uint64, gaslimit, gasprice *big.Int, key *ecdsa.PrivateKey) *types.Transaction {
	tx, _ := types.SignTx(types.NewTransaction(nonce, zrmcom.Address{}, big.NewInt(100), gaslimit, gasprice, nil), types.HomesteadSigner{}, key)
	return tx
}
...
func deriveSender(tx *types.Transaction) (zrmcom.Address, error) {
...
func (c *testChain) State() (*state.StateDB, error) {
	...
	if *c.trigger {
		db, _ := zrmdb.NewMemDatabase()
		c.statedb, _ = state.New(zrmcom.Hash{}, state.NewDatabase(db))
...
func TestTransactionNegativeValue(t *testing.T) {
	...
	tx, _ := types.SignTx(types.NewTransaction(0, zrmcom.Address{}, big.NewInt(-1), big.NewInt(100), big.NewInt(1), nil), types.HomesteadSigner{}, key)
	from, _ := deriveSender(tx)
	pool.currentState.AddBalance(from, big.NewInt(1))
...
func TestTransactionDoubleNonce(t *testing.T) {
	...
	tx1, _ := types.SignTx(types.NewTransaction(0, zrmcom.Address{}, big.NewInt(100), big.NewInt(100000), big.NewInt(1), nil), signer, key)
	tx2, _ := types.SignTx(types.NewTransaction(0, zrmcom.Address{}, big.NewInt(100), big.NewInt(1000000), big.NewInt(2), nil), signer, key)
	tx3, _ := types.SignTx(types.NewTransaction(0, zrmcom.Address{}, big.NewInt(100), big.NewInt(1000000), big.NewInt(1), nil), signer, key)
	...
	pool.promoteExecutables([]zrmcom.Address{addr})
...
func testTransactionQueueGlobalLimiting(t *testing.T, nolocals bool) {
	...
	nonces := make(map[zrmcom.Address]uint64)
