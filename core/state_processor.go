...
// Process processes the state changes according to the Zerium rules by running
...
func ApplyTransaction(config *params.ChainConfig, bc *BlockChain, author *zrmcom.Address, gp *GasPool, statedb *state.StateDB, header *types.Header, tx *types.Transaction, usedGas *big.Int, cfg vm.Config) (*types.Receipt, *big.Int, error) {
	msg, err := tx.AsMessage(types.MakeSigner(config, header.Number))
	if err != nil {
		return nil, nil, err
	}
	// Create a new context to be used in the Zvm environment
	context := NewZvmContext(msg, header, bc, author)
	// Create a new environment which holds all relevant information
	// about the transaction and calling mechanisms.
	vmenv := vm.NewZvm(context, statedb, config, cfg)
