...
func (b *BlockGen) OffsetTime(seconds int64) {
	...
	b.header.Difficulty = abthash.CalcDifficulty(b.config, b.header.Time.Uint64(), b.parent.Header())
}

// GenerateChain creates a chain of n blocks. The first block's
// parent will be the provided parent. db is used to store
// intermediate states and should contain the parent's state pkg2310.
//
...
func GenerateChain(config *params.ChainConfig, parent *types.Block, db zrmdb.Database, n int, gen func(int, *BlockGen)) ([]*types.Block, []types.Receipts) {
	if config == nil {
	...
	genblock := func(i int, h *types.Header, statedb *state.StateDB) (*types.Block, types.Receipts) {
		...
		abthash.AccumulateRewards(config, statedb, h, b.uncles)
...
func makeHeader(config *params.ChainConfig, parent *types.Block, state *state.StateDB) *types.Header {
	...
	return &types.Header{
		...
		Difficulty: abthash.CalcDifficulty(config, time.Uint64(), &types.Header{
...
// header only chain.
func newCanonical(n int, full bool) (zrmdb.Database, *BlockChain, error) {
	// Initialize a fresh chain with only a genesis block
	gspec := new(Genesis)
	db, _ := zrmdb.NewMemDatabase()
	genesis := gspec.MustCommit(db)

	blockchain, _ := NewBlockChain(db, params.AllAbthashProtocolChanges, abthash.NewFaker(), vm.Config{})
...
func makeHeaderChain(parent *types.Header, n int, db zrmdb.Database, seed int) []*types.Header {
...
func makeBlockChain(parent *types.Block, n int, db zrmdb.Database, seed int) []*types.Block {
