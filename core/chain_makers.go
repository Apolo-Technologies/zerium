...
func (b *BlockGen) SetCoinbase(addr zrmcom.Address) {
	if b.gasPool != nil {
		if len(b.txs) > 0 {
...
func (b *BlockGen) AddTx(tx *types.Transaction) {
	if b.gasPool == nil {
		b.SetCoinbase(zrmcom.Address{})
	}
	b.statedb.Prepare(tx.Hash(), zrmcom.Hash{}, len(b.txs))
...
func (b *BlockGen) TxNonce(addr zrmcom.Address) uint64 {
		...
		if daoBlock := config.DAOForkBlock; daoBlock != nil {
			...
			if h.Number.Cmp(daoBlock) >= 0 && h.Number.Cmp(limit) < 0 {
				if config.DAOForkSupport {
					h.Extra = zrmcom.CopyBytes(params.DAOForkBlockExtra)
				}
			}
		}
...
func makeHeader(config *params.ChainConfig, parent *types.Block, state *state.StateDB) *types.Header {
	...
	return &types.Header{
		...
		GasUsed:  new(big.Int),
		Number:   new(big.Int).Add(parent.Number(), zrmcom.Big1),
		Time:     time,
	}
}
...
func makeBlockChain(parent *types.Block, n int, db zrmdb.Database, seed int) []*types.Block {
	blocks, _ := GenerateChain(params.TestChainConfig, parent, db, n, func(i int, b *BlockGen) {
		b.SetCoinbase(zrmcom.Address{0: byte(seed), 19: byte(i)})
	})
	return blocks
}
