...
type HeaderChain struct {
	...
	currentHeaderHash zrmcom.Hash   // Hash of the current head of the header chain (prevent recomputing all the time)
...
func NewHeaderChain(chainDb zrmdb.Database, config *params.ChainConfig, engine consensus.Engine, procInterrupt func() bool) (*HeaderChain, error) {
	...
	if head := GetHeadBlockHash(chainDb); head != (zrmcom.Hash{}) {
...
func (hc *HeaderChain) GetBlockNumber(hash zrmcom.Hash) uint64 {
...
func (hc *HeaderChain) InsertHeaderChain(chain []*types.Header, writeHeader WhCallback, start time.Time) (int, error) {
	...
	log.Info("Imported new block headers", "count", stats.processed, "elapsed", zrmcom.PrettyDuration(time.Since(start)),
...
func (hc *HeaderChain) GetBlockHashesFromHash(hash zrmcom.Hash, max uint64) []zrmcom.Hash {
...
func (hc *HeaderChain) GetTd(hash zrmcom.Hash, number uint64) *big.Int {
...
func (hc *HeaderChain) GetTdByHash(hash zrmcom.Hash) *big.Int {
	return hc.GetTd(hash, hc.GetBlockNumber(hash))
}

// WriteTd stores a block's total difficulty into the database, also caching it
// along the way.
func (hc *HeaderChain) WriteTd(hash zrmcom.Hash, number uint64, td *big.Int) error {
...
// caching it if found.
func (hc *HeaderChain) GetHeader(hash zrmcom.Hash, number uint64) *types.Header {
...
// found.
func (hc *HeaderChain) GetHeaderByHash(hash zrmcom.Hash) *types.Header {
	return hc.GetHeader(hash, hc.GetBlockNumber(hash))
}

// HasHeader checks if a block header is present in the database or not.
func (hc *HeaderChain) HasHeader(hash zrmcom.Hash, number uint64) bool {
...
type DeleteCallback func(zrmcom.Hash, uint64)
...
func (hc *HeaderChain) GetBlock(hash zrmcom.Hash, number uint64) *types.Block {
	return nil
}
