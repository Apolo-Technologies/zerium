...
type Matcher struct {
	...
	running uint32 // Atomic flag whzerium a session is live or not
}
...
// that address/topic, and binary AND-ing those vectors togzerium.
	...
	go func() {
		...
		for {
			...
			case subres, ok := <-process:
				...
				// Gather all the sub-results and merge them togzerium
				var orVector []byte
...
// the reuested retrieval queue to be serviced togzerium with other sessions.
//
