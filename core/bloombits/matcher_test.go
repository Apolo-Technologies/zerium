...
func TestMatcherWildcards(t *testing.T) {
	matcher := NewMatcher(testSectionSize, [][][]byte{
		{zrmcom.Address{}.Bytes(), zrmcom.Address{0x01}.Bytes()}, // Default address is not a wildcard
		{zrmcom.Hash{}.Bytes(), zrmcom.Hash{0x01}.Bytes()},       // Default hash is not a wildcard
		{zrmcom.Hash{0x01}.Bytes()},                              // Plain rule, sanity check
		{zrmcom.Hash{0x01}.Bytes(), nil},                         // Wildcard suffix, drop rule
		{nil, zrmcom.Hash{0x01}.Bytes()},                         // Wildcard prefix, drop rule
...
// bitset matching https://github.com/apolo-technologies/zerium/issues/15309.
