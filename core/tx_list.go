...
// Remove deletes a transaction from the maintained map, returning whzerium the
...
type txList struct {
	strict bool         // Whzerium nonces are strictly continuous or not
...
// Overlaps returns whzerium the transaction specified has the same nonce as one
// already contained within the list.
func (l *txList) Overlaps(tx *types.Transaction) bool {
	return l.txs.Get(tx.Nonce()) != nil
}

// Add tries to insert a new transaction into the list, returning whzerium the
...
// Remove deletes a transaction from the maintained list, returning whzerium the
...
// Empty returns whzerium the list of transactions is empty or not.
...
// Underpriced checks whzerium a transaction is cheaper than (or as cheap as) the
