...
/*
Package node sets up multi-protocol Zerium nodes.

In the model exposed by this package, a node is a collection of services which use shared
resources to provide RPC APIs. Services can also offer devpeertopeer protocols, which are wired
up to the devpeertopeer network when the node instance is started.
...
To access to the devpeertopeer network, Node configures and starts peertopeer.Server. Each host on the
devpeertopeer network has a unique identifier, the node key. The Node instance persists this key
...
endpoint to a subset of RPC modules. Node itself offers the "debug", "admin" and "abt17"
modules.
...
Node also creates the shared store of encrypted Zerium account keys. Services can access
...
devpeertopeer-related resources (node key, static/trusted node lists, known hosts database) are
...
"db" and "db-2". The following files will be created in the data directory:
   ...
   data-directory/
        A/
            nodekey            -- devpeertopeer node key of instance A
            nodes/             -- devpeertopeer discovery knowledge database of instance A
            db/                -- LevelDB content for "db"
        A.ipc                  -- JSON-RPC UNIX domain socket endpoint of instance A
        B/
            nodekey            -- devpeertopeer node key of node B
            nodes/             -- devpeertopeer discovery knowledge database of instance B
            static-nodes.json  -- devpeertopeer static node list of instance B
            db/                -- LevelDB content for "db"
