...
type Node struct {
	...
	serverConfig peertopeer.Config
	server       *peertopeer.Server // Currently running P2P networking layer
...
func (n *Node) Start() error {
	...
	// Initialize the peertopeer server. This creates the node key and
	// discovery databases.
	...
	running := &peertopeer.Server{Config: n.serverConfig}
...
func (n *Node) Stop() error {
	...
	// Terminate the API, services and the peertopeer server.
	n.stopWS()
	n.stopHTTP()
	n.stopIPC()
	n.rpcAPIs = nil
...
func (n *Node) Server() *peertopeer.Server {
	n.lock.RLock()
	defer n.lock.RUnlock()
...
func (n *Node) OpenDatabase(name string, cache, handles int) (zrmdb.Database, error) {
	if n.config.DataDir == "" {
		return zrmdb.NewMemDatabase()
	}
	return zrmdb.NewLDBDatabase(n.config.resolvePath(name), cache, handles)
}
