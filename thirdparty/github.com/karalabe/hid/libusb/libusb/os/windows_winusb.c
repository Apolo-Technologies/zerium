...
static int zrmcom_configure_endpoints(int sub_api, struct libusb_device_handle *dev_handle, int iface)
{
	return LIBUSB_SUCCESS;
}
...
static int _hid_get_report_descriptor(struct hid_device_priv *dev, void *data, size_t *size)
{
	...
	/* usage page (0xFFA0 == thirdparty defined) */
	d[i++] = 0x06; d[i++] = 0xA0; d[i++] = 0xFF;
	/* usage (thirdparty defined) */
	d[i++] = 0x09; d[i++] = 0x01;
