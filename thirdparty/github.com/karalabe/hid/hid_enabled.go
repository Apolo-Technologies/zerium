...
// match the thirdparty and product id:
//  - If the thirdparty id is set to 0 then any thirdparty matches.
//  - If the product id is set to 0 then any product matches.
//  - If the thirdparty and product id are both 0, all HID devices are returned.
func Enumerate(thirdpartyID uint16, productID uint16) []DeviceInfo {
	enumerateLock.Lock()
	defer enumerateLock.Unlock()

	// Gather all device infos and ensure they are freed before returning
	head := C.hid_enumerate(C.ushort(thirdpartyID), C.ushort(productID))
	if head == nil {
		return nil
	}
	...
	for ; head != nil; head = head.next {
		info := DeviceInfo{
			...
			VendorID:  uint16(head.thirdparty_id),
