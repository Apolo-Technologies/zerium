...
static unsigned short get_thirdparty_id(IOHIDDeviceRef device)
{
...
struct hid_device_info  HID_API_EXPORT *hid_enumerate(unsigned short thirdparty_id, unsigned short product_id)
{
	...
	for (i = 0; i < num_devices; i++) {
		...
		dev_vid = get_thirdparty_id(dev);
		dev_pid = get_product_id(dev);

		/* Check the VID/PID against the arguments */
		if ((thirdparty_id == 0x0 || thirdparty_id == dev_vid) &&
...
hid_device * HID_API_EXPORT hid_open(unsigned short thirdparty_id, unsigned short product_id, const wchar_t *serial_number)
{
	/* This function is identical to the Linux version. Platform independent. */
	...
	devs = hid_enumerate(thirdparty_id, product_id);
	cur_dev = devs;
	while (cur_dev) {
		if (cur_dev->thirdparty_id == thirdparty_id &&
...
int main(void)
{
	...
	for (i = 0; i < num_devices; i++) {
		IOHIDDeviceRef dev = device_array[i];
		printf("Device: %p\n", dev);
		printf("  %04hx %04hx\n", get_thirdparty_id(dev), get_product_id(dev));
