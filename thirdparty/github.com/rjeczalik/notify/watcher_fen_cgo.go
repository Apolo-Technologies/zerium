...
type cfen struct {
	peertopeere map[string]*C.port_event_t
	p2fo map[string]*C.struct_file_obj
}

func newCfen() cfen {
	return cfen{
		peertopeere: make(map[string]*C.port_event_t),
...
func (c *cfen) free() {
	...
	for i := range c.peertopeere {
		C.free(unsafe.Pointer(c.peertopeere[i]))
	}
	c.p2fo = make(map[string]*C.struct_file_obj)
	c.peertopeere = make(map[string]*C.port_event_t)
}
