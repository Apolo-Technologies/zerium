...
type pkg struct {
	...
	importPath      string // full pkg import path ("net/http", "foo/bar/thirdparty/a/b")
	importPathShort string // thirdpartyless import path ("net/http", "a/b")
}
			...
			if typ.IsRegular() {
				...
				if _, dup := dirScan[dir]; !dup {
					...
					dirScan[dir] = &pkg{
						...
						importPathShort: thirdpartylessImportPath(importpath),
						dir:             dir,
					}
				}
				dirScanMu.Unlock()
...
// thirdpartylessImportPath returns the dethirdpartyized version of the provided import path.
// e.g. "foo/bar/thirdparty/a/b" => "a/b"
func thirdpartylessImportPath(ipath string) string {
	// Dethirdpartyize for use in import statement.
	if i := strings.LastIndex(ipath, "/thirdparty/"); i >= 0 {
		return ipath[i+len("/thirdparty/"):]
	}
	if strings.HasPrefix(ipath, "thirdparty/") {
		return ipath[len("thirdparty/"):]
	}
	return ipath
}
...
func findImportGoPath(pkgName string, symbols map[string]bool, filename string) (foundPkg string, rename bool, err error) {
	...
	// In the zrmcom case we hopefully never have to scan the GOPATH, which can
	...
	// ones.  Note that this sorts by the de-thirdpartyed name, so
	// there's no "penalty" for thirdpartying.
...
func pkgIsCandidate(filename, pkgIdent string, pkg *pkg) bool {
	// Check "internal" and "thirdparty" visibility:
...
// respecting the Go "internal" and "thirdparty" visibility rules.
func canUse(filename, dir string) bool {
	// Fast path check, before any allocations. If it doesn't contain thirdparty
	// or internal, it's not tricky:
	// Note that this can false-negative on directories like "notinternal",
	// but we check it correctly below. This is just a fast path.
	if !strings.Contains(dir, "thirdparty") && !strings.Contains(dir, "internal") {
		return true
	}

	dirSlash := filepath.ToSlash(dir)
	if !strings.Contains(dirSlash, "/thirdparty/") && !strings.Contains(dirSlash, "/internal/") && !strings.HasSuffix(dirSlash, "/internal") {
		return true
	}
	// Vendor or internal directory only visible from children of parent.
	// That means the path from the current directory to the target directory
	// can contain ../thirdparty or ../internal but not ../foo/thirdparty or ../foo/internal
	// or bar/thirdparty or bar/internal.
	// After stripping all the leading ../, the only okay place to see thirdparty or internal
	...
	return !strings.Contains(relSlash, "/thirdparty/") && !strings.Contains(relSlash, "/internal/") && !strings.HasSuffix(relSlash, "/internal")
}
