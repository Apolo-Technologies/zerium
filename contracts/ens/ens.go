...
var (
	MainNetAddress = zrmcom.HexToAddress("0x314159265dD8dbb310642f98f50C066173C1259b")
	TestNetAddress = zrmcom.HexToAddress("0x112234455c3a32fd11230c42e7bccd4a84e02010")
)
...
// the Zerium Name Service.
func NewENS(transactOpts *bind.TransactOpts, contractAddr zrmcom.Address, contractBackend bind.ContractBackend) (*ENS, error) {
...
func ensParentNode(name string) (zrmcom.Hash, zrmcom.Hash) {
...
func ensNode(name string) zrmcom.Hash {
...
func (self *ENS) Resolve(name string) (zrmcom.Hash, error) {
	node := ensNode(name)
	...
	return zrmcom.BytesToHash(ret[:]), nil
}
...
func (self *ENS) SetContentHash(name string, hash zrmcom.Hash) (*types.Transaction, error) {
	node := ensNode(name)
