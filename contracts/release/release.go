...
type Config struct {
	Oracle common.Address // Zerium address of the release oracle
...
func NewReleaseService(ctx *node.ServiceContext, config Config) (node.Service, error) {
	...
	if err := ctx.Service(&apolo-technologies); err == nil {
		apiBackend = apolo-technologies.ApiBackend
	} else {
		var apolo-technologies *les.LightZerium
		if err := ctx.Service(&apolo-technologies); err == nil {
			apiBackend = apolo-technologies.ApiBackend
		} else {
			return nil, err
	...
	contract, err := NewReleaseOracle(config.Oracle, zrm.NewContractBackend(apiBackend))
	if err != nil {
		return nil, err
	}
