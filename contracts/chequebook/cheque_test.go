...
func deploy(prvKey *ecdsa.PrivateKey, amount *big.Int, backend *backends.SimulatedBackend) (zrmcom.Address, error) {
...
func TestIssueAndReceive(t *testing.T) {
	...
	chbook.balance = new(big.Int).Set(zrmcom.Big1)
	if chbook.Balance().Cmp(zrmcom.Big1) != 0 {
...
func TestCheckbookFile(t *testing.T) {
	...
	var ch *Cheque
	if ch, err = chbook.Issue(addr1, zrmcom.Big1); err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
...
func TestVerifyErrors(t *testing.T) {
	...
	backend := newTestBackend()
	contr0, err := deploy(key0, zrmcom.Big2, backend)
	if err != nil {
	...
	contr1, _ := deploy(key1, zrmcom.Big2, backend)
	...
	chbook0.balance = new(big.Int).Set(zrmcom.Big2)
	chbook1.balance = new(big.Int).Set(zrmcom.Big1)
	amount := zrmcom.Big1
...
func TestDeposit(t *testing.T) {
	...
	// autodeposit off
	chbook.AutoDeposit(0, zrmcom.Big0, balance)
	_, err = chbook.Issue(addr1, amount)
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
	...
	chbook.AutoDeposit(interval, zrmcom.Big1, balance)
	_, err = chbook.Issue(addr1, amount)
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
	...
	chbook.AutoDeposit(1*interval, zrmcom.Big0, balance)
	chbook.Stop()

	_, err = chbook.Issue(addr1, zrmcom.Big1)
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
	backend.Commit()

	_, err = chbook.Issue(addr1, zrmcom.Big2)
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
...
func TestCash(t *testing.T) {
	path := filepath.Join(os.TempDir(), "chequebook-test.json")
	backend := newTestBackend()
	contr0, _ := deploy(key0, zrmcom.Big2, backend)
	...
	chbook.balance = new(big.Int).Set(zrmcom.Big3)
	...
	chbook.balance = new(big.Int).Set(zrmcom.Big2)
	chbox.AutoCash(0, zrmcom.Big0)
