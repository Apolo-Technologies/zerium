...
// Package chequebook package wraps the 'chequebook' Zerium smart contract.
//
...
import (
	...
	"github.com/apolo-technologies/zerium/contracts/chequebook/contract"
...
type Backend interface {
	...
	TransactionReceipt(ctx context.Context, txHash zrmcom.Hash) (*types.Receipt, error)
	BalanceAt(ctx context.Context, address zrmcom.Address, blockNum *big.Int) (*big.Int, error)
}

// Cheque represents a payment promise to a single beneficiary.
type Cheque struct {
	Contract    zrmcom.Address // address of chequebook, needed to avoid cross-contract submission
	Beneficiary zrmcom.Address
...
type Chequebook struct {
	...
	owner    zrmcom.Address              // owner address (derived from pubkey)
	contract *contract.Chequebook        // abigen binding
	session  *contract.ChequebookSession // abigen binding with Tx Opts

	// persisted fields
	balance      *big.Int                    // not synced with blockchain
	contractAddr zrmcom.Address              // contract address
	sent         map[zrmcom.Address]*big.Int //tallies for beneficiaries
...
func NewChequebook(path string, contractAddr zrmcom.Address, prvKey *ecdsa.PrivateKey, backend Backend) (self *Chequebook, err error) {
	balance := new(big.Int)
	sent := make(map[zrmcom.Address]*big.Int)
	...
	if (contractAddr != zrmcom.Address{}) {
...
func LoadChequebook(path string, prvKey *ecdsa.PrivateKey, backend Backend, checkBalance bool) (self *Chequebook, err error) {
	...
	self, _ = NewChequebook(path, zrmcom.Address{}, prvKey, backend)
...
func (self *Chequebook) UnmarshalJSON(data []byte) error {
	...
	self.contractAddr = zrmcom.HexToAddress(file.Contract)
	for addr, sent := range file.Sent {
		self.sent[zrmcom.HexToAddress(addr)], ok = new(big.Int).SetString(sent, 10)
		if !ok {
...
func (self *Chequebook) Issue(beneficiary zrmcom.Address, amount *big.Int) (ch *Cheque, err error) {
	defer self.lock.Unlock()
	self.lock.Lock()
...
func sigHash(contract, beneficiary zrmcom.Address, sum *big.Int) []byte {
	bigamount := sum.Bytes()
	if len(bigamount) > 32 {
		return nil
	}
...
func (self *Chequebook) Owner() zrmcom.Address {
	return self.owner
}

// Address returns the on-chain contract address of the chequebook.
func (self *Chequebook) Address() zrmcom.Address {
...
func NewOutbox(chbook *Chequebook, beneficiary zrmcom.Address) *Outbox {
...
type Inbox struct {
	lock        sync.Mutex
	contract    zrmcom.Address              // peer's chequebook contract
	beneficiary zrmcom.Address              // local peer's receiving address
	sender      zrmcom.Address              // local peer's address to send cashing tx from
...
func NewInbox(prvKey *ecdsa.PrivateKey, contractAddr, beneficiary zrmcom.Address, signer *ecdsa.PublicKey, abigen bind.ContractBackend) (self *Inbox, err error) {
	if signer == nil {
	...
	self = &Inbox{
		...
		signer:      signer,
		session:     session,
		cashed:      new(big.Int).Set(zrmcom.Big0),
		log:         log.New("contract", contractAddr),
	}
...
func (self *Cheque) Verify(signerKey *ecdsa.PublicKey, contract, beneficiary zrmcom.Address, sum *big.Int) (*big.Int, error) {
...
// Cash cashes the cheque by sending an Zerium transaction.
...
func ValidateCode(ctx context.Context, b Backend, address zrmcom.Address) (ok bool, err error) {
	code, err := b.CodeAt(ctx, address, nil)
	if err != nil {
		return false, err
	}
	return bytes.Equal(code, zrmcom.FromHex(contract.ContractDeployedCode)), nil
}
