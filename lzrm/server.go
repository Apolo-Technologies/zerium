...
// package lzrm implements the Light Zerium Subprotocol.
package lzrm
...
func NewLesServer(zrm *zrm.Zerium, config *zrm.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(zrm.BlockChain().Config(), false, ServerProtocolVersions, config.NetworkId, zrm.EventMux(), zrm.Engine(), newPeerSet(), zrm.BlockChain(), zrm.TxPool(), zrm.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range ServerProtocolVersions {
		lesTopics[i] = lesTopic(zrm.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		...
		quitSync:         quitSync,
		lesTopics:        lesTopics,
		chtIndexer:       light.NewChtIndexer(zrm.ChainDb(), false),
		bloomTrieIndexer: light.NewBloomTrieIndexer(zrm.ChainDb(), false),
	}
	logger := log.New()
	...
	srv.chtIndexer.Start(zrm.BlockChain())
	pm.server = srv
	...
	srv.fcCostStats = newCostStats(zrm.ChainDb())
	return srv, nil
}
...
func newCostStats(db zrmdb.Database) *requestCostStats {
