...
// meteredMsgReadWriter is a wrapper around a peertopeer.MsgReadWriter, capable of
// accumulating the above defined metrics based on the data stream contents.
type meteredMsgReadWriter struct {
	peertopeer.MsgReadWriter     // Wrapped message stream to meter
	version           int // Protocol version to select correct meters
}

// newMeteredMsgWriter wraps a peertopeer MsgReadWriter with metering support. If the
// metrics system is disabled, this function returns the original object.
func newMeteredMsgWriter(rw peertopeer.MsgReadWriter) peertopeer.MsgReadWriter {
	if !metrics.Enabled {
		return rw
	}
...
func (rw *meteredMsgReadWriter) ReadMsg() (peertopeer.Msg, error) {
...
func (rw *meteredMsgReadWriter) WriteMsg(msg peertopeer.Msg) error {
	// Account for the data traffic
	packets, traffic := miscOutPacketsMeter, miscOutTrafficMeter
	packets.Mark(1)
	traffic.Mark(int64(msg.Size))

	// Send the packet to the peertopeer layer
