...
func expectResponse(r peertopeer.MsgReader, msgcode, reqID, bv uint64, data interface{}) error {
	type resp struct {
		ReqID, BV uint64
		Data      interface{}
	}
	return peertopeer.ExpectMsg(r, msgcode, resp{reqID, bv, data})
}
...
func testGetBlockHeaders(t *testing.T, protocol int) {
	...
	tests := []struct {
		...
		// A single random block should be retrievable by hash and number too
		{
			...
			[]zrmcom.Hash{bc.GetBlockByNumber(limit / 2).Hash()},
		}, {
...
func testGetBlockBodies(t *testing.T, protocol int) {
	...
	tests := []struct {
		...
		{0, []zrmcom.Hash{bc.Genesis().Hash()}, []bool{true}, 1},      // The genesis block should be retrievable
		{0, []zrmcom.Hash{bc.CurrentBlock().Hash()}, []bool{true}, 1}, // The chains head block should be retrievable
...
func testGetProofs(t *testing.T, protocol int) {
	...
	accounts := []zrmcom.Address{testBankAddress, acc1Addr, acc2Addr, {}}
	for i := uint64(0); i <= bc.CurrentBlock().NumberU64(); i++ {
		header := bc.GetHeaderByNumber(i)
		root := header.Root
...
func TestTransactionStatusLes2(t *testing.T) {
	...
	test := func(tx *types.Transaction, send bool, expStatus txStatus) {
		...
		if send {
			...
			sendRequest(peer.app, GetTxStatusMsg, reqID, cost, []zrmcom.Hash{tx.Hash()})
		}
