...
type peer struct {
	*peertopeer.Peer
	...
	poolEntry      *poolEntry
	hasBlock       func(zrmcom.Hash, uint64) bool
	responseErrors int
...
func newPeer(version int, network uint64, p *peertopeer.Peer, rw peertopeer.MsgReadWriter) *peer {
	id := p.ID()
	pubKey, _ := id.Pubkey()
...
func (p *peer) Head() (hash zrmcom.Hash) {
	p.lock.RLock()
...
func (p *peer) HeadAndTd() (hash zrmcom.Hash, td *big.Int) {
	p.lock.RLock()
	defer p.lock.RUnlock()
...
func sendRequest(w peertopeer.MsgWriter, msgcode, reqID, cost uint64, data interface{}) error {
	type req struct {
		ReqID uint64
		Data  interface{}
	}
	return peertopeer.Send(w, msgcode, req{reqID, data})
}

func sendResponse(w peertopeer.MsgWriter, msgcode, reqID, bv uint64, data interface{}) error {
	type resp struct {
		ReqID, BV uint64
		Data      interface{}
	}
	return peertopeer.Send(w, msgcode, resp{reqID, bv, data})
}
...
func (p *peer) HasBlock(hash zrmcom.Hash, number uint64) bool {
	p.lock.RLock()
	hasBlock := p.hasBlock
	p.lock.RUnlock()
...
func (p *peer) SendAnnounce(request announceData) error {
	return peertopeer.Send(p.rw, AnnounceMsg, request)
}
...
func (p *peer) RequestHeadersByHash(reqID, cost uint64, origin zrmcom.Hash, amount int, skip int, reverse bool) error {
...
// specified.
func (p *peer) RequestBodies(reqID, cost uint64, hashes []zrmcom.Hash) error {
...
func (p *peer) RequestReceipts(reqID, cost uint64, hashes []zrmcom.Hash) error {
...
func (p *peer) RequestTxStatus(reqID, cost uint64, txHashes []zrmcom.Hash) error {
...
func (p *peer) SendTxs(reqID, cost uint64, txs types.Transactions) error {
	...
	switch p.version {
	case lpv1:
		return peertopeer.Send(p.rw, SendTxMsg, txs) // old message format does not include reqID
	case lpv2:
...
func (p *peer) sendReceiveHandshake(sendList keyValueList) (keyValueList, error) {
	...
	go func() {
		errc <- peertopeer.Send(p.rw, StatusMsg, sendList)
	}()
...
func (p *peer) Handshake(td *big.Int, head zrmcom.Hash, headNum uint64, genesis zrmcom.Hash, server *LesServer) error {
	p.lock.Lock()
	defer p.lock.Unlock()
	...
	var rGenesis, rHash zrmcom.Hash
...
func (ps *peerSet) Unregister(id string) error {
	...
	if p, ok := ps.peers[id]; !ok {
		...
		p.sendQueue.quit()
		p.Peer.Disconnect(peertopeer.DiscUselessPeer)
		return nil
	}
}
