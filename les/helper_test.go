...
// This file contains some shares testing functionality, zrmcom to  multiple
...
// fails the test.
func newTestProtocolManagerMust(t *testing.T, lightSync bool, blocks int, generator func(int, *core.BlockGen), peers *peerSet, odr *LesOdr, db zrmdb.Database) *ProtocolManager {
...
type testPeer struct {
	net peertopeer.MsgReadWriter // Network layer reader/writer to simulate remote messaging
	app *peertopeer.MsgPipeRW    // Application layer reader/writer to simulate the local side
	*peer
}
...
func newTestPeer(t *testing.T, name string, version int, pm *ProtocolManager, shake bool) (*testPeer, <-chan error) {
	...
	app, net := peertopeer.MsgPipe()

	// Generate a random id and create the peer
	var id discover.NodeID
	rand.Read(id[:])

	peer := pm.newPeer(version, NetworkId, peertopeer.NewPeer(id, name, nil), net)
...
func newTestPeerPair(name string, version int, pm, pm2 *ProtocolManager) (*peer, <-chan error, *peer, <-chan error) {
	...
	peer := pm.newPeer(version, NetworkId, peertopeer.NewPeer(id, name, nil), net)
	peer2 := pm2.newPeer(version, NetworkId, peertopeer.NewPeer(id, name, nil), app)
...
func (p *testPeer) handshake(t *testing.T, td *big.Int, head zrmcom.Hash, headNum uint64, genesis zrmcom.Hash) {
	...
	if err := peertopeer.ExpectMsg(p.app, StatusMsg, expList); err != nil {
		t.Fatalf("status recv: %v", err)
	}
	if err := peertopeer.Send(p.app, StatusMsg, sendList); err != nil {
