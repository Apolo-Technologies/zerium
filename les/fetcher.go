...
type fetcherPeerInfo struct {
	...
	nodeByHash          map[zrmcom.Hash]*fetcherTreeNode
...
func (f *lightFetcher) registerPeer(p *peer) {
	p.lock.Lock()
	p.hasBlock = func(hash zrmcom.Hash, number uint64) bool {
	...
	f.peers[p] = &fetcherPeerInfo{nodeByHash: make(map[zrmcom.Hash]*fetcherTreeNode)}
}
...
func (f *lightFetcher) announce(p *peer, head *announceData) {
	...
	if n != nil {
		// n is now the reorg zrmcom ancestor, add a new branch of nodes
	...
	if n == nil {
		// could not find reorg zrmcom ancestor or had to delete entire tree, a new root and a resync is needed
		if fp.root != nil {
			fp.deleteNode(fp.root)
		}
...
func (f *lightFetcher) peerHasBlock(p *peer, hash zrmcom.Hash, number uint64) bool {
	f.lock.Lock()
	defer f.lock.Unlock()
...
// adds it to the end of a linked list tozrmder with the time it has been confirmed. Then checks which peers have
