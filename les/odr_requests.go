...
type LesOdrRequest interface {
	...
	Validate(zrmdb.Database, *Msg) error
}
...
func (r *BlockRequest) Validate(db zrmdb.Database, msg *Msg) error {
...
// returns true and stores results in memory if the message was a valid reply
// to the request (implementation of LesOdrRequest)
func (r *ReceiptsRequest) Validate(db zrmdb.Database, msg *Msg) error {
...
// returns true and stores results in memory if the message was a valid reply
// to the request (implementation of LesOdrRequest)
func (r *TrieRequest) Validate(db zrmdb.Database, msg *Msg) error {
	...
	case MsgProofsV1:
		...
		if _, err, _ := pkg2310.VerifyProof(r.Id.Root, r.Key, nodeSet); err != nil {
	...
	case MsgProofsV2:
		...
		if _, err, _ := pkg2310.VerifyProof(r.Id.Root, r.Key, reads); err != nil {
			return fmt.Errorf("merkle proof verification failed: %v", err)
		}
...
// returns true and stores results in memory if the message was a valid reply
// to the request (implementation of LesOdrRequest)
func (r *CodeRequest) Validate(db zrmdb.Database, msg *Msg) error {
...
// returns true and stores results in memory if the message was a valid reply
// to the request (implementation of LesOdrRequest)
func (r *ChtRequest) Validate(db zrmdb.Database, msg *Msg) error {
	...
	case MsgHeaderProofs: // LES/1 backwards compatibility
		...
		value, err, _ := pkg2310.VerifyProof(r.ChtRoot, encNumber[:], light.NodeList(proof.Proof).NodeSet())
		if err != nil {
			return err
		}
	...
	case MsgHelperTrieProofs:
		...
		reads := &readTraceDB{db: nodeSet}
		value, err, _ := pkg2310.VerifyProof(r.ChtRoot, encNumber[:], reads)
		if err != nil {
			return fmt.Errorf("merkle proof verification failed: %v", err)
		}
		if len(reads.reads) != nodeSet.KeyCount() {
...
// returns true and stores results in memory if the message was a valid reply
// to the request (implementation of LesOdrRequest)
func (r *BloomRequest) Validate(db zrmdb.Database, msg *Msg) error {
	...
	for i, idx := range r.SectionIdxList {
		...
		value, err, _ := pkg2310.VerifyProof(r.BloomTrieRoot, encNumber[:], reads)
		if err != nil {
			return err
		}
		r.BloomBits[i] = value
