...
type BlockChain interface {
	HasHeader(hash zrmcom.Hash, number uint64) bool
	GetHeader(hash zrmcom.Hash, number uint64) *types.Header
	GetHeaderByHash(hash zrmcom.Hash) *types.Header
	CurrentHeader() *types.Header
	GetTdByHash(hash zrmcom.Hash) *big.Int
	InsertHeaderChain(chain []*types.Header, checkFreq int) (int, error)
	Rollback(chain []zrmcom.Hash)
	Status() (td *big.Int, currentBlock zrmcom.Hash, genesisBlock zrmcom.Hash)
	GetHeaderByNumber(number uint64) *types.Header
	GetBlockHashesFromHash(hash zrmcom.Hash, max uint64) []zrmcom.Hash
	LastBlockHash() zrmcom.Hash
	Genesis() *types.Block
...
type txPool interface {
	...
	Status(hashes []zrmcom.Hash) []core.TxStatus
}
...
type ProtocolManager struct {
	...
	SubProtocols []peertopeer.Protocol
...
func NewProtocolManager(chainConfig *params.ChainConfig, lightSync bool, protocolVersions []uint, networkId uint64, mux *event.TypeMux, engine consensus.Engine, peers *peerSet, blockchain BlockChain, txpool txPool, chainDb zrmdb.Database, odr *LesOdr, txrelay *LesTxRelay, quitSync chan struct{}, wg *sync.WaitGroup) (*ProtocolManager, error) {
	...
	manager.SubProtocols = make([]peertopeer.Protocol, 0, len(protocolVersions))
	for _, version := range protocolVersions {
		// Compatible, initialize the sub-protocol
		version := version // Closure for the run
		manager.SubProtocols = append(manager.SubProtocols, peertopeer.Protocol{
			Name:    "les",
			Version: version,
			Length:  ProtocolLengths[version],
			Run: func(p *peertopeer.Peer, rw peertopeer.MsgReadWriter) error {
				var entry *poolEntry
				...
				case <-manager.quitSync:
					...
					return peertopeer.DiscQuitting
				}
			},
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light Zerium protocol")
	...
	log.Info("Light Zerium protocol stopped")
}

func (pm *ProtocolManager) newPeer(pv int, nv uint64, p *peertopeer.Peer, rw peertopeer.MsgReadWriter) *peer {
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light Zerium peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light Zerium message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light Zerium message arrived", "code", msg.Code, "bytes", msg.Size)
	...
	reject := func(reqCnt, maxCnt uint64) bool {
		...
		if cost > bufValue {
			...
			p.Log().Error("Request came too early", "recharge", zrmcom.PrettyDuration(recharge))
			return true
		}
		return false
	}
	...
	case GetBlockHeadersMsg:
		...
		hashMode := query.Origin.Hash != (zrmcom.Hash{})

		// Gather headers until the fetch or network limits is reached
		var (
			bytes   zrmcom.StorageSize
		...
		for !unknown && len(headers) < int(query.Amount) && bytes < softResponseLimit {
			...
			switch {
			case query.Origin.Hash != (zrmcom.Hash{}) && query.Reverse:
			...
			if tr != nil {
				if len(req.AccKey) > 0 {
					if str == nil || !bytes.Equal(req.AccKey, lastAccKey) {
						...
						lastAccKey = zrmcom.CopyBytes(req.AccKey)
					}
					if str != nil {
	...
	case GetHeaderProofsMsg:
		...
		for _, req := range req.Reqs {
			...
			if header := pm.blockchain.GetHeaderByNumber(req.BlockNum); header != nil {
				...
				if root := light.GetChtRoot(pm.chainDb, req.ChtNum, sectionHead); root != (zrmcom.Hash{}) {
	...
	case SendTxV2Msg:
		...
		hashes := make([]zrmcom.Hash, len(req.Txs))
		...
		for i, stat := range stats {
			if stat.Status == core.TxStatusUnknown {
				...
				stats[i] = pm.txStatus([]zrmcom.Hash{hashes[i]})[0]
			}
		}
...
func (pm *ProtocolManager) getHelperTrie(id uint, idx uint64) (zrmcom.Hash, string) {
	switch id {
	case htCanonical:
...
func (pm *ProtocolManager) txStatus(hashes []zrmcom.Hash) []txStatus {
	...
	for i, stat := range pm.txpool.Status(hashes) {
		...
		if stat == core.TxStatusUnknown {
			if block, number, index := core.GetTxLookupEntry(pm.chainDb, hashes[i]); block != (zrmcom.Hash{}) {
...
func (pc *peerConnection) Head() (zrmcom.Hash, *big.Int) {
	return pc.peer.HeadAndTd()
}

func (pc *peerConnection) RequestHeadersByHash(origin zrmcom.Hash, amount int, skip int, reverse bool) error {
	reqID := genReqID()
	rq := &distReq{
