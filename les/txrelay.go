...
type LesTxRelay struct {
	txSent       map[zrmcom.Hash]*ltrInfo
	txPending    map[zrmcom.Hash]struct{}
	ps           *peerSet
	peerList     []*peer
	peerStartPos int
	lock         sync.RWMutex
...
func NewLesTxRelay(ps *peerSet, reqDist *requestDistributor) *LesTxRelay {
	r := &LesTxRelay{
		txSent:    make(map[zrmcom.Hash]*ltrInfo),
		txPending: make(map[zrmcom.Hash]struct{}),
		ps:        ps,
		reqDist:   reqDist,
	}
	ps.notify(r)
...
func (self *LesTxRelay) NewHead(head zrmcom.Hash, mined []zrmcom.Hash, rollback []zrmcom.Hash) {
	self.lock.Lock()
	defer self.lock.Unlock()
...
func (self *LesTxRelay) Discard(hashes []zrmcom.Hash) {
	self.lock.Lock()
	defer self.lock.Unlock()
