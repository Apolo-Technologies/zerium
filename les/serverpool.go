...
func newServerPool(db zrmdb.Database, quit chan struct{}, wg *sync.WaitGroup) *serverPool {
	pool := &serverPool{
		db:           db,
		quit:         quit,
		wg:           wg,
...
func (pool *serverPool) start(server *peertopeer.Server, topic discv5.Topic) {
	pool.server = server
	pool.topic = topic
