...
func secAddr(addr zrmcom.Address) []byte {
	return crypto.Keccak256(addr[:])
}

type accessTestFn func(db zrmdb.Database, bhash zrmcom.Hash, number uint64) light.OdrRequest

func TestBlockAccessLes1(t *testing.T) { testAccess(t, 1, tfBlockAccess) }

func TestBlockAccessLes2(t *testing.T) { testAccess(t, 2, tfBlockAccess) }

func tfBlockAccess(db zrmdb.Database, bhash zrmcom.Hash, number uint64) light.OdrRequest {
...
func tfReceiptsAccess(db zrmdb.Database, bhash zrmcom.Hash, number uint64) light.OdrRequest {
...
func tfTrieEntryAccess(db zrmdb.Database, bhash zrmcom.Hash, number uint64) light.OdrRequest {
...
func tfCodeAccess(db zrmdb.Database, bhash zrmcom.Hash, number uint64) light.OdrRequest {
	header := core.GetHeader(db, bhash, core.GetBlockNumber(db, bhash))
	if header.Number.Uint64() < testContractDeployed {
		return nil
	}
	sti := light.StateTrieID(header)
	ci := light.StorageTrieID(sti, crypto.Keccak256Hash(testContractAddr[:]), zrmcom.Hash{})
