...
type odrTestFn func(ctx context.Context, db zrmdb.Database, config *params.ChainConfig, bc *core.BlockChain, lc *light.LightChain, bhash zrmcom.Hash) []byte

func TestOdrGetBlockLes1(t *testing.T) { testOdr(t, 1, 1, odrGetBlock) }

func TestOdrGetBlockLes2(t *testing.T) { testOdr(t, 2, 1, odrGetBlock) }

func odrGetBlock(ctx context.Context, db zrmdb.Database, config *params.ChainConfig, bc *core.BlockChain, lc *light.LightChain, bhash zrmcom.Hash) []byte {
...
func odrGetReceipts(ctx context.Context, db zrmdb.Database, config *params.ChainConfig, bc *core.BlockChain, lc *light.LightChain, bhash zrmcom.Hash) []byte {
...
func odrAccounts(ctx context.Context, db zrmdb.Database, config *params.ChainConfig, bc *core.BlockChain, lc *light.LightChain, bhash zrmcom.Hash) []byte {
...
func odrContractCall(ctx context.Context, db zrmdb.Database, config *params.ChainConfig, bc *core.BlockChain, lc *light.LightChain, bhash zrmcom.Hash) []byte {
	...
	for i := 0; i < 3; i++ {
		...
		if bc != nil {
			...
			if err == nil {
				...
				context := core.NewZvmContext(msg, header, bc, nil)
				vmenv := vm.NewZvm(context, statedb, config, vm.Config{})
			...
			context := core.NewZvmContext(msg, header, lc, nil)
			vmenv := vm.NewZvm(context, state, config, vm.Config{})
			gp := new(core.GasPool).AddGas(math.MaxBig256)
			ret, _, _, _ := core.ApplyMessage(vmenv, msg, gp)
			if state.Error() == nil {
				res = append(res, ret...)
...
func testOdr(t *testing.T, protocol int, expFail uint64, fn odrTestFn) {
	...
	time.Sleep(time.Millisecond * 10) // ensure that all peerSetNotify callbacks are executed
	lpeer.lock.Lock()
	lpeer.hasBlock = func(zrmcom.Hash, uint64) bool { return true }
	lpeer.lock.Unlock()
	test(5)
