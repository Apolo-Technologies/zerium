...
func (t *testService) Protocols() []peertopeer.Protocol {
...
func (t *testService) Start(server *peertopeer.Server) error {
	return nil
}
...
func (t *testService) handshake(rw peertopeer.MsgReadWriter, code uint64) error {
	errc := make(chan error, 2)
	go func() { errc <- peertopeer.Send(rw, code, struct{}{}) }()
	go func() { errc <- peertopeer.ExpectMsg(rw, code, struct{}{}) }()
	for i := 0; i < 2; i++ {
		if err := <-errc; err != nil {
			return err
		}
...
func (t *testService) RunTest(p *peertopeer.Peer, rw peertopeer.MsgReadWriter) error {
	peer := t.peer(p.ID())
...
func (t *testService) RunDum(p *peertopeer.Peer, rw peertopeer.MsgReadWriter) error {
	peer := t.peer(p.ID())
...
func (t *testService) RunPrb(p *peertopeer.Peer, rw peertopeer.MsgReadWriter) error {
	peer := t.peer(p.ID())
...
func TestHTTPSnapshot(t *testing.T) {
	...
	client := NewClient(s.URL)
	nodeCount := 2
	nodes := make([]*peertopeer.NodeInfo, nodeCount)
	for i := 0; i < nodeCount; i++ {
		node, err := client.CreateNode(nil)
		if err != nil {
			t.Fatalf("error creating node: %s", err)
