...
import (
	...
	"github.com/apolo-technologies/zerium/peertopeer/simulations"
...
func main() {
	...
	case "exec":
		tmpdir, err := ioutil.TempDir("", "peertopeer-example")
		if err != nil {
...
func (p *pingPongService) Protocols() []peertopeer.Protocol {
...
func (p *pingPongService) Start(server *peertopeer.Server) error {
...
func (p *pingPongService) Run(peer *peertopeer.Peer, rw peertopeer.MsgReadWriter) error {
	...
	go func() {
		for range time.Tick(10 * time.Second) {
			...
			if err := peertopeer.Send(rw, pingMsgCode, "PING"); err != nil {
				errC <- err
				return
			}
		}
	...
	go func() {
		for {
			...
			if msg.Code == pingMsgCode {
				...
				go peertopeer.Send(rw, pongMsgCode, "PONG")
			}
		}
	}()
	return <-errC
