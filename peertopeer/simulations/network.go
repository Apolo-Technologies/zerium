...
import (
	...
	"github.com/apolo-technologies/zerium/peertopeer/simulations/adapters"
)
...
// Network models a peertopeer simulation network which consists of a collection of
...
// starting nodes and connecting them tozrmder.
//
...
func (self *Network) startWithSnapshots(id discover.NodeID, snapshots map[string][]byte) error {
	...
	events := make(chan *peertopeer.PeerEvent)
...
func (self *Network) watchPeerEvents(id discover.NodeID, events chan *peertopeer.PeerEvent, sub event.Subscription) {
	defer func() {
		sub.Unsubscribe()
	...
	for {
		...
		case event, ok := <-events:
			...
			case peertopeer.PeerEventTypeAdd:
				self.DidConnect(id, peer)

			case peertopeer.PeerEventTypeDrop:
				self.DidDisconnect(id, peer)

			case peertopeer.PeerEventTypeMsgSend:
				self.DidSend(id, peer, event.Protocol, *event.MsgCode)

			case peertopeer.PeerEventTypeMsgRecv:
...
// Connect connects two nodes tozrmder by calling the "admin_addPeer" RPC
...
func (self *Node) NodeInfo() *peertopeer.NodeInfo {
...
func (self *Node) MarshalJSON() ([]byte, error) {
	return json.Marshal(struct {
		Info   *peertopeer.NodeInfo        `json:"info,omitempty"`
...
// Msg represents a peertopeer message sent between two nodes in the network
type Msg struct {
	One      discover.NodeID `json:"one"`
	Other    discover.NodeID `json:"other"`
