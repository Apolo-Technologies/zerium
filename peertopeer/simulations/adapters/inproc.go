...
// Dial implements the peertopeer.NodeDialer interface by connecting to the node using
...
// an in-memory net.Pipe connection (see SimAdapter.Dial), running devpeertopeer
...
// Start registers the services and starts the underlying devpeertopeer node
...
// Stop closes the RPC client and stops the underlying devpeertopeer node
...
// Server returns the underlying peertopeer.Server
func (self *SimNode) Server() *peertopeer.Server {
	return self.node.Server()
}

// SubscribeEvents subscribes the given channel to peer events from the
// underlying peertopeer.Server
func (self *SimNode) SubscribeEvents(ch chan *peertopeer.PeerEvent) event.Subscription {
	srv := self.Server()
	if srv == nil {
...
func (self *SimNode) NodeInfo() *peertopeer.NodeInfo {
	server := self.Server()
	if server == nil {
		return &peertopeer.NodeInfo{
			ID:    self.ID.String(),
