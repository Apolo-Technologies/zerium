...
// the current binary but setting argv[0] to "peertopeer-node" so that the child
// runs execP2PNode
func (n *ExecNode) execCommand() *exec.Cmd {
	return &exec.Cmd{
		Path: reexec.Self(),
		Args: []string{"peertopeer-node", strings.Join(n.Config.Node.Services, ","), n.ID.String()},
	}
}
...
func (n *ExecNode) NodeInfo() *peertopeer.NodeInfo {
	info := &peertopeer.NodeInfo{
		ID: n.ID.String(),
	}
	if n.client != nil {
...
func init() {
	// register a reexec function to start a devpeertopeer node when the current
	// binary is executed as "peertopeer-node"
	reexec.Register("peertopeer-node", execP2PNode)
}
...
// execP2PNode starts a devpeertopeer node when the current binary is executed with
// argv[0] being "peertopeer-node", reading the service / ID from argv[1] / argv[2]
...
func execP2PNode() {
	...
	// initialize the devpeertopeer stack
...
func (s *snapshotService) Protocols() []peertopeer.Protocol {
	return nil
}

func (s *snapshotService) Start(*peertopeer.Server) error {
	return nil
}
