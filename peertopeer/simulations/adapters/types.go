...
type NodeConfig struct {
	...
	// PrivateKey is the node's private key which is used by the devpeertopeer
...
// ServiceFunc returns a node.Service which can be used to boot a devpeertopeer node
type ServiceFunc func(ctx *ServiceContext) (node.Service, error)

// serviceFuncs is a map of registered services which are used to boot devpeertopeer
// nodes
var serviceFuncs = make(Services)

// RegisterServices registers the given Services which can then be used to
// start devpeertopeer nodes using either the Exec or Docker adapters.
//
...
func RegisterServices(services Services) {
	...
	// been exec'd with argv[0] set to "peertopeer-node"
	if reexec.Init() {
		os.Exit(0)
	}
}
