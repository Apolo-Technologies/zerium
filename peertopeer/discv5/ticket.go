...
type ticketStore struct {
	...
	queriesSent           map[*Node]map[zrmcom.Hash]sentQuery
}
...
func newTicketStore() *ticketStore {
	return &ticketStore{
		...
		queriesSent:    make(map[*Node]map[zrmcom.Hash]sentQuery),
	}
}
	...
	for i, n := range nodes {
		if i == 0 || (binary.BigEndian.Uint64(n.sha[:8])^binary.BigEndian.Uint64(lookup.target[:8])) < s.radius[lookup.topic].minRadius {
			...
			if s.canQueryTopic(n, lookup.topic) {
				...
				if hash != nil {
					s.addTopicQuery(zrmcom.BytesToHash(hash), n, lookup)
				}
			}
			//}
		}
...
func (s *ticketStore) adjustWithTicket(now mclock.AbsTime, targetHash zrmcom.Hash, t *ticket) {
...
func (s *ticketStore) addTopicQuery(hash zrmcom.Hash, node *Node, lookup lookupInfo) {
	now := mclock.Now()
	qq := s.queriesSent[node]
	if qq == nil {
		qq = make(map[zrmcom.Hash]sentQuery)
...
func (s *ticketStore) gotTopicNodes(from *Node, hash zrmcom.Hash, nodes []rpcNode) (timeout bool) {
	now := mclock.Now()
...
type topicRadiusBucket struct {
	...
	value      float64
	lookupSent map[zrmcom.Hash]mclock.AbsTime
}
...
func (r *topicRadius) getBucketIdx(addrHash zrmcom.Hash) int {
...
func (r *topicRadius) targetForBucket(bucket int) zrmcom.Hash {
...
func (r *topicRadius) isInRadius(addrHash zrmcom.Hash) bool {
...
func (r *topicRadius) recalcRadius() (radius uint64, radiusLookup int) {
	...
	if slopeCross != maxBucket && (minRadBucket <= maxBucket || r.needMoreLookups(maxBucket+lookupWidth, len(r.buckets)-1, maxValue)) {
		for len(r.buckets) <= maxBucket+lookupWidth {
			r.buckets = append(r.buckets, topicRadiusBucket{lookupSent: make(map[zrmcom.Hash]mclock.AbsTime)})
		}
...
func (r *topicRadius) adjustWithTicket(now mclock.AbsTime, targetHash zrmcom.Hash, t ticketRef) {
...
func (r *topicRadius) adjust(now mclock.AbsTime, targetHash, addrHash zrmcom.Hash, inside float64) {
