...
type transport interface {
	...
	sendFindnodeHash(remote *Node, target zrmcom.Hash)
	sendTopicRegister(remote *Node, topics []Topic, topicIdx int, pong []byte)
	sendTopicNodes(remote *Node, queryHash zrmcom.Hash, nodes []*Node)
...
func (net *Network) lookup(target zrmcom.Hash, stopOnMatch bool) []*Node {
	var (
...
func (net *Network) reqQueryFindnode(n *Node, target zrmcom.Hash, reply chan []*Node) bool {
...
loop:
	for {
		...
		case req := <-net.topicRegisterReq:
			...
			if topicRegisterLookupTarget.target == (zrmcom.Hash{}) {
