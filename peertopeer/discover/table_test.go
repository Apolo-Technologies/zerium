...
func TestBucket_bumpNoDuplicates(t *testing.T) {
	...
	cfg := &quick.Config{
		...
		Values: func(args []reflect.Value, rand *rand.Rand) {
			...
			nodes := make([]*Node, n)
			for i := range nodes {
				nodes[i] = nodeAtDistance(zrmcom.Hash{}, 200)
			}
...
func nodeAtDistance(base zrmcom.Hash, ld int) (n *Node) {
	n = new(Node)
...
func (*closeTest) Generate(rand *rand.Rand, size int) reflect.Value {
	t := &closeTest{
		...
		Target: gen(zrmcom.Hash{}, rand).(zrmcom.Hash),
...
func TestTable_Lookup(t *testing.T) {
	self := nodeAtDistance(zrmcom.Hash{}, 0)
...
var lookupTestnet = &preminedTestnet{
	...
	targetSha: zrmcom.Hash{0x5c, 0x94, 0x4e, 0xe5, 0x1c, 0x5a, 0xe9, 0xf7, 0x2a, 0x95, 0xec, 0xcb, 0x8a, 0xed, 0x3, 0x74, 0xee, 0xcb, 0x51, 0x19, 0xd7, 0x20, 0xcb, 0xea, 0x68, 0x13, 0xe8, 0xe0, 0xd6, 0xad, 0x92, 0x61},
	dists: [257][]NodeID{
		240: {
...
type preminedTestnet struct {
	target    NodeID
	targetSha zrmcom.Hash // sha3(target)
...
func sortedByDistanceTo(distbase zrmcom.Hash, slice []*Node) bool {
	var last zrmcom.Hash
	for i, e := range slice {
