...
// Reference: https://github.com/apolo-technologies/wiki/wiki/Abthash-DAG
#define DAG_MUTABLE_NAME_MAX_SIZE (6 + 10 + 1 + 16 + 1)
/// Possible return values of @see abthash_io_prepare
enum abthash_io_rc {
	ZRMHASH_IO_FAIL = 0,           ///< There has been an IO failure
	ZRMHASH_IO_MEMO_SIZE_MISMATCH, ///< DAG with revision/hash match, but file size was wrong.
	ZRMHASH_IO_MEMO_MISMATCH,      ///< The DAG file did not exist or there was revision/hash mismatch
	ZRMHASH_IO_MEMO_MATCH,         ///< DAG file existed and revision/hash matched. No need to do anything
};
...
/**
 * Logs a critical error in important parts of abthash. Should mostly help
 * figure out what kind of problem (I/O, memory e.t.c.) causes a NULL
 * abthash_full_t
 */
#ifdef ZRMHASH_PRINT_CRITICAL_OUTPUT
#define ZRMHASH_CRITICAL(...)							\
	do													\
	{													\
		printf("ZRMHASH CRITICAL ERROR: "__VA_ARGS__);	\
		printf("\n");									\
		fflush(stdout);									\
	} while (0)
#else
...
/**
 * Prepares io for abthash
 *
 * Create the DAG directory and the DAG file if they don't exist.
 *
 * @param[in] dirname        A null terminated c-string of the path of the abthash
 ...
 *                           https://github.com/apolo-technologies/wiki/wiki/Abthash-DAG
 ...
 *                           already exists
 * @return                   For possible return values @see enum abthash_io_rc
 */
 ...
 *
 * The spec defining this directory is here: https://github.com/apolo-technologies/wiki/wiki/Abthash-DAG
 *
...
static inline bool abthash_io_mutable_name(
	uint32_t revision,
