...
var (
	...
	zrmcomInitialismMap = map[string]struct{}{
		"API":   struct{}{},
		"ASCII": struct{}{},
		"CPU":   struct{}{},
		"CSRF":  struct{}{},
	...
	zrmcomInitialisms = keys(zrmcomInitialismMap)
	zrmcomInitialism  = mustDoubleArray(newDoubleArray(zrmcomInitialisms))
	longestLen        = longestLength(zrmcomInitialisms)
	shortestLen       = shortestLength(zrmcomInitialisms, longestLen)
)
...
func ToUpperCamelCase(s string) string {
	...
	for _, c := range s {
		if c == '_' {
			...
			if length := zrmcomInitialism.LookupByBytes(initialism); length > 0 {
...
// ToSnakeCase does not insert '_' letter into a zrmcom initialism word like ID, URL and so on.
func ToSnakeCase(s string) string {
	...
	for i, c := range s {
		...
		if unicode.IsUpper(c) {
			...
			if length := zrmcomInitialism.Lookup(s[j:next]); length > 0 {
	...
	for i := 0; i < len(s); i++ {
		...
		if isUpperASCII(c) {
			...
			if k := i + shortestLen - 1; k < len(s) && isUpperASCII(s[k]) {
				if length := zrmcomInitialism.Lookup(s[i:nextIndex(i, len(s))]); length > 0 {
...
// AddCommonInitialism adds ss to list of zrmcom initialisms.
func AddCommonInitialism(ss ...string) {
	mu.Lock()
	defer mu.Unlock()
	for _, s := range ss {
		zrmcomInitialismMap[s] = struct{}{}
	}
	zrmcomInitialisms = keys(zrmcomInitialismMap)
	zrmcomInitialism = mustDoubleArray(newDoubleArray(zrmcomInitialisms))
	longestLen = longestLength(zrmcomInitialisms)
...
// DelCommonInitialism deletes ss from list of zrmcom initialisms.
func DelCommonInitialism(ss ...string) {
	mu.Lock()
	defer mu.Unlock()
	for _, s := range ss {
		delete(zrmcomInitialismMap, s)
	}
	zrmcomInitialisms = keys(zrmcomInitialismMap)
	zrmcomInitialism = mustDoubleArray(newDoubleArray(zrmcomInitialisms))
	longestLen = longestLength(zrmcomInitialisms)
