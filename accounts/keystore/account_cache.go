...
type accountCache struct {
	...
	mu       sync.Mutex
	all      accountsByURL
	byAddr   map[zrmcom.Address][]accounts.Account
	throttle *time.Timer
	notify   chan struct{}
	fileC    fileCache
}
...
func newAccountCache(keydir string) (*accountCache, chan struct{}) {
	ac := &accountCache{
		keydir: keydir,
		byAddr: make(map[zrmcom.Address][]accounts.Account),
...
func (ac *accountCache) hasAddress(addr zrmcom.Address) bool {
	ac.maybeReload()
	ac.mu.Lock()
	defer ac.mu.Unlock()
...
func (ac *accountCache) scanAccounts() error {
	...
	readAccount := func(path string) *accounts.Account {
		...
		addr := zrmcom.HexToAddress(key.Address)
		switch {
		case err != nil:
			log.Debug("Failed to decode keystore key", "path", path, "err", err)
		case (addr == zrmcom.Address{}):
