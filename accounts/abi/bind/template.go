...
type tmplContract struct {
	...
	InputBin    string                 // Optional Zvm bytecode used to denetare deploy code from
...
{{range $contract := .Contracts}}
	...
	{{if .InputBin}}
		...
		// Deploy{{.Type}} deploys a new Zerium contract, binding an instance of {{.Type}} to it.
		func Deploy{{.Type}}(auth *bind.TransactOpts, backend bind.ContractBackend {{range .Constructor.Inputs}}, {{.Name}} {{bindtype .Type}}{{end}}) (zrmcom.Address, *types.Transaction, *{{.Type}}, error) {
		  parsed, err := abi.JSON(strings.NewReader({{.Type}}ABI))
		  if err != nil {
		    return zrmcom.Address{}, nil, nil, err
		  }
		  address, tx, contract, err := bind.DeployContract(auth, parsed, zrmcom.FromHex({{.Type}}Bin), backend {{range .Constructor.Inputs}}, {{.Name}}{{end}})
		  if err != nil {
		    return zrmcom.Address{}, nil, nil, err
		  }
	...
	// {{.Type}} is an auto generated Go binding around an Zerium contract.
	type {{.Type}} struct {
	  {{.Type}}Caller     // Read-only binding to the contract
	  {{.Type}}Transactor // Write-only binding to the contract
	}

	// {{.Type}}Caller is an auto generated read-only Go binding around an Zerium contract.
	type {{.Type}}Caller struct {
	  contract *bind.BoundContract // Generic contract wrapper for the low level calls
	}

	// {{.Type}}Transactor is an auto generated write-only Go binding around an Zerium contract.
	type {{.Type}}Transactor struct {
	  contract *bind.BoundContract // Generic contract wrapper for the low level calls
	}

	// {{.Type}}Session is an auto generated Go binding around an Zerium contract,
	...
	// {{.Type}}CallerSession is an auto generated read-only Go binding around an Zerium contract,
	...
	// {{.Type}}TransactorSession is an auto generated write-only Go binding around an Zerium contract,
	...
	// {{.Type}}Raw is an auto generated low-level Go binding around an Zerium contract.
	type {{.Type}}Raw struct {
	  Contract *{{.Type}} // Generic contract binding to access the raw methods on
	}

	// {{.Type}}CallerRaw is an auto generated low-level read-only Go binding around an Zerium contract.
	type {{.Type}}CallerRaw struct {
		Contract *{{.Type}}Caller // Generic read-only contract binding to access the raw methods on
	}

	// {{.Type}}TransactorRaw is an auto generated low-level write-only Go binding around an Zerium contract.
	type {{.Type}}TransactorRaw struct {
		Contract *{{.Type}}Transactor // Generic write-only contract binding to access the raw methods on
	}

	// New{{.Type}} creates a new instance of {{.Type}}, bound to a specific deployed contract.
	func New{{.Type}}(address zrmcom.Address, backend bind.ContractBackend) (*{{.Type}}, error) {
	...
	func New{{.Type}}Caller(address zrmcom.Address, caller bind.ContractCaller) (*{{.Type}}Caller, error) {
	...
	func New{{.Type}}Transactor(address zrmcom.Address, transactor bind.ContractTransactor) (*{{.Type}}Transactor, error) {
	...
	func bind{{.Type}}(address zrmcom.Address, caller bind.ContractCaller, transactor bind.ContractTransactor) (*bind.BoundContract, error) {
	  parsed, err := abi.JSON(strings.NewReader({{.Type}}ABI))
	  if err != nil {
	    return nil, err
	  }
...
import org.zerium.zrmd.*;
import org.zerium.zrmd.internal.*;
...
{{range $contract := .Contracts}}
	public class {{.Type}} {
		...
		{{if .InputBin}}
			...
			// deploy deploys a new Zerium contract, binding an instance of {{.Type}} to it.
			public static {{.Type}} deploy(TransactOpts auth, ZeriumClient client{{range .Constructor.Inputs}}, {{bindtype .Type}} {{.Name}}{{end}}) throws Exception {
		...
		// Zerium address where this contract is located at.
		public final Address Address;

		// Zerium transaction in which this contract was deployed (if known!).
		...
		public {{.Type}}(Address address, ZeriumClient client) throws Exception {
