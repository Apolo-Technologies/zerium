...
func WaitDeployed(ctx context.Context, b DeployBackend, tx *types.Transaction) (zrmcom.Address, error) {
	if tx.To() != nil {
		return zrmcom.Address{}, fmt.Errorf("tx is not contract creation")
	}
	...
	if receipt.ContractAddress == (zrmcom.Address{}) {
		return zrmcom.Address{}, fmt.Errorf("zero address")
	}
