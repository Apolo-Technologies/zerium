...
func ExampleJSON() {
	...
	out, err := abi.Pack("isBar", zrmcom.HexToAddress("01"))
	if err != nil {
		log.Fatalln(err)
	}
...
func TestInputVariableInputLength(t *testing.T) {
	...
	value := zrmcom.RightPadBytes([]byte(strin), 32)
	...
	value1 := zrmcom.RightPadBytes([]byte(str1), 32)

	offset2 := make([]byte, 32)
	offset2[31] = 128
	length2 := make([]byte, 32)
	length2[31] = byte(len(str2))
	value2 := zrmcom.RightPadBytes([]byte(str2), 32)
	...
	length1 = make([]byte, 32)
	length1[31] = byte(len(str1))
	value1 = zrmcom.RightPadBytes([]byte(str1), 64)
	offset2[31] = 160
	...
	length2 = make([]byte, 32)
	length2[31] = byte(len(str2))
	value2 = zrmcom.RightPadBytes([]byte(str2), 64)
