...
func packBytesSlice(bytes []byte, l int) []byte {
	...
	return append(len, zrmcom.RightPadBytes(bytes, (l+31)/32*32)...)
}
...
func packElement(t Type, reflectValue reflect.Value) []byte {
	...
	case AddressTy:
		...
		return zrmcom.LeftPadBytes(reflectValue.Bytes(), 32)
	case BoolTy:
		if reflectValue.Bool() {
			return math.PaddedBigBytes(zrmcom.Big1, 32)
		} else {
			return math.PaddedBigBytes(zrmcom.Big0, 32)
		}
	case BytesTy:
		if reflectValue.Kind() == reflect.Array {
			reflectValue = mustArrayToByteSlice(reflectValue)
	...
	case FixedBytesTy, FunctionTy:
		...
		return zrmcom.RightPadBytes(reflectValue.Bytes(), 32)
	default:
