...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the Zerium
	// address located on that path.
	Derive(path accounts.DerivationPath) (zrmcom.Address, error)

	// SignTx sends the transaction to the USB device and waits for the user to confirm
	// or deny the transaction.
	SignTx(path accounts.DerivationPath, tx *types.Transaction, chainID *big.Int) (zrmcom.Address, *types.Transaction, error)
}
...
type wallet struct {
	...
	paths    map[zrmcom.Address]accounts.DerivationPath // Known derivation paths for signing operations

	deriveNextPath accounts.DerivationPath   // Next derivation path for account auto-discovery
	deriveNextAddr zrmcom.Address            // Next derived account address for auto-discovery
	deriveChain    zerium.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) Open(passphrase string) error {
	...
	w.paths = make(map[zrmcom.Address]accounts.DerivationPath)
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived Zerium account
			if nextAddr == (zrmcom.Address{}) {
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain zerium.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
	...
	w.deriveNextAddr = zrmcom.Address{}
	w.deriveChain = chain
}
