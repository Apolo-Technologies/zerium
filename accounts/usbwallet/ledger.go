...
// https://raw.githubusercontent.com/LedgerHQ/blue-app-zrm/master/doc/ethapp.asc
...
type ledgerDriver struct {
	...
	browser bool          // Flag whzerium the Ledger is in browser mode (reply channel mismatch)
...
// offline returns whzerium the wallet and the Zerium app is offline or not.
//
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// Create the transaction RLP based on whzerium legacy or EIP155 signing was requeste
	var (
		txrlp []byte
		err   error
	)
