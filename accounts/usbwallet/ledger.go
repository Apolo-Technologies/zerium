...
// https://raw.githubusercontent.com/LedgerHQ/blue-app-eth/master/doc/zrmapp.asc
...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and Zerium address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an Zerium transaction after having the user validate the parameters
...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "Zerium app in browser mode", w.failure
	}
	if w.offline() {
		return "Zerium app offline", w.failure
	}
	return fmt.Sprintf("Zerium app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}

// offline returns whether the wallet and the Zerium app is offline or not.
//
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// Zerium app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the Zerium app's version, will fail prior to v1.0.2
...
// and returning the Zerium address located on that derivation path.
func (w *ledgerDriver) Derive(path accounts.DerivationPath) (zrmcom.Address, error) {
...
func (w *ledgerDriver) SignTx(path accounts.DerivationPath, tx *types.Transaction, chainID *big.Int) (zrmcom.Address, *types.Transaction, error) {
	// If the Zerium app doesn't run, abort
	if w.offline() {
		return zrmcom.Address{}, nil, accounts.ErrWalletClosed
	}
	// Ensure the wallet is capable of signing the given transaction
	if chainID != nil && w.version[0] <= 1 && w.version[1] <= 0 && w.version[2] <= 2 {
		return zrmcom.Address{}, nil, fmt.Errorf("Ledger v%d.%d.%d doesn't support signing this transaction, please update to v1.0.3 at least", w.version[0], w.version[1], w.version[2])
	}
...
// ledgerDerive retrieves the currently active Zerium address from a Ledger
...
//   Zerium address length | 1 byte
//   Zerium address        | 40 bytes hex ascii
//   Chain code if requested | 32 bytes
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (zrmcom.Address, error) {
	...
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return zrmcom.Address{}, errors.New("reply lacks public key entry")
	}
	reply = reply[1+int(reply[0]):]

	// Extract the Zerium hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return zrmcom.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into an Zerium address and return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (zrmcom.Address, *types.Transaction, error) {
	// Flatten the derivation path into the Ledger request
	path := make([]byte, 1+4*len(derivationPath))
	path[0] = byte(len(derivationPath))
	for i, component := range derivationPath {
	...
	// Extract the Zerium signature and do a sanity validation
	if len(reply) != 65 {
		return zrmcom.Address{}, nil, errors.New("reply lacks signature")
	}
...
//
// The zrmcom transport header is defined as follows:
//
//  Description                           | Length
//  --------------------------------------+----------
