...
// or whether the Zerium app was not started on it.
...
func (w *trezorDriver) Derive(path accounts.DerivationPath) (zrmcom.Address, error) {
	return w.trezorDerive(path)
}

// SignTx implements usbwallet.driver, sending the transaction to the Trezor and
// waiting for the user to confirm or deny the transaction.
func (w *trezorDriver) SignTx(path accounts.DerivationPath, tx *types.Transaction, chainID *big.Int) (zrmcom.Address, *types.Transaction, error) {
	if w.device == nil {
...
// Zerium address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (zrmcom.Address, error) {
	address := new(trezor.ZeriumAddress)
	if _, err := w.trezorExchange(&trezor.ZeriumGetAddress{AddressN: derivationPath}, address); err != nil {
		return zrmcom.Address{}, err
	}
	return zrmcom.BytesToAddress(address.GetAddress()), nil
}

// trezorSign sends the transaction to the Trezor wallet, and waits for the user
// to confirm or deny the transaction.
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (zrmcom.Address, *types.Transaction, error) {
	// Create the transaction initiation message
	data := tx.Data()
	length := uint32(len(data))

	request := &trezor.ZeriumSignTx{
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.ZeriumTxAck{DataChunk: chunk}, response); err != nil {
			return zrmcom.Address{}, nil, err
		}
	}
