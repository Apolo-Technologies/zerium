...
var (
	// Files that end up in the gzrm*.zip archive.
	gzrmArchiveFiles = []string{
		"COPYING",
		executablePath("gzrm"),
	}
...
func doArchive(cmdline []string) {
	...
	var (
		...
		gzrm     = "gzrm-" + base + ext
		alltools = "gzrm-alltools-" + base + ext
	)
	maybeSkipArchive(env)
	if err := build.WriteArchive(gzrm, gzrmArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{gzrm, alltools} {
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "gzrm-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func newDebMetadata(distro, author string, env build.Environment, t time.Time) debMetadata {
	if author == "" {
		...
		author = "Hugo Teijiz <hteijiz@zerium.org>"
	}
	return debMetadata{
		Env:         env,
		Author:      author,
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "gzrm.exe" {
			gzrmTool = file
		} else {
	...
	// first section contains the gzrm binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"Gzrm":     gzrmTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.gzrm.nsi", filepath.Join(*workdir, "gzrm.nsi"), 0644, nil)
	...
	installer, _ := filepath.Abs("gzrm-" + archiveBasename(*arch, env) + ".exe")
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "gzrm.nsi"),
	)
...
func doAndroidArchive(cmdline []string) {
	...
	build.MustRun(gomobileTool("bind", "--target", "android", "--javapkg", "org.abt", "-v", "github.com/apolo-technologies/zerium/mobile"))

	if *local {
		// If we're building locally, copy bundle to build dir and skip Maven
		os.Rename("gzrm.aar", filepath.Join(GOBIN, "gzrm.aar"))
		return
	}
	...
	archive := "gzrm-" + archiveBasename("android", env) + ".aar"
	os.Rename("gzrm.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	archive := "gzrm-" + archiveBasename("ios", env)
	...
	if *deploy != "" {
		...
		build.Render("build/pod.podspec", "Gzrm.podspec", 0755, meta)
		build.MustRunCommand("pod", *deploy, "push", "Gzrm.podspec", "--allow-warnings", "--verbose")
	}
}
...
func doPurge(cmdline []string) {
	var (
		store = flag.String("store", "", `Destination from where to purge archives (usually "gzrmstore/builds")`)
