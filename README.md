...
Binary archives are published at https://zrmd.zerium.org/downloads/.
...
Building zrmd requires both a Go (version 1.7 or later) and a C compiler.
...
enumerated a few zrmcom parameter combos to get you up to speed quickly on how you can run your
own Geth instance.

### Full node on the main Zerium network

By far the most zrmcom scenario is people wanting to simply interact with the Zerium network:
create accounts; transfer funds; deploy and interact with contracts. For this particular use-case
the user doesn't care about years-old historical data, so we can fast-sync quickly to the current
state of the network. To do so:

```
$ zrmd --fast --cache=512 console
```

This command will:

 * Start zrmd in fast sync mode (`--fast`), causing it to download more data in exchange for avoiding
 ...
 * Start up Geth's built-in interactive [JavaScript console](https://github.com/apolo-technologies/zerium/wiki/JavaScript-Console),
   (via the trailing `console` subcommand) through which you can invoke all official [`abt17` methods](https://github.com/apolo-technologies/wiki/wiki/JavaScript-API)
...
```
$ zrmd --testnet --fast --cache=512 console
```
 ...
 * Instead of using the default data directory (`~/.zerium` on Linux for example), Geth will nest
   itself one level deeper into a `testnet` subfolder (`~/.zerium/testnet` on Linux). Note, on OSX
   and Linux this also means that attaching to a running testnet node requires the use of a custom
   endpoint since `zrmd attach` will try to attach to a production node endpoint by default. E.g.
   `zrmd attach <datadir>/testnet/zrmd.ipc`. Windows users are not affected by this.
...
As an alternative to passing the numerous flags to the `zrmd` binary, you can also pass a configuration file via:

```
$ zrmd --config /path/to/your_config.toml
```

To get an idea how the file should look like you can use the `dumpconfig` subcommand to export your existing configuration:

```
$ zrmd --your-favourite-flags dumpconfig
```

*Note: This works only with zrmd v1.6.0 and above.*
...
This will start zrmd in fast sync mode with a DB memory allowance of 512MB just as the above command does.  It will also create a persistent volume in your home directory for saving your blockchain as well as map the default ports. There is also an `alpine` tag available for a slim version of the image.

Do not forget `--rpcaddr 0.0.0.0`, if you want to access RPC from other containers and/or hosts. By default, `zrmd` binds to the local interface and RPC endpoints is not accessible from the outside.
...
HTTP based JSON-RPC API options:
  ...
  * `--rpcapi` API's offered over the HTTP-RPC interface (default: "eth,net,abt17")
  * `--rpccorsdomain` Comma separated list of domains from which to accept cross origin requests (browser enforced)
  * `--ws` Enable the WS-RPC server
  * `--wsaddr` WS-RPC server listening interface (default: "localhost")
  * `--wsport` WS-RPC server listening port (default: 8546)
  * `--wsapi` API's offered over the WS-RPC interface (default: "eth,net,abt17")
  * `--wsorigins` Origins from which to accept websockets requests
  * `--ipcdisable` Disable the IPC-RPC server
  * `--ipcapi` API's offered over the IPC-RPC interface (default: "admin,debug,eth,miner,net,personal,shh,txpool,abt17")
...
```
$ zrmd init path/to/genesis.json
```
...
```
$ zrmd --datadir=path/to/custom/data/folder --bootnodes=<bootnode-enode-url-from-above>
```
...
[EtherMining subreddit](https://www.reddit.com/r/zrmerMining/) and the [Genoil miner](https://github.com/Genoil/cpp-zerium)
repository.
...
```
$ zrmd <usual-flags> --mine --minerthreads=1 --etherbase=0x0000000000000000000000000000000000000000
```
