...
Binary archives are published at https://zaed.zerium.org/downloads/.
...
Building zaed requires both a Go (version 1.7 or later) and a C compiler.
...
| `evm` | Developer utility version of the EVM (Zerium Virtual Machine) that is capable of running bytecode snippets within a configurable environment and execution mode. Its purpose is to allow isolated, fine-grained debugging of EVM opcodes (e.g. `evm --code 60ff60ff --debug`). |
| `zaedrpctest` | Developer utility tool to support our [zerium/rpc-test](https://github.com/apolo-technologies/rpc-tests) test suite which validates baseline conformity to the [Zerium JSON RPC](https://github.com/apolo-technologies/wiki/wiki/JSON-RPC) specs. Please see the [test suite's readme](https://github.com/apolo-technologies/rpc-tests/blob/master/README.md) for details. |
...
```
$ zaed --fast --cache=512 zaeconsole
```

This command will:

 * Start zaed in fast sync mode (`--fast`), causing it to download more data in exchange for avoiding
 ...
 * Start up zaed's built-in interactive [JavaScript zaeconsole](https://github.com/apolo-technologies/zerium/wiki/JavaScript-Console),
   ...
   as well as zaed's own [management APIs](https://github.com/apolo-technologies/zerium/wiki/Management-APIs).
   This too is optional and if you leave it out you can always attach to an already running zaed instance
   with `zaed attach`.
...
```
$ zaed --testnet --fast --cache=512 zaeconsole
```
...
Specifying the `--testnet` flag however will reconfigure your zaed instance a bit:

 * Instead of using the default data directory (`~/.zerium` on Linux for example), zaed will nest
   itself one level deeper into a `testnet` subfolder (`~/.zerium/testnet` on Linux). Note, on OSX
   and Linux this also means that attaching to a running testnet node requires the use of a custom
   endpoint since `zaed attach` will try to attach to a production node endpoint by default. E.g.
   `zaed attach <datadir>/testnet/zaed.ipc`. Windows users are not affected by this.
...
for play-money and real-money. Unless you manually move accounts, zaed will by default correctly
separate the two networks and will not make any accounts available between them.*

### Configuration

As an alternative to passing the numerous flags to the `zaed` binary, you can also pass a configuration file via:

```
$ zaed --config /path/to/your_config.toml
```

To get an idea how the file should look like you can use the `dumpconfig` subcommand to export your existing configuration:

```
$ zaed --your-favourite-flags dumpconfig
```

*Note: This works only with zaed v1.6.0 and above.*
...
This will start zaed in fast sync mode with a DB memory allowance of 512MB just as the above command does.  It will also create a persistent volume in your home directory for saving your blockchain as well as map the default ports. There is also an `alpine` tag available for a slim version of the image.

Do not forget `--rpcaddr 0.0.0.0`, if you want to access RPC from other containers and/or hosts. By default, `zaed` binds to the local interface and RPC endpoints is not accessible from the outside.

### Programatically interfacing zaed nodes

As a developer, sooner rather than later you'll want to start interacting with zaed and the Zerium
network via your own programs and not manually through the zaeconsole. To aid this, zaed has built in
support for a JSON-RPC based APIs ([standard APIs](https://github.com/apolo-technologies/wiki/wiki/JSON-RPC) and
[zaed specific APIs](https://github.com/apolo-technologies/zerium/wiki/Management-APIs)). These can be
exposed via HTTP, WebSockets and IPC (unix sockets on unix based platforms, and named pipes on Windows).

The IPC interface is enabled by default and exposes all the APIs supported by zaed, whereas the HTTP
...
via HTTP, WS or IPC to a zaed node configured with the above flags and you'll need to speak [JSON-RPC](http://www.jsonrpc.org/specification)
...
With the genesis state defined in the above JSON file, you'll need to initialize **every** zaed node
with it prior to starting it up to ensure all blockchain parameters are correctly set:

```
$ zaed init path/to/genesis.json
```
...
*Note: You could also use a full fledged zaed node as a bootnode, but it's the less recommended way.*

#### Starting up your member nodes

With the bootnode operational and externally reachable (you can try `telnet <ip> <port>` to ensure
it's indeed reachable), start every subsequent zaed node pointed to the bootnode for peer discovery
via the `--bootnodes` flag. It will probably also be desirable to keep the data directory of your
private network separated, so do also specify a custom `--datadir` flag.

```
$ zaed --datadir=path/to/custom/data/folder --bootnodes=<bootnode-enode-url-from-above>
```
...
[EtherMining subreddit](https://www.reddit.com/r/EtherMining/) and the [Genoil miner](https://github.com/Genoil/cpp-zerium)
repository.

In a private network setting however, a single CPU miner instance is more than enough for practical
purposes as it can produce a stable stream of blocks at the correct intervals without needing heavy
resources (consider running on a single thread, no need for multiple ones either). To start a zaed
instance for mining, run it with all your usual flags, extended by:

```
$ zaed <usual-flags> --mine --minerthreads=1 --zeriumbase=0x0000000000000000000000000000000000000000
```

Which will start mining bocks and transactions on a single CPU thread, crediting all proceedings to
the account specified by `--zeriumbase`. You can further tune the mining by changing the default gas
