...
Binary archives are published at https://gabt.zerium.org/downloads/.
...
Building gabt requires both a Go (version 1.7 or later) and a C compiler.
 ...
 * Start gabt in fast sync mode (`--fast`), causing it to download more data in exchange for avoiding
...
Specifying the `--testnet` flag however will reconfigure your Gabt instance a bit:

 * Instead of using the default data directory (`~/.zerium` on Linux for example), Gabt will nest
   itself one level deeper into a `testnet` subfolder (`~/.zerium/testnet` on Linux). Note, on OSX
   and Linux this also means that attaching to a running testnet node requires the use of a custom
   endpoint since `gabt attach` will try to attach to a production node endpoint by default. E.g.
   `gabt attach <datadir>/testnet/gabt.ipc`. Windows users are not affected by this.
...
for play-money and real-money. Unless you manually move accounts, Gabt will by default correctly
separate the two networks and will not make any accounts available between them.*

### Configuration

As an alternative to passing the numerous flags to the `gabt` binary, you can also pass a configuration file via:

```
$ gabt --config /path/to/your_config.toml
```

To get an idea how the file should look like you can use the `dumpconfig` subcommand to export your existing configuration:

```
$ gabt --your-favourite-flags dumpconfig
```

*Note: This works only with gabt v1.6.0 and above.*
...
This will start gabt in fast sync mode with a DB memory allowance of 512MB just as the above command does.  It will also create a persistent volume in your home directory for saving your blockchain as well as map the default ports. There is also an `alpine` tag available for a slim version of the image.

Do not forget `--rpcaddr 0.0.0.0`, if you want to access RPC from other containers and/or hosts. By default, `gabt` binds to the local interface and RPC endpoints is not accessible from the outside.
...
The IPC interface is enabled by default and exposes all the APIs supported by Gabt, whereas the HTTP
...
via HTTP, WS or IPC to a Gabt node configured with the above flags and you'll need to speak [JSON-RPC](http://www.jsonrpc.org/specification)
...
With the genesis state defined in the above JSON file, you'll need to initialize **every** Gabt node
with it prior to starting it up to ensure all blockchain parameters are correctly set:

```
$ gabt init path/to/genesis.json
```
...
*Note: You could also use a full fledged Gabt node as a bootnode, but it's the less recommended way.*

#### Starting up your member nodes

With the bootnode operational and externally reachable (you can try `telnet <ip> <port>` to ensure
it's indeed reachable), start every subsequent Gabt node pointed to the bootnode for peer discovery
via the `--bootnodes` flag. It will probably also be desirable to keep the data directory of your
private network separated, so do also specify a custom `--datadir` flag.

```
$ gabt --datadir=path/to/custom/data/folder --bootnodes=<bootnode-enode-url-from-above>
```
...
resources (consider running on a single thread, no need for multiple ones either). To start a Gabt
instance for mining, run it with all your usual flags, extended by:

```
$ gabt <usual-flags> --mine --minerthreads=1 --zeriumbase=0x0000000000000000000000000000000000000000
```
