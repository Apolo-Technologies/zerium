...
func Keccak256Hash(data ...[]byte) (h zrmcom.Hash) {
	d := sha3.NewKeccak256()
	for _, b := range data {
		d.Write(b)
	}
...
// Creates an zerium address given the bytes and the nonce
func CreateAddress(b zrmcom.Address, nonce uint64) zrmcom.Address {
	data, _ := rlp.EncodeToBytes([]interface{}{b, nonce})
	return zrmcom.BytesToAddress(Keccak256(data)[12:])
}
...
func ValidateSignatureValues(v byte, r, s *big.Int, homestead bool) bool {
	if r.Cmp(zrmcom.Big1) < 0 || s.Cmp(zrmcom.Big1) < 0 {
		return false
	}
...
func PubkeyToAddress(p ecdsa.PublicKey) zrmcom.Address {
	pubBytes := FromECDSAPub(&p)
	return zrmcom.BytesToAddress(Keccak256(pubBytes[1:])[12:])
}
