...
type OdrRequest interface {
	StoreResult(db zrmdb.Database)
}

// TrieID identifies a state or account storage trie
type TrieID struct {
	BlockHash, Root zrmcom.Hash
	BlockNumber     uint64
	AccKey          []byte
}
...
func StorageTrieID(state *TrieID, addrHash, root zrmcom.Hash) *TrieID {
	return &TrieID{
...
func (req *TrieRequest) StoreResult(db zrmdb.Database) {
	req.Proof.Store(db)
}
...
// StoreResult stores the retrieved data in local database
func (req *CodeRequest) StoreResult(db zrmdb.Database) {
...
// StoreResult stores the retrieved data in local database
func (req *BlockRequest) StoreResult(db zrmdb.Database) {
...
// StoreResult stores the retrieved data in local database
func (req *ReceiptsRequest) StoreResult(db zrmdb.Database) {
...
// StoreResult stores the retrieved data in local database
func (req *ChtRequest) StoreResult(db zrmdb.Database) {
...
type BloomRequest struct {
	...
	BloomTrieRoot  zrmcom.Hash
	BloomBits      [][]byte
	Proofs         *NodeSet
}

// StoreResult stores the retrieved data in local database
func (req *BloomRequest) StoreResult(db zrmdb.Database) {
