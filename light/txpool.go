...
type TxPool struct {
	...
	head         zrmcom.Hash
	nonce        map[zrmcom.Address]uint64            // "pending" nonce
	pending      map[zrmcom.Hash]*types.Transaction   // pending transactions by tx hash
	mined        map[zrmcom.Hash][]*types.Transaction // mined transactions by block hash
...
type TxRelayBackend interface {
	...
	NewHead(head zrmcom.Hash, mined []zrmcom.Hash, rollback []zrmcom.Hash)
	Discard(hashes []zrmcom.Hash)
}
...
func NewTxPool(config *params.ChainConfig, chain *LightChain, relay TxRelayBackend) *TxPool {
	pool := &TxPool{
		...
		nonce:       make(map[zrmcom.Address]uint64),
		pending:     make(map[zrmcom.Hash]*types.Transaction),
		mined:       make(map[zrmcom.Hash][]*types.Transaction),
		quit:        make(chan bool),
...
func (pool *TxPool) GetNonce(ctx context.Context, addr zrmcom.Address) (uint64, error) {
...
type txStateChanges map[zrmcom.Hash]bool

// setState sets the status of a tx to either recently mined or recently rolled back
func (txc txStateChanges) setState(txHash zrmcom.Hash, mined bool) {
	val, ent := txc[txHash]
	if ent && (val != mined) {
		delete(txc, txHash)
	} else {
...
func (txc txStateChanges) getLists() (mined []zrmcom.Hash, rollback []zrmcom.Hash) {
...
func (pool *TxPool) checkMinedTxs(ctx context.Context, hash zrmcom.Hash, number uint64, txc txStateChanges) error {
...
func (pool *TxPool) rollbackTxs(hash zrmcom.Hash, txc txStateChanges) {
...
func (pool *TxPool) reorgOnNewHead(ctx context.Context, newHeader *types.Header) (txStateChanges, error) {
	...
	newh := newHeader
	// find zrmcom ancestor, create list of rolled back and new block hashes
	var oldHashes, newHashes []zrmcom.Hash
	...
	if idx := newHeader.Number.Uint64(); idx > pool.clearIdx+txPermanent {
		...
		if len(pool.mined) > 0 {
			for i := pool.clearIdx; i < idx2; i++ {
				...
				if list, ok := pool.mined[hash]; ok {
					hashes := make([]zrmcom.Hash, len(list))
					for i, tx := range list {
						hashes[i] = tx.Hash()
					}
...
func (self *TxPool) add(ctx context.Context, tx *types.Transaction) error {
	...
	log.Debug("Pooled new transaction", "hash", hash, "from", log.Lazy{Fn: func() zrmcom.Address { from, _ := types.Sender(self.signer, tx); return from }}, "to", tx.To())
	return nil
}
...
// and nil otherwise.
func (tp *TxPool) GetTransaction(hash zrmcom.Hash) *types.Transaction {
	// check the txs first
...
func (self *TxPool) Content() (map[zrmcom.Address]types.Transactions, map[zrmcom.Address]types.Transactions) {
	self.mu.RLock()
	defer self.mu.RUnlock()
...
func (pool *TxPool) RemoveTx(hash zrmcom.Hash) {
	...
	pool.relay.Discard([]zrmcom.Hash{hash})
}
