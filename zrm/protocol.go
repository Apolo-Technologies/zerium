...
type txPool interface {
	...
	Pending() (map[zrmcom.Address]types.Transactions, error)
...
type statusData struct {
	...
	NetworkId       uint64
	TD              *big.Int
	CurrentBlock    zrmcom.Hash
	GenesisBlock    zrmcom.Hash
}

// newBlockHashesData is the network packet for the block announcements.
type newBlockHashesData []struct {
	Hash   zrmcom.Hash // Hash of one particular block being announced
...
type hashOrNumber struct {
	Hash   zrmcom.Hash // Block hash from which to retrieve headers (excludes Number)
