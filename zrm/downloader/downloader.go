...
var (
	...
	maxQueuedHeaders  = 32 * 1024 // [zrm/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	// Channels
	headerCh      chan dataPack        // [zrm/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [zrm/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [zrm/63] Channel receiving inbound receipts
	bodyWakeCh    chan bool            // [zrm/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [zrm/63] Channel to signal the receipt fetcher of new tasks
	headerProcCh  chan []*types.Header // [zrm/62] Channel to feed the header processor new tasks

	// for stateFetcher
	stateSyncStart chan *stateSync
	trackStateReq  chan *stateReq
	stateCh        chan dataPack // [zrm/63] Channel receiving inbound node state data
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	log.Debug("Synchronising with the network", "peer", p.id, "zrm", p.version, "head", hash, "td", td, "mode", d.mode)
