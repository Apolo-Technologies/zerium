...
import (
	...
	zerium "github.com/apolo-technologies/zerium"
...
type Downloader struct {
	...
	// Status
	synchroniseMock func(id string, hash zrmcom.Hash) error // Replacement for synchronise during testing
	synchronising   int32
	notified        int32
...
type LightChain interface {
	...
	HasHeader(h zrmcom.Hash, number uint64) bool

	// GetHeaderByHash retrieves a header from the local chain.
	GetHeaderByHash(zrmcom.Hash) *types.Header

	// CurrentHeader retrieves the head header from the local chain.
	CurrentHeader() *types.Header

	// GetTdByHash returns the total difficulty of a local block.
	GetTdByHash(zrmcom.Hash) *big.Int

	// InsertHeaderChain inserts a batch of headers into the local chain.
	InsertHeaderChain([]*types.Header, int) (int, error)

	// Rollback removes a few recently added elements from the local chain.
	Rollback([]zrmcom.Hash)
}
...
type BlockChain interface {
	...
	HasBlockAndState(zrmcom.Hash) bool

	// GetBlockByHash retrieves a block from the local chain.
	GetBlockByHash(zrmcom.Hash) *types.Block
	...
	FastSyncCommitHead(zrmcom.Hash) error
...
func New(mode SyncMode, stateDb zrmdb.Database, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
...
// these are zero.
func (d *Downloader) Progress() zerium.SyncProgress {
...
func (d *Downloader) Synchronise(id string, head zrmcom.Hash, td *big.Int, mode SyncMode) error {
...
func (d *Downloader) synchronise(id string, hash zrmcom.Hash, td *big.Int, mode SyncMode) error {
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash zrmcom.Hash, td *big.Int) (err error) {
	...
	// Look up the sync boundaries: the zrmcom ancestor and the target block
...
// findAncestor tries to locate the zrmcom ancestor link of the local chain and
// a remote peers blockchain. In the general case when our node was in sync and
// on the correct chain, checking the top N links should already get us a match.
// In the rare scenario when we ended up on a long reorganisation (i.e. none of
// the head links match), we do a binary search to find the zrmcom ancestor.
func (d *Downloader) findAncestor(p *peerConnection, height uint64) (uint64, error) {
	// Figure out the valid ancestor range to prevent rewrite attacks
	floor, ceil := int64(-1), d.lightchain.CurrentHeader().Number.Uint64()

	p.log.Debug("Looking for zrmcom ancestor", "local", ceil, "remote", height)
	if d.mode == FullSync {
	...
	number, hash := uint64(0), zrmcom.Hash{}
	...
	for finished := false; !finished; {
		...
		case packet := <-d.headerCh:
			...
			// Check if a zrmcom ancestor was found
			finished = true
	...
	if !zrmcom.EmptyHash(hash) {
		if int64(number) <= floor {
			p.log.Warn("Ancestor below allowance", "number", number, "hash", hash, "allowance", floor)
			return 0, errInvalidAncestor
		}
		p.log.Debug("Found zrmcom ancestor", "number", number, "hash", hash)
		return number, nil
	}
	...
	p.log.Debug("Found zrmcom ancestor", "number", start, "hash", hash)
	return start, nil
}
...
func (d *Downloader) processHeaders(origin uint64, td *big.Int) error {
	...
	defer func() {
		if len(rollback) > 0 {
			...
			hashes := make([]zrmcom.Hash, len(rollback))
			for i, header := range rollback {
				hashes[i] = header.Hash()
			}
			lastHeader, lastFastBlock, lastBlock := d.lightchain.CurrentHeader().Number, zrmcom.Big0, zrmcom.Big0
			if d.mode != LightSync {
				lastFastBlock = d.blockchain.CurrentFastBlock().Number()
				lastBlock = d.blockchain.CurrentBlock().Number()
			}
			d.lightchain.Rollback(hashes)
			curFastBlock, curBlock := zrmcom.Big0, zrmcom.Big0
			if d.mode != LightSync {
	...
	for {
		...
		case headers := <-d.headerProcCh:
			...
			if len(headers) == 0 {
				...
				// L: Sync begins, and finds zrmcom ancestor at 11
