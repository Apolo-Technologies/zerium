...
type peerConnection struct {
	...
	lacking map[zrmcom.Hash]struct{} // Set of hashes not to request (didn't have previously)
...
type LightPeer interface {
	Head() (zrmcom.Hash, *big.Int)
	RequestHeadersByHash(zrmcom.Hash, int, int, bool) error
...
type Peer interface {
	LightPeer
	RequestBodies([]zrmcom.Hash) error
	RequestReceipts([]zrmcom.Hash) error
	RequestNodeData([]zrmcom.Hash) error
}
...
func (w *lightPeerWrapper) Head() (zrmcom.Hash, *big.Int) { return w.peer.Head() }
func (w *lightPeerWrapper) RequestHeadersByHash(h zrmcom.Hash, amount int, skip int, reverse bool) error {
	return w.peer.RequestHeadersByHash(h, amount, skip, reverse)
}
func (w *lightPeerWrapper) RequestHeadersByNumber(i uint64, amount int, skip int, reverse bool) error {
	return w.peer.RequestHeadersByNumber(i, amount, skip, reverse)
}
func (w *lightPeerWrapper) RequestBodies([]zrmcom.Hash) error {
	panic("RequestBodies not supported in light client mode sync")
}
func (w *lightPeerWrapper) RequestReceipts([]zrmcom.Hash) error {
	panic("RequestReceipts not supported in light client mode sync")
}
func (w *lightPeerWrapper) RequestNodeData([]zrmcom.Hash) error {
...
func newPeerConnection(id string, version int, peer Peer, logger log.Logger) *peerConnection {
	return &peerConnection{
		id:      id,
		lacking: make(map[zrmcom.Hash]struct{}),
...
func (p *peerConnection) Reset() {
	...
	p.lacking = make(map[zrmcom.Hash]struct{})
}
...
func (p *peerConnection) FetchBodies(request *fetchRequest) error {
	...
	hashes := make([]zrmcom.Hash, 0, len(request.Headers))
...
func (p *peerConnection) FetchNodeData(hashes []zrmcom.Hash) error {
	// Sanity check the protocol version
	if p.version < 63 {
...
func (p *peerConnection) MarkLacking(hash zrmcom.Hash) {
	p.lock.Lock()
	defer p.lock.Unlock()
...
func (p *peerConnection) Lacks(hash zrmcom.Hash) bool {
	p.lock.RLock()
	defer p.lock.RUnlock()
