...
type downloadTester struct {
	...
	stateDb zrmdb.Database // Database used by the tester for syncing from peers
	peerDb  zrmdb.Database // Database of the peers containing all data

	ownHashes   []zrmcom.Hash                  // Hash chain belonging to the tester
	ownHeaders  map[zrmcom.Hash]*types.Header  // Headers belonging to the tester
	ownBlocks   map[zrmcom.Hash]*types.Block   // Blocks belonging to the tester
	ownReceipts map[zrmcom.Hash]types.Receipts // Receipts belonging to the tester
	ownChainTd  map[zrmcom.Hash]*big.Int       // Total difficulties of the blocks in the local chain

	peerHashes   map[string][]zrmcom.Hash                  // Hash chain belonging to different test peers
	peerHeaders  map[string]map[zrmcom.Hash]*types.Header  // Headers belonging to different test peers
	peerBlocks   map[string]map[zrmcom.Hash]*types.Block   // Blocks belonging to different test peers
	peerReceipts map[string]map[zrmcom.Hash]types.Receipts // Receipts belonging to different test peers
	peerChainTds map[string]map[zrmcom.Hash]*big.Int       // Total difficulties of the blocks in the peer chains

	peerMissingStates map[string]map[zrmcom.Hash]bool // State entries that fast sync should not return
...
func newTester() *downloadTester {
	testdb, _ := zrmdb.NewMemDatabase()
	genesis := core.GenesisBlockForTesting(testdb, testAddress, big.NewInt(1000000000))

	tester := &downloadTester{
		genesis:           genesis,
		peerDb:            testdb,
		ownHashes:         []zrmcom.Hash{genesis.Hash()},
		ownHeaders:        map[zrmcom.Hash]*types.Header{genesis.Hash(): genesis.Header()},
		ownBlocks:         map[zrmcom.Hash]*types.Block{genesis.Hash(): genesis},
		ownReceipts:       map[zrmcom.Hash]types.Receipts{genesis.Hash(): nil},
		ownChainTd:        map[zrmcom.Hash]*big.Int{genesis.Hash(): genesis.Difficulty()},
		peerHashes:        make(map[string][]zrmcom.Hash),
		peerHeaders:       make(map[string]map[zrmcom.Hash]*types.Header),
		peerBlocks:        make(map[string]map[zrmcom.Hash]*types.Block),
		peerReceipts:      make(map[string]map[zrmcom.Hash]types.Receipts),
		peerChainTds:      make(map[string]map[zrmcom.Hash]*big.Int),
		peerMissingStates: make(map[string]map[zrmcom.Hash]bool),
	}
	tester.stateDb, _ = zrmdb.NewMemDatabase()
...
// reassembly.
func (dl *downloadTester) makeChain(n int, seed byte, parent *types.Block, parentReceipts types.Receipts, heavy bool) ([]zrmcom.Hash, map[zrmcom.Hash]*types.Header, map[zrmcom.Hash]*types.Block, map[zrmcom.Hash]types.Receipts) {
	// Generate the block chain
	blocks, receipts := core.GenerateChain(params.TestChainConfig, parent, dl.peerDb, n, func(i int, block *core.BlockGen) {
		block.SetCoinbase(zrmcom.Address{seed})
		...
		if parent == dl.genesis && i%3 == 0 {
			...
			tx, err := types.SignTx(types.NewTransaction(block.TxNonce(testAddress), zrmcom.Address{seed}, big.NewInt(1000), new(big.Int).SetUint64(params.TxGas), nil, nil), signer, testKey)
			if err != nil {
				panic(err)
			}
			block.AddTx(tx)
	...
	hashes := make([]zrmcom.Hash, n+1)
	hashes[len(hashes)-1] = parent.Hash()

	headerm := make(map[zrmcom.Hash]*types.Header, n+1)
	headerm[parent.Hash()] = parent.Header()

	blockm := make(map[zrmcom.Hash]*types.Block, n+1)
	blockm[parent.Hash()] = parent

	receiptm := make(map[zrmcom.Hash]types.Receipts, n+1)
...
// h2[:f] are different but have a zrmcom suffix of length n-f.
func (dl *downloadTester) makeChainFork(n, f int, parent *types.Block, parentReceipts types.Receipts, balanced bool) ([]zrmcom.Hash, []zrmcom.Hash, map[zrmcom.Hash]*types.Header, map[zrmcom.Hash]*types.Header, map[zrmcom.Hash]*types.Block, map[zrmcom.Hash]*types.Block, map[zrmcom.Hash]types.Receipts, map[zrmcom.Hash]types.Receipts) {
	// Create the zrmcom suffix
...
func (dl *downloadTester) HasHeader(hash zrmcom.Hash, number uint64) bool {
	return dl.GetHeaderByHash(hash) != nil
}

// HasBlockAndState checks if a block and associated state is present in the testers canonical chain.
func (dl *downloadTester) HasBlockAndState(hash zrmcom.Hash) bool {
...
func (dl *downloadTester) GetHeaderByHash(hash zrmcom.Hash) *types.Header {
	dl.lock.RLock()
...
func (dl *downloadTester) GetBlockByHash(hash zrmcom.Hash) *types.Block {
	dl.lock.RLock()
	defer dl.lock.RUnlock()
...
func (dl *downloadTester) FastSyncCommitHead(hash zrmcom.Hash) error {
...
func (dl *downloadTester) GetTdByHash(hash zrmcom.Hash) *big.Int {
	dl.lock.RLock()
	defer dl.lock.RUnlock()
...
func (dl *downloadTester) Rollback(hashes []zrmcom.Hash) {
	dl.lock.Lock()
	defer dl.lock.Unlock()
...
func (dl *downloadTester) newPeer(id string, version int, hashes []zrmcom.Hash, headers map[zrmcom.Hash]*types.Header, blocks map[zrmcom.Hash]*types.Block, receipts map[zrmcom.Hash]types.Receipts) error {
...
func (dl *downloadTester) newSlowPeer(id string, version int, hashes []zrmcom.Hash, headers map[zrmcom.Hash]*types.Header, blocks map[zrmcom.Hash]*types.Block, receipts map[zrmcom.Hash]types.Receipts, delay time.Duration) error {
	dl.lock.Lock()
	defer dl.lock.Unlock()
	...
	if err == nil {
		...
		dl.peerHashes[id] = make([]zrmcom.Hash, len(hashes))
		copy(dl.peerHashes[id], hashes)

		dl.peerHeaders[id] = make(map[zrmcom.Hash]*types.Header)
		dl.peerBlocks[id] = make(map[zrmcom.Hash]*types.Block)
		dl.peerReceipts[id] = make(map[zrmcom.Hash]types.Receipts)
		dl.peerChainTds[id] = make(map[zrmcom.Hash]*big.Int)
		dl.peerMissingStates[id] = make(map[zrmcom.Hash]bool)
...
func (dlp *downloadTesterPeer) Head() (zrmcom.Hash, *big.Int) {
	dlp.dl.lock.RLock()
...
func (dlp *downloadTesterPeer) RequestHeadersByHash(origin zrmcom.Hash, amount int, skip int, reverse bool) error {
...
func (dlp *downloadTesterPeer) RequestBodies(hashes []zrmcom.Hash) error {
	dlp.waitDelay()
...
func (dlp *downloadTesterPeer) RequestReceipts(hashes []zrmcom.Hash) error {
	dlp.waitDelay()
...
func (dlp *downloadTesterPeer) RequestNodeData(hashes []zrmcom.Hash) error {
	dlp.waitDelay()
...
func assertOwnForkedChain(t *testing.T, tester *downloadTester, zrmcom int, lengths []int) {
	...
	for _, length := range lengths[1:] {
		headers += length - zrmcom
		blocks += length - zrmcom

		minReceipts += length - zrmcom - fsMinFullBlocks - fsPivotInterval
		maxReceipts += length - zrmcom - fsMinFullBlocks
	}
	...
	if tester.downloader.mode == FastSync {
		var index int
		if pivot := int(tester.downloader.queue.fastSyncPivot); pivot < zrmcom {
			index = pivot
		} else {
...
// In this test zrmcom ancestor lookup should be short circuited and not require
// binary searching.
...
// this test zrmcom ancestor lookup should *not* be short circuited, and a full
...
func testForkedSync(t *testing.T, protocol int, mode SyncMode) {
	...
	zrmcom, fork := MaxHashFetch, 2*MaxHashFetch
	hashesA, hashesB, headersA, headersB, blocksA, blocksB, receiptsA, receiptsB := tester.makeChainFork(zrmcom+fork, fork, tester.genesis, nil, true)
	...
	assertOwnChain(t, tester, zrmcom+fork+1)

	// Synchronise with the second peer and make sure that fork is pulled too
	if err := tester.sync("fork B", nil, mode); err != nil {
		t.Fatalf("failed to synchronise blocks: %v", err)
	}
	assertOwnForkedChain(t, tester, zrmcom+1, []int{zrmcom + fork + 1, zrmcom + fork + 1})
}
...
func testHeavyForkedSync(t *testing.T, protocol int, mode SyncMode) {
	...
	// Create a long enough forked chain
	zrmcom, fork := MaxHashFetch, 4*MaxHashFetch
	hashesA, hashesB, headersA, headersB, blocksA, blocksB, receiptsA, receiptsB := tester.makeChainFork(zrmcom+fork, fork, tester.genesis, nil, false)
	...
	assertOwnForkedChain(t, tester, zrmcom+1, []int{zrmcom + fork + 1, zrmcom + fork/2 + 1})
}
...
func testBoundedForkedSync(t *testing.T, protocol int, mode SyncMode) {
	...
	// Create a long enough forked chain
	zrmcom, fork := 13, int(MaxForkAncestry+17)
	hashesA, hashesB, headersA, headersB, blocksA, blocksB, receiptsA, receiptsB := tester.makeChainFork(zrmcom+fork, fork, tester.genesis, nil, true)
...
func testHighTDStarvationAttack(t *testing.T, protocol int, mode SyncMode) {
	...
	tester.newPeer("attack", protocol, []zrmcom.Hash{hashes[0]}, headers, blocks, receipts)
...
func testBlockHeaderAttackerDropping(t *testing.T, protocol int) {
	...
	for i, tt := range tests {
		...
		if err := tester.newPeer(id, protocol, []zrmcom.Hash{tester.genesis.Hash()}, nil, nil, nil); err != nil {
		...
		tester.downloader.synchroniseMock = func(string, zrmcom.Hash) error { return tt.result }
...
func testForkedSyncProgress(t *testing.T, protocol int, mode SyncMode) {
	...
	<-starting
	if progress := tester.downloader.Progress(); progress.StartingBlock != uint64(zrmcom) || progress.CurrentBlock != uint64(len(hashesA)-1) || progress.HighestBlock != uint64(len(hashesB)-1) {
		t.Fatalf("Forking progress mismatch: have %v/%v/%v, want %v/%v/%v", progress.StartingBlock, progress.CurrentBlock, progress.HighestBlock, zrmcom, len(hashesA)-1, len(hashesB)-1)
	}
	progress <- struct{}{}
	pending.Wait()

	// Check final progress after successful sync
	if progress := tester.downloader.Progress(); progress.StartingBlock != uint64(zrmcom) || progress.CurrentBlock != uint64(len(hashesB)-1) || progress.HighestBlock != uint64(len(hashesB)-1) {
		t.Fatalf("Final progress mismatch: have %v/%v/%v, want %v/%v/%v", progress.StartingBlock, progress.CurrentBlock, progress.HighestBlock, zrmcom, len(hashesB)-1, len(hashesB)-1)
	}
}
...
func (ftp *floodingTestPeer) Head() (zrmcom.Hash, *big.Int) { return ftp.peer.Head() }
func (ftp *floodingTestPeer) RequestHeadersByHash(hash zrmcom.Hash, count int, skip int, reverse bool) error {
	return ftp.peer.RequestHeadersByHash(hash, count, skip, reverse)
}
func (ftp *floodingTestPeer) RequestBodies(hashes []zrmcom.Hash) error {
	return ftp.peer.RequestBodies(hashes)
}
func (ftp *floodingTestPeer) RequestReceipts(hashes []zrmcom.Hash) error {
	return ftp.peer.RequestReceipts(hashes)
}
func (ftp *floodingTestPeer) RequestNodeData(hashes []zrmcom.Hash) error {
...
func testFastCriticalRestarts(t *testing.T, protocol int, progress bool) {
	...
	for i := 0; i < int(fsCriticalTrials)-1; i++ {
		...
		if i == 0 {
			...
			tester.peerMissingStates["peer"] = map[zrmcom.Hash]bool{tester.downloader.fsPivotLock.Root: true}
	...
	if progress {
		tester.lock.Lock()
		tester.peerMissingStates["peer"] = map[zrmcom.Hash]bool{}
		tester.lock.Unlock()
