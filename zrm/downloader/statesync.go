...
// stateReq represents a batch of state fetch requests groupped tozrmder into
// a single data retrieval network packet.
type stateReq struct {
	items    []zrmcom.Hash              // Hashes of the state items to download
	tasks    map[zrmcom.Hash]*stateTask // Download tasks to track previous attempts
...
func (d *Downloader) syncState(root zrmcom.Hash) *stateSync {
...
type stateSync struct {
	...
	tasks  map[zrmcom.Hash]*stateTask // Set of tasks currently queued for retrieval
...
func newStateSync(d *Downloader, root zrmcom.Hash) *stateSync {
	return &stateSync{
		d:       d,
		sched:   state.NewStateSync(root, d.stateDB),
		keccak:  sha3.NewKeccak256(),
		tasks:   make(map[zrmcom.Hash]*stateTask),
...
func (s *stateSync) fillTasks(n int, req *stateReq) {
	...
	req.items = make([]zrmcom.Hash, 0, n)
	req.tasks = make(map[zrmcom.Hash]*stateTask, n)
...
// error occurred.
func (s *stateSync) processNodeData(blob []byte) (bool, zrmcom.Hash, error) {
...
func (s *stateSync) updateStats(written, duplicate, unexpected int, duration time.Duration) {
	...
	if written > 0 || duplicate > 0 || unexpected > 0 {
		log.Info("Imported new state entries", "count", written, "elapsed", zrmcom.PrettyDuration(duration), "processed", s.d.syncStatsState.processed, "pending", s.d.syncStatsState.pending, "retry", len(s.tasks), "duplicate", s.d.syncStatsState.duplicate, "unexpected", s.d.syncStatsState.unexpected)
	}
}
