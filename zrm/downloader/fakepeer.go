...
func NewFakePeer(id string, db zrmdb.Database, hc *core.HeaderChain, dl *Downloader) *FakePeer {
...
func (p *FakePeer) Head() (zrmcom.Hash, *big.Int) {
...
func (p *FakePeer) RequestHeadersByHash(hash zrmcom.Hash, amount int, skip int, reverse bool) error {
	var (
...
func (p *FakePeer) RequestBodies(hashes []zrmcom.Hash) error {
	var (
...
func (p *FakePeer) RequestReceipts(hashes []zrmcom.Hash) error {
...
func (p *FakePeer) RequestNodeData(hashes []zrmcom.Hash) error {
	var data [][]byte
	for _, hash := range hashes {
