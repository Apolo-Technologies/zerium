...
type fetchRequest struct {
	...
	Hashes  map[zrmcom.Hash]int // [eth/61] Requested hashes with their insertion index (priority)
...
type queue struct {
	...
	headerHead zrmcom.Hash // [eth/62] Hash of the last queued header to verify order
	...
	blockTaskPool  map[zrmcom.Hash]*types.Header // [eth/62] Pending block (body) retrieval tasks, mapping hashes to headers
	blockTaskQueue *prque.Prque                  // [eth/62] Priority queue of the headers to fetch the blocks (bodies) for
	blockPendPool  map[string]*fetchRequest      // [eth/62] Currently pending block (body) retrieval operations
	blockDonePool  map[zrmcom.Hash]struct{}      // [eth/62] Set of the completed block (body) fetches

	receiptTaskPool  map[zrmcom.Hash]*types.Header // [eth/63] Pending receipt retrieval tasks, mapping hashes to headers
	receiptTaskQueue *prque.Prque                  // [eth/63] Priority queue of the headers to fetch the receipts for
	receiptPendPool  map[string]*fetchRequest      // [eth/63] Currently pending receipt retrieval operations
	receiptDonePool  map[zrmcom.Hash]struct{}      // [eth/63] Set of the completed receipt fetches
...
func newQueue() *queue {
	...
	return &queue{
		...
		blockTaskPool:    make(map[zrmcom.Hash]*types.Header),
		blockTaskQueue:   prque.New(),
		blockPendPool:    make(map[string]*fetchRequest),
		blockDonePool:    make(map[zrmcom.Hash]struct{}),
		receiptTaskPool:  make(map[zrmcom.Hash]*types.Header),
		receiptTaskQueue: prque.New(),
		receiptPendPool:  make(map[string]*fetchRequest),
		receiptDonePool:  make(map[zrmcom.Hash]struct{}),
...
func (q *queue) Reset() {
	...
	q.headerHead = zrmcom.Hash{}

	q.headerPendPool = make(map[string]*fetchRequest)

	q.blockTaskPool = make(map[zrmcom.Hash]*types.Header)
	q.blockTaskQueue.Reset()
	q.blockPendPool = make(map[string]*fetchRequest)
	q.blockDonePool = make(map[zrmcom.Hash]struct{})

	q.receiptTaskPool = make(map[zrmcom.Hash]*types.Header)
	q.receiptTaskQueue.Reset()
	q.receiptPendPool = make(map[string]*fetchRequest)
	q.receiptDonePool = make(map[zrmcom.Hash]struct{})
...
func (q *queue) Schedule(headers []*types.Header, from uint64) []*types.Header {
	...
	for _, header := range headers {
		...
		if q.headerHead != (zrmcom.Hash{}) && q.headerHead != header.ParentHash {
...
func (q *queue) reserveHeaders(p *peerConnection, count int, taskPool map[zrmcom.Hash]*types.Header, taskQueue *prque.Prque,
	pendPool map[string]*fetchRequest, donePool map[zrmcom.Hash]struct{}, isNoop func(*types.Header) bool) (*fetchRequest, bool, error) {
	...
	for proc := 0; proc < space && len(send) < count && !taskQueue.Empty(); proc++ {
		...
		if index >= len(q.resultCache) || index < 0 {
			zrmcom.Report("index allocation went beyond available resultCache space")
...
// reason the lock is not obtained in here is because the parameters already need
// to access the queue, so they already need a lock anyway.
func (q *queue) deliver(id string, taskPool map[zrmcom.Hash]*types.Header, taskQueue *prque.Prque,
	pendPool map[string]*fetchRequest, donePool map[zrmcom.Hash]struct{}, reqTimer metrics.Timer,
