...
// reassembly.
func makeChain(n int, seed byte, parent *types.Block) ([]zrmcom.Hash, map[zrmcom.Hash]*types.Block) {
...
type fetcherTester struct {
	...
	hashes []zrmcom.Hash                // Hash chain belonging to the tester
	blocks map[zrmcom.Hash]*types.Block // Blocks belonging to the tester
...
func newTester() *fetcherTester {
	tester := &fetcherTester{
		hashes: []zrmcom.Hash{genesis.Hash()},
		blocks: map[zrmcom.Hash]*types.Block{genesis.Hash(): genesis},
...
func (f *fetcherTester) getBlock(hash zrmcom.Hash) *types.Block {
	f.lock.RLock()
...
func (f *fetcherTester) makeHeaderFetcher(peer string, blocks map[zrmcom.Hash]*types.Block, drift time.Duration) headerRequesterFn {
	closure := make(map[zrmcom.Hash]*types.Block)
	for hash, block := range blocks {
		closure[hash] = block
	}
	// Create a function that return a header from the closure
	return func(hash zrmcom.Hash) error {
...
func (f *fetcherTester) makeBodyFetcher(peer string, blocks map[zrmcom.Hash]*types.Block, drift time.Duration) bodyRequesterFn {
	closure := make(map[zrmcom.Hash]*types.Block)
	for hash, block := range blocks {
		closure[hash] = block
	}
	// Create a function that returns blocks from the closure
	return func(hashes []zrmcom.Hash) error {
...
func verifyFetchingEvent(t *testing.T, fetching chan []zrmcom.Hash, arrive bool) {
	if arrive {
		select {
		case <-fetching:
...
func verifyCompletingEvent(t *testing.T, completing chan []zrmcom.Hash, arrive bool) {
	if arrive {
		select {
		case <-completing:
		case <-time.After(time.Second):
...
func testConcurrentAnnouncements(t *testing.T, protocol int) {
	...
	counter := uint32(0)
	firstHeaderWrapper := func(hash zrmcom.Hash) error {
		atomic.AddUint32(&counter, 1)
		return firstHeaderFetcher(hash)
	}
	secondHeaderWrapper := func(hash zrmcom.Hash) error {
		atomic.AddUint32(&counter, 1)
...
func testPendingDeduplication(t *testing.T, protocol int) {
	...
	counter := uint32(0)
	headerWrapper := func(hash zrmcom.Hash) error {
		atomic.AddUint32(&counter, 1)
...
func testImportDeduplication(t *testing.T, protocol int) {
	...
	fetching := make(chan []zrmcom.Hash)
	imported := make(chan *types.Block, len(hashes)-1)
	tester.fetcher.fetchingHook = func(hashes []zrmcom.Hash) { fetching <- hashes }
	tester.fetcher.importedHook = func(block *types.Block) { imported <- block }
...
func TestDistantPropagationDiscarding(t *testing.T) {
	...
	tester.lock.Lock()
	tester.hashes = []zrmcom.Hash{head}
	tester.blocks = map[zrmcom.Hash]*types.Block{head: blocks[head]}
	tester.lock.Unlock()
...
func testDistantAnnouncementDiscarding(t *testing.T, protocol int) {
	...
	tester.fetcher.fetchingHook = func(hashes []zrmcom.Hash) { fetching <- struct{}{} }
...
func testEmptyBlockShortCircuit(t *testing.T, protocol int) {
	...
	completing := make(chan []zrmcom.Hash)
	tester.fetcher.completingHook = func(hashes []zrmcom.Hash) { completing <- hashes }
...
func testHashMemoryExhaustionAttack(t *testing.T, protocol int) {
	...
	tester.fetcher.importedHook = func(block *types.Block) { imported <- block }
	tester.fetcher.announceChangeHook = func(hash zrmcom.Hash, added bool) {
		if added {
...
func TestBlockMemoryExhaustionAttack(t *testing.T) {
	...
	tester.fetcher.importedHook = func(block *types.Block) { imported <- block }
	tester.fetcher.queueChangeHook = func(hash zrmcom.Hash, added bool) {
		if added {
	...
	targetBlocks := hashLimit + 2*maxQueueDist
	hashes, blocks := makeChain(targetBlocks, 0, genesis)
	attack := make(map[zrmcom.Hash]*types.Block)
