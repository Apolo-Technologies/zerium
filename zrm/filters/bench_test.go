...
func benchmarkBloomBits(b *testing.B, sectionSize uint64) {
	benchDataDir := node.DefaultDataDir() + "/zrmd/chaindata"
	fmt.Println("Running bloombits benchmark   section size:", sectionSize)

	db, err := zrmdb.NewLDBDatabase(benchDataDir, 128, 1024)
	if err != nil {
	...
	for i := 0; i < benchFilterCnt; i++ {
		if i%20 == 0 {
			db.Close()
			db, _ = zrmdb.NewLDBDatabase(benchDataDir, 128, 1024)
			backend = &testBackend{mux, db, cnt, new(event.Feed), new(event.Feed), new(event.Feed), new(event.Feed)}
		}
		var addr zrmcom.Address
		addr[0] = byte(i)
		addr[1] = byte(i / 256)
		filter := New(backend, 0, int64(cnt*sectionSize-1), []zrmcom.Address{addr}, nil)
...
func forEachKey(db zrmdb.Database, startPrefix, endPrefix []byte, fn func(key []byte)) {
	it := db.(*zrmdb.LDBDatabase).NewIterator()
	it.Seek(startPrefix)
	for it.Valid() {
		key := it.Key()
		cmpLen := len(key)
...
func clearBloomBits(db zrmdb.Database) {
...
func BenchmarkNoBloomBits(b *testing.B) {
	...
	filter := New(backend, 0, int64(headNum), []zrmcom.Address{{}}, nil)
