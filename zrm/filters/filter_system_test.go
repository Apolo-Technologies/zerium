...
// TestPendingTxFilter tests whzerium pending tx filters retrieve all pending transactions that are posted to the event mux.
...
// TestLogFilterCreation test whzerium a given filter criteria makes sense.
...
// TestInvalidLogFilterCreation tests whzerium invalid filter log criteria results in an error
...
// TestLogFilter tests whzerium log filters match the correct logs that are posted to the event feed.
