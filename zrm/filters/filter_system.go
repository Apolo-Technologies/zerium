...
// Package filters implements an zerium filtering system for block,
...
// transactions that enter the transaction pool.
func (es *EventSystem) SubscribePendingTxEvents(hashes chan zrmcom.Hash) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) lightFilterLogs(header *types.Header, addresses []zrmcom.Address, topics [][]zrmcom.Hash, remove bool) []*types.Log {
