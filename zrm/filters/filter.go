...
func New(backend Backend, begin, end int64, addresses []zrmcom.Address, topics [][]zrmcom.Hash) *Filter {
...
func includes(addresses []zrmcom.Address, a zrmcom.Address) bool {
...
func filterLogs(logs []*types.Log, fromBlock, toBlock *big.Int, addresses []zrmcom.Address, topics [][]zrmcom.Hash) []*types.Log {
	var ret []*types.Log
Logs:
...
func bloomFilter(bloom types.Bloom, addresses []zrmcom.Address, topics [][]zrmcom.Hash) bool {
	if len(addresses) > 0 {
		var included bool
		for _, addr := range addresses {
