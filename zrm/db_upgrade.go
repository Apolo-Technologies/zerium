...
// been safely stopped.
func upgradeDeduplicateData(db zrmdb.Database) func() error {
	...
	go func() {
		...
		for failed == nil && it.Next() {
			...
			key := it.Key()
			if len(key) != zrmcom.HashLength+1 || key[zrmcom.HashLength] != 0x01 {
				continue
			}
			...
			hash := key[:zrmcom.HashLength]

			if hash[0] == byte('l') {
				// Potential clash, the "old" `hash` must point to a live transaction.
				if tx, _, _, _ := core.GetTransaction(db, zrmcom.BytesToHash(hash)); tx == nil || !bytes.Equal(tx.Hash().Bytes(), hash) {
					continue
				}
			}
