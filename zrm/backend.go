...
// Package eth implements the Zerium protocol.
package zrm
...
type LesServer interface {
	Start(srvr *peertopeer.Server)
	Stop()
	Protocols() []peertopeer.Protocol
...
// initialisation of the zrmcom Zerium object)
func New(ctx *node.ServiceContext, config *Config) (*Zerium, error) {
	if config.SyncMode == downloader.LightSync {
		return nil, errors.New("can't run eth.Zerium in light sync mode, use les.LightZerium")
	}
	...
	if !config.SkipBcVersionCheck {
		...
		if bcVersion != core.BlockChainVersion && bcVersion != 0 {
			return nil, fmt.Errorf("Blockchain DB version mismatch (%d / %d). Run zrmd upgradedb.\n", bcVersion, core.BlockChainVersion)
		}
...
func CreateConsensusEngine(ctx *node.ServiceContext, config *ethash.Config, chainConfig *params.ChainConfig, db zrmdb.Database) consensus.Engine {
...
func (s *Zerium) Etherbase() (eb zrmcom.Address, err error) {
	s.lock.RLock()
	etherbase := s.etherbase
	s.lock.RUnlock()

	if etherbase != (zrmcom.Address{}) {
	...
	return zrmcom.Address{}, fmt.Errorf("etherbase must be explicitly specified")
}

// set in js console via admin interface or wrapper from cli flags
func (self *Zerium) SetEtherbase(etherbase zrmcom.Address) {
	self.lock.Lock()
...
func (s *Zerium) Version() int                    { return int(s.protocolManager.SubProtocols[0].Version) }
...
func (s *Zerium) Protocols() []peertopeer.Protocol {
	if s.lesServer == nil {
...
func (s *Zerium) Start(srvr *peertopeer.Server) error {
