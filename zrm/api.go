...
// PublicZeriumAPI provides an API to access Zerium full node-related
// information.
type PublicZeriumAPI struct {
	e *Zerium
}

// NewPublicZeriumAPI creates a new Zerium protocol API for full nodes.
func NewPublicZeriumAPI(e *Zerium) *PublicZeriumAPI {
	return &PublicZeriumAPI{e}
}

// Etherbase is the address that mining rewards will be send to
func (api *PublicZeriumAPI) Etherbase() (zrmcom.Address, error) {
	return api.e.Etherbase()
}

// Coinbase is the address that mining rewards will be send to (alias for Etherbase)
func (api *PublicZeriumAPI) Coinbase() (zrmcom.Address, error) {
	return api.Etherbase()
}

// Hashrate returns the POW hashrate
func (api *PublicZeriumAPI) Hashrate() hexutil.Uint64 {
...
func (api *PublicMinerAPI) SubmitWork(nonce types.BlockNonce, solution, digest zrmcom.Hash) bool {
...
func (api *PublicMinerAPI) SubmitHashrate(hashrate hexutil.Uint64, id zrmcom.Hash) bool {
...
func (api *PrivateMinerAPI) SetEtherbase(etherbase zrmcom.Address) bool {
...
// PrivateAdminAPI is the collection of Zerium full node-related APIs
...
// admin methods of the Zerium service.
func NewPrivateAdminAPI(eth *Zerium) *PrivateAdminAPI {
...
// PublicDebugAPI is the collection of Zerium full node APIs exposed
...
// related public debug methods of the Zerium service.
func NewPublicDebugAPI(eth *Zerium) *PublicDebugAPI {
...
// PrivateDebugAPI is the collection of Zerium full node APIs exposed over
...
// private debug methods of the Zerium service.
func NewPrivateDebugAPI(config *params.ChainConfig, eth *Zerium) *PrivateDebugAPI {
...
func (api *PrivateDebugAPI) TraceBlockByHash(hash zrmcom.Hash, config *vm.LogConfig) BlockTraceResult {
	// Fetch the block that we aim to reprocess
...
func (api *PrivateDebugAPI) TraceTransaction(ctx context.Context, txHash zrmcom.Hash, config *TraceArgs) (interface{}, error) {
	...
	vmenv := vm.NewZvm(context, statedb, api.config, vm.Config{Debug: true, Tracer: tracer})
...
func (api *PrivateDebugAPI) computeTxEnv(blockHash zrmcom.Hash, txIndex int) (core.Message, vm.Context, *state.StateDB, error) {
	...
	for idx, tx := range txs {
		...
		context := core.NewZvmContext(msg, block.Header(), api.eth.BlockChain(), nil)
		if idx == txIndex {
			return msg, context, statedb, nil
		}

		vmenv := vm.NewZvm(context, statedb, api.config, vm.Config{})
...
func (api *PrivateDebugAPI) Preimage(ctx context.Context, hash zrmcom.Hash) (hexutil.Bytes, error) {
...
type StorageRangeResult struct {
	...
	NextKey *zrmcom.Hash `json:"nextKey"` // nil if Storage includes the last key in the trie.
}

type storageMap map[zrmcom.Hash]storageEntry

type storageEntry struct {
	Key   *zrmcom.Hash `json:"key"`
	Value zrmcom.Hash  `json:"value"`
}

// StorageRangeAt returns the storage at the given block height and transaction index.
func (api *PrivateDebugAPI) StorageRangeAt(ctx context.Context, blockHash zrmcom.Hash, txIndex int, contractAddress zrmcom.Address, keyStart hexutil.Bytes, maxResult int) (StorageRangeResult, error) {
...
func storageRangeAt(st state.Trie, start []byte, maxResult int) (StorageRangeResult, error) {
	...
	for i := 0; i < maxResult && it.Next(); i++ {
		...
		e := storageEntry{Value: zrmcom.BytesToHash(content)}
		if preimage := st.GetKey(it.Key); preimage != nil {
			preimage := zrmcom.BytesToHash(preimage)
			e.Key = &preimage
		}
		result.Storage[zrmcom.BytesToHash(it.Key)] = e
	}
	// Add the 'next key' so clients can continue downloading.
	if it.Next() {
		next := zrmcom.BytesToHash(it.Key)
		result.NextKey = &next
	}
	return result, nil
}
...
func (api *PrivateDebugAPI) GetModifiedAccountsByNumber(startNum uint64, endNum *uint64) ([]zrmcom.Address, error) {
...
//
// With one parameter, returns the list of accounts modified in the specified block.
func (api *PrivateDebugAPI) GetModifiedAccountsByHash(startHash zrmcom.Hash, endHash *zrmcom.Hash) ([]zrmcom.Address, error) {
	var startBlock, endBlock *types.Block
...
func (api *PrivateDebugAPI) getModifiedAccounts(startBlock, endBlock *types.Block) ([]zrmcom.Address, error) {
	...
	var dirty []zrmcom.Address
	for iter.Next() {
		key := newTrie.GetKey(iter.Key)
		if key == nil {
			return nil, fmt.Errorf("no preimage found for hash %x", iter.Key)
		}
		dirty = append(dirty, zrmcom.BytesToAddress(key))
	}
	return dirty, nil
}
