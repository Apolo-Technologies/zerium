...
// swarm.
//
// Internally this backend uses the already exposed API endpoints of the Zerium
...
func (b *ContractBackend) CodeAt(ctx context.Context, contract zrmcom.Address, blockNum *big.Int) ([]byte, error) {
	return b.bcapi.GetCode(ctx, contract, toBlockNumber(blockNum))
}

// CodeAt retrieves any code associated with the contract from the local API.
func (b *ContractBackend) PendingCodeAt(ctx context.Context, contract zrmcom.Address) ([]byte, error) {
	return b.bcapi.GetCode(ctx, contract, rpc.PendingBlockNumber)
}

// ContractCall implements bind.ContractCaller executing an Zerium contract
// call with the specified data as the input. The pending flag requests execution
// against the pending block, not the stable head of the chain.
func (b *ContractBackend) CallContract(ctx context.Context, msg zerium.CallMsg, blockNum *big.Int) ([]byte, error) {
...
// call with the specified data as the input. The pending flag requests execution
// against the pending block, not the stable head of the chain.
func (b *ContractBackend) PendingCallContract(ctx context.Context, msg zerium.CallMsg) ([]byte, error) {
	out, err := b.bcapi.Call(ctx, toCallArgs(msg), rpc.PendingBlockNumber)
	return out, err
}

func toCallArgs(msg zerium.CallMsg) ethapi.CallArgs {
...
func (b *ContractBackend) PendingNonceAt(ctx context.Context, account zrmcom.Address) (nonce uint64, err error) {
...
func (b *ContractBackend) EstimateGas(ctx context.Context, msg zerium.CallMsg) (*big.Int, error) {
