...
func testStatusMsgErrors(t *testing.T, protocol int) {
	...
	tests := []struct {
		...
		{
			code: StatusMsg, data: statusData{uint32(protocol), DefaultConfig.NetworkId, td, currentBlock, zrmcom.Hash{3}},
	...
	for i, test := range tests {
		...
		go peertopeer.Send(p.app, test.code, test.data)
...
func testRecvTransactions(t *testing.T, protocol int) {
	...
	if err := peertopeer.Send(p.app, TxMsg, []interface{}{tx}); err != nil {
...
func testSendTransactions(t *testing.T, protocol int) {
	...
	checktxs := func(p *testPeer) {
		defer wg.Done()
		defer p.close()
		seen := make(map[zrmcom.Hash]bool)
