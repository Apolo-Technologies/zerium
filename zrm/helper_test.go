...
func newTestProtocolManager(mode downloader.SyncMode, blocks int, generator func(int, *core.BlockGen), newtx chan<- []*types.Transaction) (*ProtocolManager, error) {
	...
	pm, err := NewProtocolManager(gspec.Config, mode, DefaultConfig.NetworkId, zvmux, &testTxPool{added: newtx}, engine, blockchain, db)
	if err != nil {
		return nil, err
	}
	pm.Start(1000)
...
func (p *testTxPool) Pending() (map[zrmcom.Address]types.Transactions, error) {
	p.lock.RLock()
	defer p.lock.RUnlock()

	batches := make(map[zrmcom.Address]types.Transactions)
...
func newTestTransaction(from *ecdsa.PrivateKey, nonce uint64, datasize int) *types.Transaction {
	tx := types.NewTransaction(nonce, zrmcom.Address{}, big.NewInt(0), big.NewInt(100000), big.NewInt(0), make([]byte, datasize))
...
func newTestPeer(name string, version int, pm *ProtocolManager, shake bool) (*testPeer, <-chan error) {
	...
	peer := pm.newPeer(version, peertopeer.NewPeer(id, name, nil), net)
...
func (p *testPeer) handshake(t *testing.T, td *big.Int, head zrmcom.Hash, genesis zrmcom.Hash) {
	msg := &statusData{
	...
	if err := peertopeer.ExpectMsg(p.app, StatusMsg, msg); err != nil {
		t.Fatalf("status recv: %v", err)
	}
	if err := peertopeer.Send(p.app, StatusMsg, msg); err != nil {
