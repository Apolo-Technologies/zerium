...
func testGetBlockHeaders(t *testing.T, protocol int) {
	...
	tests := []struct {
		...
		expect []zrmcom.Hash        // The hashes of the block whose headers are expected
	}{
		// A single random block should be retrievable by hash and number too
		{
			&getBlockHeadersData{Origin: hashOrNumber{Hash: pm.blockchain.GetBlockByNumber(limit / 2).Hash()}, Amount: 1},
			[]zrmcom.Hash{pm.blockchain.GetBlockByNumber(limit / 2).Hash()},
		}, {
		...
		// The chain endpoints should be retrievable
		{
			...
			[]zrmcom.Hash{pm.blockchain.GetBlockByNumber(0).Hash()},
		}, {
			&getBlockHeadersData{Origin: hashOrNumber{Number: pm.blockchain.CurrentBlock().NumberU64()}, Amount: 1},
			[]zrmcom.Hash{pm.blockchain.CurrentBlock().Hash()},
		},
	...
	for i, tt := range tests {
		...
		peertopeer.Send(peer.app, 0x03, tt.query)
		if err := peertopeer.ExpectMsg(peer.app, 0x04, headers); err != nil {
			t.Errorf("test %d: headers mismatch: %v", i, err)
		}
		// If the test used number origins, repeat with hashes as the too
		if tt.query.Origin.Hash == (zrmcom.Hash{}) {
...
func testGetBlockBodies(t *testing.T, protocol int) {
	...
	tests := []struct {
		...
		explicit  []zrmcom.Hash // Explicitly requested blocks
		...
		{0, []zrmcom.Hash{pm.blockchain.Genesis().Hash()}, []bool{true}, 1},      // The genesis block should be retrievable
		{0, []zrmcom.Hash{pm.blockchain.CurrentBlock().Hash()}, []bool{true}, 1}, // The chains head block should be retrievable
		{0, []zrmcom.Hash{{}}, []bool{false}, 0},                                 // A non existent block should not be returned
	...
	for i, tt := range tests {
		...
		hashes, seen := []zrmcom.Hash{}, make(map[int64]bool)
		bodies := []*blockBody{}
		...
		// Send the hash request and verify the response
		peertopeer.Send(peer.app, 0x05, hashes)
		if err := peertopeer.ExpectMsg(peer.app, 0x06, bodies); err != nil {
...
func testGetNodeData(t *testing.T, protocol int) {
	...
	hashes := []zrmcom.Hash{}
	for _, key := range pm.chaindb.(*zrmdb.MemDatabase).Keys() {
		if len(key) == len(zrmcom.Hash{}) {
			hashes = append(hashes, zrmcom.BytesToHash(key))
		}
	}
	peertopeer.Send(peer.app, 0x0d, hashes)
	...
	statedb, _ := zrmdb.NewMemDatabase()
	for i := 0; i < len(data); i++ {
		statedb.Put(hashes[i].Bytes(), data[i])
	}
	accounts := []zrmcom.Address{testBank, acc1Addr, acc2Addr}
...
func testGetReceipt(t *testing.T, protocol int) {
	...
	// Collect the hashes to request, and the response to expect
	hashes, receipts := []zrmcom.Hash{}, []types.Receipts{}
	for i := uint64(0); i <= pm.blockchain.CurrentBlock().NumberU64(); i++ {
	...
	// Send the hash request and verify the response
	peertopeer.Send(peer.app, 0x0f, hashes)
	if err := peertopeer.ExpectMsg(peer.app, 0x10, receipts); err != nil {
...
func testDAOChallenge(t *testing.T, localForked, remoteForked bool, timeout bool) {
	...
	pm, err := NewProtocolManager(config, downloader.FullSync, DefaultConfig.NetworkId, zvmux, new(testTxPool), pow, blockchain, db)
	if err != nil {
	...
	if err := peertopeer.ExpectMsg(peer.app, GetBlockHeadersMsg, challenge); err != nil {
	...
	if !timeout {
		...
		if err := peertopeer.Send(peer.app, BlockHeadersMsg, []*types.Header{blocks[0].Header()}); err != nil {
