...
func NewHeaderFromRLP(data []byte) (*Header, error) {
	...
	if err := rlp.DecodeBytes(zrmcom.CopyBytes(data), h.header); err != nil {
		return nil, err
	}
	return h, nil
}
...
func NewBlockFromRLP(data []byte) (*Block, error) {
	...
	if err := rlp.DecodeBytes(zrmcom.CopyBytes(data), b.block); err != nil {
		return nil, err
	}
	return b, nil
}
...
// Transaction represents a single Zerium transaction.
...
func NewTransaction(nonce int64, to *Address, amount, gasLimit, gasPrice *BigInt, data []byte) *Transaction {
	return &Transaction{types.NewTransaction(uint64(nonce), to.address, amount.bigint, gasLimit.bigint, gasPrice.bigint, zrmcom.CopyBytes(data))}
}
...
func NewTransactionFromRLP(data []byte) (*Transaction, error) {
	...
	if err := rlp.DecodeBytes(zrmcom.CopyBytes(data), tx.tx); err != nil {
		return nil, err
	}
	return tx, nil
}
...
func (tx *Transaction) WithSignature(sig []byte, chainID *BigInt) (signedTx *Transaction, _ error) {
	...
	rawTx, err := tx.tx.WithSignature(signer, zrmcom.CopyBytes(sig))
...
func NewReceiptFromRLP(data []byte) (*Receipt, error) {
	...
	if err := rlp.DecodeBytes(zrmcom.CopyBytes(data), r.receipt); err != nil {
		return nil, err
	}
	return r, nil
}
