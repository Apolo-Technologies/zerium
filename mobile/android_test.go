...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.zerium", new NodeConfig());
			node.start();
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altozrmder
	...
	workspace, err := ioutil.TempDir("", "zrmd-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/apolo-technologies/zrmdtest", "libs"} {
	...
	build.CopyFile(filepath.Join("libs", "zrmd.aar"), "zrmd.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "zerium", "zrmdtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
