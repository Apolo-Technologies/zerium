...
func (opts *TransactOpts) SetSigner(s Signer) {
	opts.opts.Signer = func(signer types.Signer, addr zrmcom.Address, tx *types.Transaction) (*types.Transaction, error) {
...
func DeployContract(opts *TransactOpts, abiJSON string, bytecode []byte, client *ZeriumClient, args *Interfaces) (contract *BoundContract, _ error) {
	// Deploy the contract to the network
	parsed, err := abi.JSON(strings.NewReader(abiJSON))
	if err != nil {
		return nil, err
	}
	addr, tx, bound, err := bind.DeployContract(&opts.opts, parsed, zrmcom.CopyBytes(bytecode), client.client, args.objects...)
	if err != nil {
		return nil, err
	}
...
func BindContract(address *Address, abiJSON string, client *ZeriumClient) (contract *BoundContract, _ error) {
	parsed, err := abi.JSON(strings.NewReader(abiJSON))
	if err != nil {
		return nil, err
	}
