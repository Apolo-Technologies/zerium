...
// Contains all the wrappers from the zrmcom package.
...
func NewHashFromBytes(binary []byte) (hash *Hash, _ error) {
	h := new(Hash)
	if err := h.SetBytes(zrmcom.CopyBytes(binary)); err != nil {
		return nil, err
	}
	return h, nil
}
...
func (h *Hash) SetBytes(hash []byte) error {
	if length := len(hash); length != zrmcom.HashLength {
		return fmt.Errorf("invalid hash length: %v != %v", length, zrmcom.HashLength)
	}
	copy(h.hash[:], hash)
	return nil
}
...
func (h *Hash) SetHex(hash string) error {
	...
	if length := len(hash); length != 2*zrmcom.HashLength {
		return fmt.Errorf("invalid hash hex length: %v != %v", length, 2*zrmcom.HashLength)
	}
...
// Address represents the 20 byte address of an Zerium account.
type Address struct {
...
func NewAddressFromBytes(binary []byte) (address *Address, _ error) {
	a := new(Address)
	if err := a.SetBytes(zrmcom.CopyBytes(binary)); err != nil {
		return nil, err
	}
	return a, nil
}
...
func (a *Address) SetBytes(address []byte) error {
	if length := len(address); length != zrmcom.AddressLength {
		return fmt.Errorf("invalid address length: %v != %v", length, zrmcom.AddressLength)
	}
...
func (a *Address) SetHex(address string) error {
	...
	if length := len(address); length != 2*zrmcom.AddressLength {
		return fmt.Errorf("invalid address hex length: %v != %v", length, 2*zrmcom.AddressLength)
	}
