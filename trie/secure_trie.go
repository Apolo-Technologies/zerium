...
func NewSecure(root zrmcom.Hash, db Database, cachelimit uint16) (*SecureTrie, error) {
	if db == nil {
...
func (t *SecureTrie) TryUpdate(key, value []byte) error {
	...
	t.getSecKeyCache()[string(hk)] = zrmcom.CopyBytes(key)
	return nil
}
...
// from the database.
func (t *SecureTrie) Commit() (root zrmcom.Hash, err error) {
	return t.CommitTo(t.trie.db)
}

func (t *SecureTrie) Hash() zrmcom.Hash {
	return t.trie.Hash()
}
...
func (t *SecureTrie) CommitTo(db DatabaseWriter) (root zrmcom.Hash, err error) {
