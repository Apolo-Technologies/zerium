...
func makeTestTrie() (zrmdb.Database, *Trie, map[string][]byte) {
	// Create an empty trie
	db, _ := zrmdb.NewMemDatabase()
	trie, _ := New(zrmcom.Hash{}, db)

	// Fill it with some arbitrary data
	content := make(map[string][]byte)
	for i := byte(0); i < 255; i++ {
		// Map the same data under multiple keys
		key, val := zrmcom.LeftPadBytes([]byte{1, i}, 32), []byte{i}
		content[string(key)] = val
		trie.Update(key, val)

		key, val = zrmcom.LeftPadBytes([]byte{2, i}, 32), []byte{i}
		content[string(key)] = val
		trie.Update(key, val)

		// Add some other data to inflate the trie
		for j := byte(3); j < 13; j++ {
			key, val = zrmcom.LeftPadBytes([]byte{j, i}, 32), []byte{j, i}
			content[string(key)] = val
			trie.Update(key, val)
		}
	}
...
func checkTrieContents(t *testing.T, db Database, root []byte, content map[string][]byte) {
	...
	trie, err := New(zrmcom.BytesToHash(root), db)
	if err != nil {
		t.Fatalf("failed to create trie at %x: %v", root, err)
	}
	if err := checkTrieConsistency(db, zrmcom.BytesToHash(root)); err != nil {
...
func checkTrieConsistency(db Database, root zrmcom.Hash) error {
...
func TestEmptyTrieSync(t *testing.T) {
	emptyA, _ := New(zrmcom.Hash{}, nil)
	emptyB, _ := New(emptyRoot, nil)

	for i, trie := range []*Trie{emptyA, emptyB} {
		db, _ := zrmdb.NewMemDatabase()
		if req := NewTrieSync(zrmcom.BytesToHash(trie.Root()), db, nil).Missing(1); len(req) != 0 {
...
func testIterativeTrieSync(t *testing.T, batch int) {
	...
	dstDb, _ := zrmdb.NewMemDatabase()
	sched := NewTrieSync(zrmcom.BytesToHash(srcTrie.Root()), dstDb, nil)

	queue := append([]zrmcom.Hash{}, sched.Missing(batch)...)
	for len(queue) > 0 {
...
func TestIterativeDelayedTrieSync(t *testing.T) {
	...
	queue := append([]zrmcom.Hash{}, sched.Missing(10000)...)
	for len(queue) > 0 {
...
func testIterativeRandomTrieSync(t *testing.T, batch int) {
	...
	queue := make(map[zrmcom.Hash]struct{})
...
func TestDuplicateAvoidanceTrieSync(t *testing.T) {
	...
	queue := append([]zrmcom.Hash{}, sched.Missing(0)...)
	requested := make(map[zrmcom.Hash]struct{})
...
func TestIncompleteTrieSync(t *testing.T) {
	...
	added := []zrmcom.Hash{}
	queue := append([]zrmcom.Hash{}, sched.Missing(1)...)
	for len(queue) > 0 {
