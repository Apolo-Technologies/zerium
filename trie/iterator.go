...
type nodeIteratorState struct {
	hash    zrmcom.Hash // Hash of the node being iterated (nil if not standalone)
	node    node        // Trie node being iterated
	parent  zrmcom.Hash // Hash of the first full ancestor node (nil if current is the root)
...
func (it *nodeIterator) Hash() zrmcom.Hash {
	if len(it.stack) == 0 {
		return zrmcom.Hash{}
	}
...
func (it *nodeIterator) Parent() zrmcom.Hash {
	if len(it.stack) == 0 {
		return zrmcom.Hash{}
	}
...
func (st *nodeIteratorState) resolve(tr *Trie, path []byte) error {
	if hash, ok := st.node.(hashNode); ok {
		...
		st.node = resolved
		st.hash = zrmcom.BytesToHash(hash)
	}
	return nil
}

func (it *nodeIterator) nextChild(parent *nodeIteratorState, ancestor zrmcom.Hash) (*nodeIteratorState, []byte, bool) {
...
func (it *differenceIterator) Hash() zrmcom.Hash {
	return it.b.Hash()
}

func (it *differenceIterator) Parent() zrmcom.Hash {
	return it.b.Parent()
}
...
func (it *unionIterator) Hash() zrmcom.Hash {
	return (*it.items)[0].Hash()
}

func (it *unionIterator) Parent() zrmcom.Hash {
...
func (it *unionIterator) Next(descend bool) bool {
	...
	for len(*it.items) > 0 && ((!descend && bytes.HasPrefix((*it.items)[0].Path(), least.Path())) || compareNodes(least, (*it.items)[0]) == 0) {
		...
		if skipped.Next(skipped.Hash() == zrmcom.Hash{}) {
			it.count += 1
