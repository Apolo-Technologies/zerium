...
func TestIteratorLargeData(t *testing.T) {
	...
	for i := byte(0); i < 255; i++ {
		value := &kv{zrmcom.LeftPadBytes([]byte{i}, 32), []byte{i}, false}
		value2 := &kv{zrmcom.LeftPadBytes([]byte{10, i}, 32), []byte{i}, false}
...
func TestNodeIteratorCoverage(t *testing.T) {
	...
	for _, key := range db.(*zrmdb.MemDatabase).Keys() {
...
func TestIteratorContinueAfterSeekError(t *testing.T) {
	...
	db, _ := zrmdb.NewMemDatabase()
	ctr, _ := New(zrmcom.Hash{}, db)
	for _, val := range testdata1 {
		ctr.Update([]byte(val.k), []byte(val.v))
	}
	root, _ := ctr.Commit()
	barNodeHash := zrmcom.HexToHash("05041990364eb72fcb1127652ce40d8bab765f2bfe53225b1170d276cc101c2e")
